...
How many levels can each technique solve by itself?
  ...
  must-cross-horizon                       6   60.0%  ████████████ (+3 via fallback)
  archetype                                4   40.0%  ████████ (+6 via fallback)
  portal-optional-endurance                2   20.0%  ████ (+7 via fallback)
  portal-optional-modern                   2   20.0%  ████ (+7 via fallback)
  portal-optional-perimeter                2   20.0%  ████ (+7 via fallback)
  endurance-longpath                       1   10.0%  ██ (+8 via fallback)
  template                                 1   10.0%  ██ (+8 via fallback)

Minimum greedy covering set (order matters):
  + structural-conservative              → +9 new (9 total)
  + archetype                            → +1 new (10 total)

Canary levels (solvable by exactly ONE technique standalone):
  archetype: L92

── BUDGET SWEEP (global time multiplier vs solve count) ────────────
Does more budget unlock new levels or just solve faster?

  Multiplier     Budget       Solved     Delta  New levels
  ──────────────────────────────────────────────────────────────────────
  0.25x          7500ms       9             -1  -
  0.5x           15000ms      10            +0  -
  1x             30000ms      10            +0  -
...
QUESTION: Which technique combination covers the most levels solo?
  Greedy minimum set: structural-conservative + archetype

QUESTION: What is the minimum technique set achieving full coverage?
  Required (regressions when disabled): archetype
  Also needed for coverage: structural-conservative (+L1, L3, L50, L120, L130, L140)
  Minimum viable set: { archetype, structural-conservative }

QUESTION: Is global budget the bottleneck (vs technique ordering)?
  NO — increasing budget does not unlock new solves; technique logic is the bottleneck.
  ×0.25 budget causes 1 failure(s); budget IS a floor for some levels.
  Solve-rate curve: ×0.25→9  ×0.5→10  ×1→10  ×2→10  ×4→10
...
1. [HIGH] Narrow the archetype classifier to avoid matching levels structural can handle
...
2. [MEDIUM] Increase stage-0 structural budget for high-reqInt levels
...
3. [MEDIUM] Promote structural-conservative to first-attempt position within stage-0
