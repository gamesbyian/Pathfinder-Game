...
- Coverage is measured, not assumed. Historical coverage snapshots are useful provenance, but regenerate current counts via `npm run stress:provenance-coverage` before decision-bearing use.
- Cold evidence is stricter than `hintGuided === false`. The classifier requires observed capability context; historical absence of `usedExistingHints` / `hintGuided` / `isolatedTechnique` is unknown rather than an implicit `false`. Use `isColdCapabilityEvidence` / purpose-specific applicability helpers and regenerate corpus shares instead of copying pre-audit percentages.
