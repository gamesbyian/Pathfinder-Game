...
| current production capability | 0 | 73,194 | 608,110* | historical deterministic cold-context events exist, but none matches current revision; 101,941 randomized events are research-context-inadmissible rather than silently cold |
...
5. **The residual atlas admitted randomized research events as rescuers.** Strict-cold compatibility classification did not exclude random seeds. Applying purpose semantics moves 31 rows from class 4 to class 5: deterministic historical production-context candidates are 169, and the no-known-rescuer population is 419/671 (62.4%), not 388/671. The observer-first priority does not change, but its correctly described target population grows.
...
9. **Legacy strict-cold classification did not distinguish randomized/enumeration modality.** That classifier remains a historical broad class for compatibility, but the purpose-aware current-production predicate now rejects randomized and exhaustive enumeration contexts before considering regime equality. A clean hint flag is not evidence that a research search modality was deployed by production.
...
No queue ordering changes. The observer-only joint-obligation pilot and bounded all-known-basin analysis remain the highest-value next work, now against the corrected 419-level no-known-rescuer population. The audit removes 31 randomized-only “rescuers,” changes the class-4/5 cohort boundary, and invalidates raw event-count ranking.
