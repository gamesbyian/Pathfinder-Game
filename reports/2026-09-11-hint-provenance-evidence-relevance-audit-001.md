...
10. **Legacy missing booleans were silently read as modern `false`.** Across the store, 505,993 provenance events omit at least `isolatedTechnique`; specifically 57,497 events previously classified strict-cold lack that field, while only 24,998 strict-cold events carry complete capability context. Flat legacy upgrades can also synthesize false context values while retaining an unknown solver version, so the historical production-context predicate requires both explicit booleans and a known version. This changes the residual atlas materially to 143 explicit historical candidates / 445 no-admissible-rescuer rows.
