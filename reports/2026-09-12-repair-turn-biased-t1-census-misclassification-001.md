...
> **Remaining gate:** none. The 9 newly class-2 levels are not a live nomination: `attempts.ts` never adds `repairTurnBiasedAttempt()` to the production ladder at all unless `STRATEGY_REPAIR_TURN_BIAS` is explicitly true (`cfg && cfg.STRATEGY_REPAIR_TURN_BIAS === true`; production passes a null `cfg`), and that flag is already **CLOSED NEGATIVE** in `docs/solver-opt-in-experiment-ledger.md` — confirmed net **−7/1700** (5 gained, 12 lost to displaced repair-family work) at matched-work population scale ([`validation`](2026-08-07-turnbias-corpus2-validation.md)). `dispatched: false` here is the already-understood, deliberate cost of that closed decision, not a new exposure bug.
> **Evidence role:** data-pipeline correctness fix plus bounded capability-memory census completion (6/6 prespecified/cheaply-recoverable sources now evaluated). No solver-policy change; no production routing change.
...
**Checked and closed, not deferred:** the 9-level `repair-turn-biased`-offered-but-not-dispatched pattern initially looked like an open WS1 exposure nomination. It is not — `STRATEGY_REPAIR_TURN_BIAS` (the only path that ever adds `repairTurnBiasedAttempt()` to the ladder) is already closed negative on matched-work population evidence (net −7/1700), so `dispatched: false` on these 9 is the known, already-priced cost of that decision, not a routing bug. Per the standing rule against reopening closed forms without a changed premise, no further WS1 action is warranted here.
