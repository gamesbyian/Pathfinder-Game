...
>
> **2026-09-12 provenance note (solver workflow remediation, wrong-ref/report-dependency audit):** `reports/stress/solver-evidence-integrity-index.json` fail-closed marks the whole pre-redesign `solver-routing-regime-sample-ab.yml`/`solver-archetype-sample-ab.yml` two-dispatch family `invalid` as paired evidence, because that design had no shared sealed population artifact and could not mechanically prove the two dispatches searched the same population. For this specific run that concern does not actually apply: both arms are documented above as running "at the identical commit" with only `enable_flags` differing, and the selector (`select-routing-regime-sample.mjs`) draws from a static committed corpus file via a seeded `mulberry32` PRNG (FNV-1a-hashed seed), not the level *generator* whose independent-invocation nondeterminism motivated the fail-closed default -- same commit + same seed + a deterministic sampler is sufficient to establish population identity from the committed record alone. The 0 gains/0 losses conclusion above stands; no rerun is required. Future routing-regime-gated A/Bs should still use the redesigned single-dispatch `solver-routing-regime-sample-ab.yml` (one sealed population, two immutable arm SHAs), which no longer requires this kind of after-the-fact review.
