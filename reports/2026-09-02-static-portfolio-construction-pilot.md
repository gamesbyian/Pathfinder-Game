...
> **Last evidence:** 2026-09-02 — this report, including all three shared-envelope execution runs below
> **Decision:** Do not build a rung-2 fixed-work static portfolio from the EW1 60-level greedy ranking alone (3 of its top-7 picks never win in real production — use the real production win-frequency ranking instead, with EW1 only for its complementary oracle-exclusivity signal). The naive "rank by production win-count, run sequentially first-come-first-served, shared work budget" execution design is closed: `repair|score=repair|guidance=standard` is non-naturally-terminating on this population and starves every later list position at any locally-testable budget. The corrected design — `technique-census-cell.mjs`'s new `cell.perTechniqueWorkCap` (bounded per-technique shares, mirroring production's real reserve-based scheduling) — is now implemented, unit-tested (including a regression test for a real false-positive bug found and fixed along the way), and produced one clean small-scale demonstration: identical solved sets between `full-menu` and `portfolio-11`, a 0.26% extra work cost for the full menu. This is a real positive signal for rung 2's premise, not yet a population-scale confirmation (only 2/60 levels solved at this modest scale).
> **Remaining gate:** a population-scale development A/B using the now-corrected `perTechniqueWorkCap` mechanism, on a population that is not the already-heavily-mined EW1 60-level sample, at an envelope large enough to give the compared portfolio sizes a well-powered chance to differ.
...
Raw result: [`reports/stress/portfolio/static-portfolio-execution-10m.json`](stress/portfolio/static-portfolio-execution-10m.json).
...
### Second run (67,000,000 shared budget, `full-menu` vs. `portfolio-11` only): also identical, and now fully explained

Relaunched with the same 60 levels at **67,000,000** shared work per cell — the envelope this program's own prior real A/Bs used (`node_budget=50,000,000` → `work=67,000,000`, e.g. the `finishFirst` concentrated-population A/B) — and only two arms (`full-menu`, `portfolio-11`; dropping `portfolio-15` since it can only ever match one of the other two once the shared 11-technique prefix's budget is spent, and cutting it saves a third of the compute).

Raw result: [`reports/stress/portfolio/static-portfolio-execution-67m.json`](stress/portfolio/static-portfolio-execution-67m.json).

**Result: still byte-identical between arms** — 7/60 solved (same exact levels: `R01673`, `R01860`, `R02095`, `R02128`, `R02657`, `R02940`, `R03241`, all via `repair|score=repair|guidance=standard`, position 1 in both lists), 3,688,988,309 total charged work, 53/60 hitting `work-budget-reached`, in both arms. A naive read might call this a clean confirmation that the tail is safe to drop even at 6.7x the budget — **that read is wrong, and chasing it would be overclaiming.**

**Root cause, checked directly rather than assumed:** `repair|score=repair|guidance=standard` — position 1 in both arms, since it is the single highest real-production-win technique — is a right-censored, non-naturally-terminating search on this population: its own EW1 per-technique pricing (each run independently at a 10,000,000 cap) shows `work-budget-reached` on **58/60 cells**, every one landing within a few thousand work units of the exact 10,000,000 boundary (e.g. `10,000,011`, `10,002,425`, `10,000,067` — not a coincidence of the search happening to finish there, but the cap cutting it off mid-search). Compare the next five techniques in rank order, which are beam searches: 55-58/60 of their own EW1 cells are `exhausted` (a real, natural stopping point — the beam ran out of frontier, not budget). **Repair simply keeps searching for as much budget as it is given, on this population; nothing after it in a naive first-come-first-served sequential list ever gets a real turn, at 10M or at 67M or (extrapolating) at any budget this cheap to test locally.** This is not a discovery about repair being broken — it is exactly why real production never runs repair this way: production gives repair several separate, deliberately bounded allocations (`early-repair-search`'s small probe, `late-repair-search`, `late-repair-multiseed-retry`'s per-seed caps, `repair-fallback`'s own reserve) interleaved with everything else, precisely the `BudgetEnvelope`/explicit-ownership machinery Workstream 2's own budget-model program (this same row, above) spent significant effort building. A naive "rank by production win-count, run sequentially first-come-first-served, shared budget" portfolio construction is fundamentally incompatible with any non-terminating technique occupying an early list position, independent of total budget size.

**This closes the exact experimental design tested here** (naive rank-ordered sequential shared-budget execution via `build-static-portfolio-plan.mjs`'s arms) as unable to answer the portfolio-cardinality question on this population, not because the underlying tooling is wrong (it correctly reproduced real early-exit-on-success semantics) but because the input ordering was wrong for this technique mix.

### Third run: `cell.perTechniqueWorkCap` implemented, one real bug found and fixed, then a genuine (small, clean) result

Option (a) above — a per-technique bounded share within the shared envelope — was implemented as `technique-census-cell.mjs`'s new `cell.perTechniqueWorkCap` field (opt-in, additive, 15 unit tests, byte-identical when unset; see that file's own header comment and `scripts/build-static-portfolio-plan.mjs`'s matching `--per-technique-work-cap` option). No solver/production code changed.

**First validation attempt found a real bug, not a result.** At `--work-budget=10000000 --per-technique-work-cap=1000000` (60 levels, `full-menu` vs `portfolio-11`), 118/120 cells came back `deadline-truncated` after only ~1-2 seconds of wall time each — clearly not genuine 10-minute wall-clock timeouts. Root cause: `runCell`'s existing deadline-truncation discriminator compared an attempt's spend against the whole **gate ceiling**, an inference that was only valid *before* a per-technique cap existed (a right-censored technique could previously only ever mean it consumed the *entire* remaining gate share — a per-technique cap deliberately breaks that by letting a technique right-censor at its own small share while the gate ceiling still has plenty left for the next technique, which the old check misclassified as a cell-aborting wall-clock problem). Fixed by comparing against the attempt's own allocated share instead (reduces to the exact original comparison byte-for-byte when `perTechniqueWorkCap` is unset); 2 new regression tests cover both the false positive this fixes and the genuine-wall-clock-timeout case it must still catch. Full account in the fix's own commit message.

**Rerunning after the fix produced a real, non-degenerate result** (status distribution now sane: 115/120 `work-budget-reached`, 4 `success`, 1 `exhausted` — no more spurious truncation):

| arm | solved | solved levels | total work | work-budget-reached |
|---|---:|---|---:|---:|
| `full-menu` (34 techniques) | 2/60 | `R02657`, `R03171` | 582,074,181 | 58/60 |
| `portfolio-11` | 2/60 | `R02657`, `R03171` | 580,564,180 | 57/60 |

Raw result: [`reports/stress/portfolio/static-portfolio-execution-10m-capped1m.json`](stress/portfolio/static-portfolio-execution-10m-capped1m.json).

**Identical solved sets, a small (0.26%) extra work cost for the full menu.** Unlike the first two runs, the arms are no longer byte-identical — the tail (positions 12-34) genuinely got exercised on at least one level this time (one level's status differs, `work-budget-reached` vs `exhausted`) — but it contributed **zero additional coverage** for a **1,509,001-unit (0.26%) extra work charge**. This is a real, if small-scale, positive signal for rung 2's premise (dropping the tail costs a little work and loses nothing, on this sample), consistent with the much larger real-production win-frequency finding above (top-11 already covers 88.3% of wins), but it must not be read as confirmatory: only 2/60 levels solved at all at this modest 10M/1M-cap scale, far too few to power a real coverage claim, and this is still the same heavily-mined 60-level EW1 hard-residual sample every prior run in this report used.

**Disposition:** the naive first-come-first-served execution design is closed (as stated above); the corrected bounded-share design is now implemented, unit-tested, and has produced one clean small-scale demonstration run — but not yet a population-scale, envelope-realistic confirmation. The next gate, if this line continues, is exactly what the "Next gate" section above already called for: a development A/B on a population that is not the already-heavily-mined EW1 60-level sample, now using the corrected `perTechniqueWorkCap` mechanism, at an envelope large enough to give the compared portfolio sizes a genuine, well-powered chance to differ.
