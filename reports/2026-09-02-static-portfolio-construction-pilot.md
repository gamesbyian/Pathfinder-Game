...
> **Last evidence:** 2026-09-02 — this report, including both shared-envelope execution runs below
> **Decision:** Do not build a rung-2 fixed-work static portfolio from the EW1 60-level greedy ranking alone (3 of its top-7 picks never win in real production — use the real production win-frequency ranking instead, with EW1 only for its complementary oracle-exclusivity signal). Separately, the naive "rank by production win-count, run sequentially first-come-first-served, shared work budget" portfolio *execution* design tested here is closed: `repair|score=repair|guidance=standard` is a non-naturally-terminating search on this population (58/60 EW1 cells right-censored at their own independent cap, not exhausted) and starves every later list position at any locally-testable budget, at position 1 by raw win-count. A real execution test needs bounded per-technique shares (mirroring production's existing reserve-based scheduling) or a repair-light/repair-last ordering, neither implemented here.
> **Remaining gate:** none for this report. If gate-sequence (C) rung 2 continues, the next step is a corrected execution design (bounded per-technique shares or repair-aware ordering) — see the addendum's closing paragraph.
...
**This closes the exact experimental design tested here** (naive rank-ordered sequential shared-budget execution via `build-static-portfolio-plan.mjs`'s arms) as unable to answer the portfolio-cardinality question on this population, not because the underlying tooling is wrong (it correctly reproduced real early-exit-on-success semantics) but because the input ordering was wrong for this technique mix. A future attempt needs one of: (a) a per-technique bounded share within the shared envelope, mirroring production's real reserve-based scheduling, rather than unbounded first-come-first-served; or (b) placing non-terminating techniques (repair) last or in their own capped slot rather than first by raw win-count; or (c) testing on a population where the leading techniques do naturally terminate (most of this EW1 hard-residual sample's leading beams do exhaust — only repair does not — so a repair-light ordering might already behave informatively; untested here). None of these are implemented; this report closes the naive design as tested and hands the corrected premise to the next attempt.
