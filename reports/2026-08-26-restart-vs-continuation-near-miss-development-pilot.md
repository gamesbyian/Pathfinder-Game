# Restart-vs-continuation development pilot: near-miss residual stratum
...
> **Decision:** on this development population, the audit's primary comparison — seed 0 continued to `W` versus seed 0 to `W/2` then fresh seed 1 to `W/2` — shows continuation **beating** the naive two-seed 50/50 restart schedule, not merely matching it: restart reached a worse best-badness than continuation on 18/20 levels (1 tie, 1 better; mean badness delta +7.3, median +5), and neither arm converted any of the 20 gates into an actual solve (0/20 both). Per [`2026-08-24-restart-continuation-value-audit.md`](2026-08-24-restart-continuation-value-audit.md)'s own stop gate ("equal-work continuation matches or beats the tested restart schedule"), this closes the naive two-seed 50/50 restart form for this population/budget regime.
> **Remaining gate:** none for the tested form. Reopening the two-seed 50/50 split needs materially new evidence (a different population, a different split ratio, or a different work band) — the audit already anticipated this direction ("continuing repair can remain valuable well into deep work bands").
> **Evidence role:** discovery (development pilot, not confirmation)
> **Selection:** prespecified population/budget with one disclosed recalibration before any outcome was inspected (see "Methodology correction" below); the two runs used to produce the reported numbers are exact reruns of the same prespecified design, not a search over alternatives.
