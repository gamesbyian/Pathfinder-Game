...
### Frozen execution result — **tested explanatory form closed**

On the two prespecified 5K plain-vs-mechanic-buckets contrasts:

- objective: raw portal-pair count is the strongest tested separator (`d=-0.677`); twist count (`-0.617`), same-parity count (`-0.545`), twist fraction (`-0.471`) and gate-demand terms are weaker;
- intersection-harvest: raw portal-pair count is again strongest (`d=-0.825`); same-parity count (`-0.744`), twist count (`-0.692`) and twist fraction (`-0.653`) remain weaker.

The richer parity/portal decomposition therefore adds **no meaningful descriptive signal beyond raw portal count in the tested form**. Close this explanatory form. Do not nominate a parity-derived selector or exact premise from these associations.

Materially different siblings remain possible only if the changed object is explicit, for example portal placement relative to a proved cut/interface rather than another decomposition of portal count.
...
### Frozen execution result — **static descriptor basis closed**

The transformation-aware basis does not yield a stable static explanation across the two CW/CCW contrasts:

- perimeter beam: 104 CW-only / 111 CCW-only; strongest tested effect is block absolute moment at `|d|=0.249`, followed by constrained absolute moment `0.224`;
- perimeter DFS: 42 CW-only / 50 CCW-only; strongest effects are must-cross absolute moment `0.426`, portal-terminal absolute moment `0.415`, and constrained absolute moment `0.403`.

The leading descriptors differ across the two technique families, and the beam separation is modest despite a large disagreement population. The declared metamorphic transform law remains useful as a correctness guard, but this static feature family does not explain the direction disagreement strongly enough to justify more coordinate mining.

**Next question:** selected operational traces. Compare where CW/CCW begin retaining different candidate regimes, especially around obstacle/obligation ordering and portal/flipper interactions. Historical winner identity remains offline selection only.
...
### Frozen execution result — **non-nested survivor regimes confirmed**

The paired oracle ran on the exact frozen narrower-beam-only cohorts across the published, stress and random corpus documents.

Objective-first 2K-only cohort:
- 29 parents / 32 comparable gates;
- only 12/32 gates have the 2K frontier fully contained in 5K;
- **20/29 parents** have at least one gate with 2K-only frontier support;
- mean Jaccard = **0.412**;
- aggregate comparable support: 46,398 shared, 9,339 2K-only, 84,855 5K-only prefixes.

Intersection-harvest 2K-only cohort:
- 39 parents / 42 comparable gates;
- only 17/42 gates have the 2K frontier fully contained in 5K;
- **25/39 parents** have at least one gate with 2K-only frontier support;
- mean Jaccard = **0.433**;
- aggregate comparable support: 65,317 shared, 9,156 2K-only, 103,716 5K-only prefixes.

Every parent counted as non-contained above also has support exclusive in the opposite direction at a comparable gate. The wider beam is therefore not merely a superset at the matched checkpoint: ranking/displacement produces materially different survivor regimes.

This earns the **retention/dominance/regime-coverage explanation lane**, not a width treatment. Freeze a bounded sample of 2K-only and 5K-only prefixes, then test viability/option containment or a smaller operational decision seam. Frontier membership alone still does not establish future feasibility or production value.
...
The next code should **not** be a selector. The parity/portal decomposition and tested static orientation basis have now received negative explanatory dispositions. The surviving response-guided lane is the paired-width retention/regime question plus a separate CW/CCW operational-trace question; both remain development-only.
...
# Width oracle uses the exact frozen 2K-only IDs across P/S/R corpus inputs; see Nomination 3.
```
