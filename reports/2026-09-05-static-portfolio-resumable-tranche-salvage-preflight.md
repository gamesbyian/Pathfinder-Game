...
> **Remaining gate:** make exact work-boundary continuation correct at production beam widths 2000/5000, then run the fixed-work development A/B below on a fresh population. A positive result earns residual-lane design; a null/negative closes this salvage form. **2026-09-10:** re-confirmed still unmet (test-locked, not just documented — see "Current status" under the engineering-feasibility-gate section below); two design paths identified, neither implemented — this is the concrete next step whenever this candidate is picked up.
...
**Recommendation:** do not implement either path opportunistically inside an unrelated pass. This is solver-hot-path state-machine surgery with real correctness stakes (option 1) or a measured-precision tradeoff that changes what the eventual development A/B can honestly claim (option 2) — it deserves its own scoped implementation + the same equivalence-test rigor `beam-resumability-pilot.test.ts` already established for the phase-boundary case, not a rushed addition alongside unrelated queue work. Treat "implement and validate one of the two paths above" as the next concrete step when this candidate is actively picked up, with option 2 as the cheaper first probe (measure actual overshoot at width 2000/5000 on a few representative levels before deciding whether it's small enough to accept, or whether option 1's exactness is required).
