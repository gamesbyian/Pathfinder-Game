...
Temporary PR-only workflow `tmp-seven-day-capability-reconcile.yml` was added only to execute repository analysis code against committed data. It ran no solver campaign.

Run `34728842684` first established a hard lower bound from source identities that required no technique-niche label recovery. Run `34728960554` then enumerated the actual `solvingActions` strings in `reports/stress/technique-niches/2026-09-03/level-capability.json` and rebuilt all six named source families through the repository's own `solver-capability-memory-lib.mjs` set-union semantics.

The exact recovered action identities were:
...
The first pass's guessed shorter names had correctly been treated as unknown rather than silently converted to zero evidence.

Both runs reproduced the boundary and atlas exactly:

- 1,700/1,700 conclusive;
- 1,048 solved, 652 residual;
- classes **22 / 39 / 37 / 123 / 431**.

### Exact six-source reconstruction

| source | demonstrated historical IDs | current residual nominations | unique nominations within six-source panel | class 1 | class 2 | class 3 | class 4 | class 5 |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| portal coarse-state merge referee-valid gain set | 158 | **137** | 128 | 6 | 9 | 9 | **113** | 0 |
| repair turn-biased isolated capability | 184 | **25** | 18 | 0 | 25 | 0 | 0 | 0 |
| intersectionHarvest mechanic-bucket beam | 713 | **9** | 5 | 8 | 0 | 1 | 0 | 0 |
| objectiveFirst mechanic-bucket beam | 705 | **5** | 3 | 3 | 0 | 2 | 0 | 0 |
| goal-attraction-disabled promoted +3 set | 3 | **0** | 0 | 0 | 0 | 0 | 0 | 0 |
| production-boundary displaced solved-set losses | 16 | **16** | 12 | 0 | 1 | 12 | 3 | 0 |
| **six-source union** | — | **179** | — | **14** | **28** | **21** | **116** | **0** |

The capability-memory library independently reports `union.nominated = 179`; the explicit ID union is also 179.

This also clarifies a source-identity wrinkle hidden by the earlier prose: the relevant repair capability source is ordinary **turn-biased** guidance, not the separate **must-turn-biased** seven-level late-repair seam now active in the class-2 queue. Those are distinct evidence lines and should not be conflated.

## What changes scientifically

### Class 5 remains an acquisition frontier under the tested evidence

The corrected six-source panel nominates **zero of 431 class-5 rows**. The broader class-5 conclusion is independently supported by the corrected T1 census, freshness work, exact extinction evidence and failed nearby retention/scoring/descriptor routes.

The correction is therefore **not** “class 5 was secretly composition.” It is more precise: known capability is highly relevant to the easier residual classes but still fails to penetrate the current class-5 frontier.

### Class 4 was prematurely flattened into the class-5 conclusion

The six-source panel nominates **116/123 class-4 rows**. One recent, mechanistically distinct, referee-valid historical treatment, portal coarse-state merge, accounts for **113/123** by itself.

That treatment was correctly rejected globally because it also lost 12 control solves and produced a hard capability regression on `R01273`. Its later collision forensic closed bounded second-survivor and predecessor-history subkey salvage forms.
...
### Classes 1–3 also confirm the value of source-specific joins

The panel nominates 14/22 class-1 rows, 28/39 class-2 rows and 21/37 class-3 rows, but those aggregate overlaps do not supersede the dedicated class-1/2/3 rejoin. That rejoin already asked whether the capability is legally exposed, reached, starved or failed under current production. Its smaller actionable descendant, the seven-level must-turn-biased seam, remains the correct class-2 changed-treatment gate.

The capability-memory correction therefore changes **class 4**, where freshness had not yet been performed, rather than reopening already-adjudicated class-1/2/3 policy forms.
...
That cheap check would have caught the error immediately. The exact source action identities should also be copied from the artifact/tool output rather than shortened by hand when they are decision-bearing.
