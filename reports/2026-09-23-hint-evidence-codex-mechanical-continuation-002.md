...
> **Last evidence:** 2026-09-23 — Codex local validation at commit `ed49b458`, followed by clean transplant onto the #2002 continuation branch
...
> **Remaining gate:** rerun the two owner tests and broader branch validation after current-main reconciliation


## Reconciliation into #2002

This tranche applied cleanly on top of the continuation branch. The #2002 implementation had not
changed the three touched join/applicability surfaces, so no semantic conflict resolution was needed.

Integrated unchanged in substance:

- explicit counts for failure/discovery records that abstain because their comparability identity is
  incomplete;
- tests proving that acquisition `runId` is not a semantic equality key;
- tests proving that different protocols remain incomparable even inside one run;
- tests proving missing historical protocol/revision remains unknown.

The continuation branch already owns canonical execution-protocol and solver-request compatibility
layers. Future migration of this join should resolve canonical/mixed-era protocol/request identity
*before* the existing exact-key boundary while retaining these abstention counters and tests.
