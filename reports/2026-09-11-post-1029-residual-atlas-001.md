...
- **no T1 winner:** cross-checked against the level's hint store for an explicit historical production-context entry: strict-cold Pathfinder, non-enumeration, known solver version, with all three capability context fields actually present. Missing `isolatedTechnique` is legacy unknown rather than `false`. Randomized repair remains eligible when its production context is explicit. Present -> class 4; absent/ambiguous -> class 5.
...
| 4 | no T1 winner but an explicit historical production-context candidate exists | **143** | 143 |
| 5 | no known admissible rescuer after cross-evidence reconciliation | **445** | 445 |

Class 4's explicit historical provenance-candidate techniques on the audited rerun: beam 129, repair 7, admissible-order-fallback 7. The large change is legacy ambiguity, not age: 57,497 strict-cold events across the full store omit `isolatedTechnique`, so they never recorded whether they came from production or an isolated run. Those paths remain atlas/oracle/history evidence but cannot fill a capability-rescuer cell.
...
| 4 | 143 | 138 (96.5%) | 101 | 112 | 18 | 13 | 78 |
| 5 | 445 | 304 (68.3%) | 249 | 367 | 27 | 44 | 139 |
...
Portal-bearing levels remain dominant, including class 3 (75%), explicit class 4 (96.5%), and class 5/ambiguous (68.3%). Missing legacy context moves a row to the unresolved class; randomization alone does not.
