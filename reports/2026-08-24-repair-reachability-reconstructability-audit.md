...
> **Status:** active
> **Last evidence:** 2026-09-02 — a fresh, prespecified, unrelated-parent recurrence check (`R02257:elite:3`, `R02426:elite:4`) neither of the two previously-named regimes recurred on; instead both land in a new near-budget-boundary shape, itself now recurring across those two independent cases
> **Decision:** do not design a new large repair operator yet. Six exact-live cases are now classified across **four distinct cost/regime shapes**: `R00648` defeats `closeLengthGap` even at 500x its production node budget (operator-incapable); `R03176` also defeats the direct diagnostic but the whole repair process eventually solves it from a different restart trajectory; `R00630` is solved by `closeLengthGap` in just 3,247 nodes, *under* its own 4,000-node production budget (cheap reconstruction — a retreat/reopening question, not a reconstruction one); `R02449` is also solved by `closeLengthGap`, but only at 1,268,180 nodes, 317x its production budget (a genuine budget-scale gap, so far a singleton — the near-budget pair below does not generalize this far outlier); `R02257` (4,674 nodes, 1.17x budget) and `R02426` (5,060 nodes, 1.27x budget) are a new, distinct, already-**recurring** shape: reconstructable, but only modestly (not dramatically) above production's own budget. Randomized rollout fails on five of six cases (0/2000); `R02426` is the first case with any blind-rollout success at all (2/2000). See [`2026-08-27 classification`](../reports/2026-08-27-repair-live-prefix-reconstruction-classification-r00630-r02449.md) and [`2026-09-02 near-budget-boundary recurrence`](../reports/2026-09-02-repair-live-prefix-reconstruction-near-budget-boundary-recurrence.md).
> **Remaining gate:** the near-budget-boundary shape (`R02257`/`R02426`) is n=2 — a third+ independent unrelated case landing in the same zone would meaningfully strengthen a future small-budget-increase candidate; do not design that mechanism yet. `R00630`'s comfortably-under-budget shape and `R02449`'s far-over-budget shape remain unrecurred singletons — do not build retreat or reconstruction-budget machinery from either alone. Use known-dead points only as correctness controls when cheap; do not buy more CP-SAT resolution merely to narrow existing UNKNOWN intervals (`R02449`'s own `[20,36]` interior stays open).
...
## Answered (2026-09-02): the recurrence check finds a fourth shape, not either named one

A fresh, prespecified, independent 30-level sample (selected before either case's classification was seen — see [`2026-09-02-repair-live-prefix-reconstruction-near-budget-boundary-recurrence.md`](2026-09-02-repair-live-prefix-reconstruction-near-budget-boundary-recurrence.md) for the full selection method) drew two unrelated-parent exact-live cases, `R02257:elite:3` (`low=18`, `high=19`) and `R02426:elite:4` (`low=54`, `high=55`), and ran both through the identical `closeLengthGap`/rollout diagnostic, referee-validated and independently replayed.

Neither `R00630`'s comfortably-under-budget shape nor `R02449`'s 317x-over-budget shape recurred. Instead, both new cases land in a shape neither prior case occupied: reconstructable, but only **modestly** above the 4,000-node production budget — `R02257` at 4,674 nodes (1.17x) and `R02426` at 5,060 nodes (1.27x). This is a genuine two-case recurrence of that specific near-budget-boundary shape, distinct from both `R00630` and `R02449`, and reframes the two originally-named "cheap" and "expensive" extremes as opposite ends of what may be a continuum rather than two isolated clusters (`R00630` at 3,247 nodes sits within about 1,800 nodes of both new cases despite being on the other side of the exact budget line; `R02449` at 1,268,180 nodes is over two orders of magnitude further out and remains an unrecurred singleton). `R02426` is also the first case in this program with any nonzero randomized-rollout success (2/2000) — flagged, not investigated further.

Per the parent stop rule, n=2 does not yet justify designing a specific budget-increase mechanism (even a small one). The next gate is a third+ independent unrelated case, selected the same prespecified way, before proposing any concrete treatment.
