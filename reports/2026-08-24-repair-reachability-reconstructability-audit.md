...
> **Status:** active
> **Last evidence:** 2026-09-02 — a fresh, prespecified, unrelated-parent recurrence check drew three independent cases (`R02257:elite:3`, `R02426:elite:4`, `R03097:elite:4`); two recurred a new near-budget-boundary shape, the third recurred the operator-incapable shape instead — three draws, three different shapes
> **Decision:** do not design a new large repair operator yet. Seven exact-live cases are now classified across **four distinct cost/regime shapes**: `R00648`/`R03176`/`R03097` defeat `closeLengthGap` even at 500x-plus its production node budget (operator-incapable — a majority of the sample, not a rare edge); `R03176` alone is known to have the whole repair process eventually solve it from a different restart trajectory (untested for `R03097`); `R00630` is solved by `closeLengthGap` in just 3,247 nodes, *under* its own 4,000-node production budget (cheap reconstruction — a retreat/reopening question, not a reconstruction one, still a singleton); `R02449` is also solved, but only at 1,268,180 nodes, 317x its production budget (a genuine budget-scale gap, also still a singleton); `R02257` (4,674 nodes, 1.17x budget) and `R02426` (5,060 nodes, 1.27x budget) are a new, distinct shape that **did** recur across those two independent draws: reconstructable, but only modestly above production's own budget. Randomized rollout fails on six of seven cases (0/2000); `R02426` is the first case with any blind-rollout success at all (2/2000). See [`2026-08-27 classification`](../reports/2026-08-27-repair-live-prefix-reconstruction-classification-r00630-r02449.md) and [`2026-09-02 near-budget-boundary recurrence`](../reports/2026-09-02-repair-live-prefix-reconstruction-near-budget-boundary-recurrence.md).
> **Remaining gate:** three independent draws landing in three different shapes (2 near-budget-boundary, 1 operator-incapable) is itself the finding — no single shape can yet be treated as dominant, so do not design any mechanism (retreat, reconstruction-budget, or destroy) from this population. The next gate is a materially larger sample with an explicit per-shape rate estimate, not one or two more individually-classified cases. Use known-dead points only as correctness controls when cheap; do not buy more CP-SAT resolution merely to narrow existing UNKNOWN intervals (`R02449`'s own `[20,36]` interior stays open).
> **Evidence role:** discovery
> **Selection:** observational — cases and candidate descriptors come from already-mined repair-retreat and beam-extinction evidence, plus (2026-09-02) one freshly-drawn independent stratified sample for the recurrence check specifically.
...
## Answered (2026-09-02): the recurrence check finds a fourth shape, then a third draw shows the population is heterogeneous

A fresh, prespecified, independent 30-level sample (each case run through CP-SAT bisection and classification before the next was selected, but the selection rule fixed before seeing any outcome — see [`2026-09-02-repair-live-prefix-reconstruction-near-budget-boundary-recurrence.md`](2026-09-02-repair-live-prefix-reconstruction-near-budget-boundary-recurrence.md) for the full method) drew three unrelated-parent exact-live cases in ascending shallowest-rollback order: `R02257:elite:3` (`low=18`, `high=19`), `R02426:elite:4` (`low=54`, `high=55`), and `R03097:elite:4` (`low=7`, `high=8`) — all run through the identical `closeLengthGap`/rollout diagnostic, referee-validated and independently replayed where a solve was found.

Neither `R00630`'s comfortably-under-budget shape nor `R02449`'s 317x-over-budget shape recurred. The first two new cases instead landed in a shape neither prior case occupied: reconstructable, but only **modestly** above the 4,000-node production budget — `R02257` at 4,674 nodes (1.17x) and `R02426` at 5,060 nodes (1.27x), a genuine two-case recurrence of that specific near-budget-boundary shape. But the third draw, `R03097:elite:4`, broke the pattern: it defeats `closeLengthGap` entirely, exhausting the full 2,000,000-node diagnostic ceiling without solving — recurring the `R00648`/`R03176` operator-incapable shape instead. Three independent, unrelated-parent draws landed in **three different shapes**. `R02426` is also the first case in this program with any nonzero randomized-rollout success (2/2000) — flagged, not investigated further.

This is the more informative outcome, not a weaker one: it shows the near-budget-boundary shape is real (two independent recurrences, both referee-verified) but the population is heterogeneous rather than dominated by one shape, so a budget-increase mechanism sized for the near-budget-boundary cases would do nothing for the operator-incapable ones a fresh draw has a real chance of hitting. Per the parent stop rule, this argues for a materially larger sample with an explicit per-shape rate estimate as the next gate, not one or two more individually-classified cases and not any mechanism design yet.
