...
> **Remaining gate:** none for this exact recurrence-check design — it reached its own target sample size and answered its question. If this workstream continues, the next gate is a qualitatively different, freshly-prespecified question (e.g. correlating operator-incapability or reconstructable-case cost with a legal static level feature), not a fourth same-design batch. Use known-dead points only as correctness controls when cheap; do not buy more CP-SAT resolution merely to narrow existing UNKNOWN intervals (`R02449`'s own `[20,36]` interior and `R02919`'s wide-open interior both stay open).
