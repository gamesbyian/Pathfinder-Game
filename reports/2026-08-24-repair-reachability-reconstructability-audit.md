...
> **Status:** active
> **Last evidence:** 2026-09-02 — a third, prespecified fresh batch (12 unrelated-parent cases, all CP-SAT-resolved, 4 solved/8 operator-incapable) reached the recurrence-check's own "order 20-30+" target: **6/28 (21.4%) reconstructable, 22/28 (78.6%) operator-incapable**, and found that reconstruction cost among the reconstructable minority is a heavy-tailed continuum (0.22x-317x the production budget), not the two-cluster shape the first batch's n=2 near-budget-boundary pair suggested.
> **Decision:** do not design a new large repair operator. Operator-incapability is the dominant outcome (~79%, stable in shape from n=16 to n=28) for an unrelated exact-live case. "Near-budget-boundary" is retired as a distinct regime — `R00630` (0.81x), `R02257` (1.17x), `R02426` (1.27x), `R02344` (19.76x), `R03104` (22.70x), `R02990` (115.40x), and `R02449` (317x) are now understood as points on one continuum, not two clusters. This makes a single fixed-budget-increase mechanism a *harder* target than the earlier reading implied (no modest multiplier catches both the near-budget and far-over-budget cases without wildly overpaying on the near/under-budget ones). See [`2026-08-27 classification`](../reports/2026-08-27-repair-live-prefix-reconstruction-classification-r00630-r02449.md) and [`2026-09-02 rate-estimate report`](../reports/2026-09-02-repair-live-prefix-reconstruction-near-budget-boundary-recurrence.md) (batches 1-3, n=28 total).
> **Remaining gate:** none for this exact recurrence-check design — it reached its own target sample size and answered its question. **2026-09-10:** ran the named next question (correlating operator-incapability with a legal static level feature) as a cheap join over the already-classified 30-case population, no new CP-SAT/solver compute — see [`2026-09-10 static-feature correlation`](../reports/2026-09-10-repair-operator-incapability-static-feature-correlation-001.md). Static block count shows the only non-trivial separation (AUC 0.69, reconstructable cases have fewer blocks), but at n=8 vs. n=22 this is a candidate signal, not a confirmed correlation. If this workstream continues, the next gate is a **fresh, prespecified sample** testing the block-count hypothesis specifically through the same CP-SAT+rollout+`closeLengthGap` pipeline (real, non-cheap compute) — not another same-design recurrence batch, and not a mechanism designed from the candidate signal alone. Use known-dead points only as correctness controls when cheap; do not buy more CP-SAT resolution merely to narrow existing UNKNOWN intervals (`R02449`'s own `[20,36]` interior and `R02919`'s wide-open interior both stay open).
> **Evidence role:** discovery
> **Selection:** observational — cases and candidate descriptors come from already-mined repair-retreat and beam-extinction evidence, plus (2026-09-02) three freshly-drawn independent stratified samples for the recurrence check specifically (batch 1, seed `repair-reachability-recurrence-check-2026-09-02`; batch 2, seed `repair-reachability-recurrence-check-2026-09-02-batch2`; batch 3, seed `repair-reachability-recurrence-check-2026-09-02-batch3`).
...
### Same day, third batch: n=28, target reached — and the regime framing itself revised

A third independent 40-level sample (seed `repair-reachability-recurrence-check-2026-09-02-batch3`, no overlap with any of the 24 previously-classified/abstained levels) drew the next 12 distinct-parent cases in the same fixed order: `R02958:elite:3`, `R02134:elite:0`, `R02344:elite:0`, `R02413:elite:3`, `R02990:elite:0`, `R03020:elite:0`, `R03104:elite:3`, `R00500:elite:0`, `R01936:elite:3`, `R03187:elite:4`, `R00479:elite:4`, `R02265:elite:2`. All 12 reached a usable CP-SAT boundary (three transient full-length abstentions and one mid-bisection abstention all resolved cleanly on a single targeted retry, matching batch 2's own finding that these are typically flukes; `R00500` alone kept a real but imprecise 3-step-wide boundary after two identical abstentions at the same depth — included with a caveat rather than excluded, since unlike `R02919` its `high` bound narrowed substantially from the elite's full length before that point). **4/12 solved, 8/12 operator-incapable.** All four solves were independently referee-verified (`validateCandidatePath` plus a from-scratch `isSolutionState` replay), via a check now built directly into `repair-plateau-rollout-classifier.mjs` rather than done by hand.

The real finding is not the count but the shape: this batch's four reconstructable cases cost 0.22x, 19.76x, 22.70x, and 115.40x the 4,000-node production budget — none anywhere near batch 1's tight 1.17x/1.27x "near-budget-boundary" pair. Combined with `R00630` (0.81x) and `R02449` (317x) from the original mining, eight known-reconstructable cases now span nearly three orders of magnitude with no gap wide enough to call any two of them a shared cluster. **This retires "near-budget-boundary" as a distinct regime**: batch 1's n=2 pair was two adjacent points on a heavy-tailed continuum, not a second cluster next to `R02449`'s outlier. The operator-incapable/reconstructable split itself is unaffected and remains the dominant distinction.

Combining all three batches (excluding `R02919`'s abstention, unchanged): **28 resolved cases, 6/28 (21.4%) reconstructable, 22/28 (78.6%) operator-incapable** — reaching this report's own "order 20-30+" target. The rate itself moved back up from batch 2's all-operator-incapable 12.5% toward the original n=6 read (a reminder that even n≈16-28 batch-to-batch composition varies meaningfully, not a contradiction of either prior read). Operator-incapability's dominance is stable in shape across n=16→n=28. The heavy-tailed reconstruction cost among the minority that does succeed makes a single fixed-budget-increase mechanism a *harder* sell than the earlier narrow-cluster reading implied: no modest multiplier catches `R02990` (115x) and `R02449` (317x) without wildly overpaying on cases like `R02134` (already under budget) or `R00630`.

**Close this recurrence-check design — it reached its own target and answered its question.** Do not run a fourth same-design batch merely to further narrow the interval. If this workstream continues, the next gate is a qualitatively different question (e.g. does operator-incapability, or reconstructable-case cost, correlate with any legal static level feature), prespecified fresh rather than inherited from this design. See [`2026-09-02 rate-estimate report`](../reports/2026-09-02-repair-live-prefix-reconstruction-near-budget-boundary-recurrence.md)'s "Batch 3 addendum" for the full account, per-case table, and reproduction commands.
