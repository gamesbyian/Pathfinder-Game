...
> **Status:** active
> **Last evidence:** 2026-08-23 — matched DFS, admissible-order, and objective-beam-width pilots on twelve exact cohort fixtures
> **Decision:** local scoring profiles cluster tightly, but admissible no-tie ordering and beam width both produce materially different retained search regions
> **Remaining gate:** capture a load-bearing paired divergence and add repair-native fingerprints
...
The taxonomy artifact alone does not support strongest-operational-pair, first-divergence, repair-fingerprint, or scheduler-exhaustion conclusions. The encountered-state pilot below now supports a narrow ordinary-profile ordering nomination, but trace/frontier/repair conclusions remain open.
...
This pilot is still prefix-censored and follows states encountered by `ida:default`. It does not yet determine whether `ida:none`'s distinctive census solves concentrate specifically in high-tie states; answering that requires a symmetric paired trace or conditioning inversion outcomes on the measured equal-slack rate.
...
These bottom-k samples are censored whenever a stage/depth has more than 512 unique paths, so their direct Jaccards are diagnostic sample overlap, not exact full-frontier overlap. Even with that limitation, the progressive collapse from shared early frontiers to low late overlap rejects the simple story that 5K merely carries 3K more copies of the same ranked mode: width changes which parent population generates the next depth and can move the search into a different residual region. The next beam pass should add diversity-bucket occupancy and winning-lineage survival before comparing plain/diverse 5K.
