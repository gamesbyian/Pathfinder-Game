...
> **Remaining gate:** do not commission a fourth cohort for this candidate until the scheduling gap diagnosed in `confirm-residual-001` (main-loop late-reserve fragility under large non-strict-budget overshoot) is either worked around (`--strict-total-work-budget`) or fixed at the source (`stage-budget.ts`); materialize `transfer-envelope-001` only after a candidate actually passes broad confirmation.
...
No new cohort is reserved for `STRATEGY_MUSTCROSS_FLIPPER_WIDE_BEAM_EXPOSURE` at this time. All three tested confirmation designs (`confirm-broad-003`, `confirm-broad-004`, `confirm-residual-001`) have now failed by a genuine instrument problem, not a candidate problem — most recently a main-loop late-reserve mechanism that does not appear to hold under the large budget overshoot this rule's very-hard archetype-eligible population produces. A fourth cohort dispatched against the current, unmodified scheduling behavior would very likely repeat `confirm-residual-001`'s exact truncation pattern (all 25 eligible rows showed the identical 4-of-6 config cutoff), so it is not a good use of compute.

Before any fifth attempt: either (a) dispatch with `--strict-total-work-budget` so the nominal `node_budget`/`work_budget` are real hard ceilings the late-reserve can reliably compute against, or (b) trace and fix `stage-budget.ts`'s `MAIN_LOOP_LATE_RESERVE_CONFIG_COUNT`/`mainLoopLateReserveFraction` arithmetic directly to hold under large overshoot. Whichever comes first, re-verify with the same `scripts/stress/confirm-residual-001-archetype-audit.mjs` audit script (durable, not one-off — kept specifically for this) before spending a new sealed cohort on it: re-running the direct `getConfiguredAttemptConfigs`/attempt-sequence checks against a small local repro is far cheaper than another full GHA dispatch. `STRATEGY_MUSTCROSS_RESERVE_WIDEN_BEAM_EXPOSURE` remains closed negative in development and needs no confirmation cohort at all.
