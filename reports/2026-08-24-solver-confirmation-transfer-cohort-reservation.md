...
> **Remaining gate:** none blocking the evidence-gathering side — same-generator confirmation passed cleanly, and the cross-generator transfer attempt is spent (clean null, zero losses). What remains is a **promotion decision, not further evidence collection**: flip `STRATEGY_MUSTCROSS_FLIPPER_WIDE_BEAM_EXPOSURE`'s default in `modules/solver/attempts.ts`, or first run a higher-power residual-conditioned topology transfer test (mirroring `confirm-residual-003`'s two-phase design) if stronger transfer evidence is wanted before promoting. Neither step is taken by this report.
...
**Next step, not taken here:** flipping `STRATEGY_MUSTCROSS_FLIPPER_WIDE_BEAM_EXPOSURE`'s default in `modules/solver/attempts.ts` is a production solver-behavior change (affects hint generation and canonical-corpus solving fleet-wide), and this report does not make that change. The evidence chain (development A/B +3/-0 → same-generator confirmation +3/-0 → cross-generator transfer clean null with zero losses) is recorded here for whoever makes that promotion decision next.
