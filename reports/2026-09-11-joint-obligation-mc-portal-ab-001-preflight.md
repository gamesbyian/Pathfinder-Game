...
**Referee validation:** GHA artifact download is blocked by this session's egress policy (`productionresultssa18.blob.core.windows.net` / `productionresultssa9.blob.core.windows.net` both return 403 at the proxy — reported, not routed around). Independently reproduced all 21 gains locally instead, on the exact same commit (`dd28ee0`) with `{ ...defaultConfig(), PRUNE_MC_PORTAL_FORCED_NEIGHBOR: true }`, matching the real dispatch's budgets exactly (`nodeBudget=50000000, workBudget=67000000, timeBudgetMs=86400000, schedulerMode='production'`): **21/21 solved and passed `Solver.validateCandidatePath`** (canonical referee). See `scripts/stress/verify-joint-obligation-ab-gains.mjs`. (An earlier run of this script omitted `workBudget`/used a tighter `timeBudgetMs`, causing 2/21 to falsely report `node-budget-reached`; fixed by reading the dispatch workflow's own budget-derivation and matching it exactly.)
