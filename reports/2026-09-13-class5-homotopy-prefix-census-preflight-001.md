...
> **Decision:** start with the smallest rigorous topology test: fixed-endpoint exact LIVE/DEAD sibling prefixes only. Do not invent an open-path homotopy signature merely to increase sample size.
> **Remaining gate:** census exact-labelled extinction prefixes for same-level/same-endpoint LIVE-vs-DEAD contrasts, then measure robust obstacle-relative winding difference on portal-free contrasts.
...
A winding comparison is rigorous when the two paths have the same endpoints. Most current extinction candidates need not. Rather than close an arbitrary connector across different endpoints and accidentally measure the connector, this first gate uses only exact-labelled sibling prefixes ending at the same cell.
...
Group cases by `(levelId, endpoint)`. A decision-bearing contrast requires at least one exact-LIVE and one exact-DEAD prefix in the group.

Report all exact-labelled cases, endpoint-group coverage, LIVE/DEAD contrast count, and why excluded pairs are ineligible.
...
A portal jump is not a planar line segment. Any contrast whose prefix contains a non-cardinal portal transition abstains from the winding decision in this first gate. Report portal exclusion explicitly. Do not draw a fictitious straight segment across a teleport.
...
1. at least two independent levels provide portal-free same-endpoint LIVE/DEAD contrasts; and
2. at least two levels show a robust nonzero winding-class difference between LIVE and DEAD prefixes.

If same-endpoint coverage is too small, close only this fixed-endpoint formulation. The next question would be whether a mathematically defensible open-path H-signature/reference construction is worth implementing, not whether arbitrary endpoint connectors can manufacture a signal.
