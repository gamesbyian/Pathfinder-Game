...
> **Last evidence:** 2026-08-28 — [`connectivity-rejection Stage B audit`](2026-08-28-connectivity-rejection-stage-b-audit.md): the dominant `goal`-unreachable, no-pending-obligation cluster's boundary-sketch recurrence (83.1% of records share a reached-shape with another record, vs. 52.6% sharing the literal exact state) trips the stop condition anyway — 91.2% of that recurrence is confined to a single level (only 1.9% of distinct shapes, 8.8% of records, ever span more than one level).
> **Decision:** do not build a cross-level bounded connectivity reason checker. The two-stage pilot the certificate-audit called for is complete: Stage A found a large, non-trivial rejection population with coarse-context recurrence; Stage B found that the actual geometric recurrence, once represented, is real but almost entirely within-level, not the cross-"unrelated development parent" generalization this file's own gate requires. A within-solve (single-level) boundary-shape memo is a different, narrower candidate the Stage B report surfaces but does not build, soundness-test, or recommend — a future session could scope that separately if it wants to.
> **Remaining gate:** none for the connectivity-derived structural certificate direction as originally scoped here (closed). If a future exact frontier/DD experiment independently produces dead interface states, the opportunistic-second-population idea below remains open on its own terms.
...
- Stage A ran on 2026-08-28 and was not a negative (see [`2026-08-28-connectivity-rejection-stage-a-audit.md`](2026-08-28-connectivity-rejection-stage-a-audit.md)); Stage B then ran on the dominant `goal`-subtype cluster it nominated and **was** a negative once the boundary sketch's cross-level span was actually measured (see [`2026-08-28-connectivity-rejection-stage-b-audit.md`](2026-08-28-connectivity-rejection-stage-b-audit.md)) — recurrence was real but 91.2% confined to a single level.
- Dead-state reuse from a frontier/DD experiment is opportunistic only and does not change that rank.
- This closes the connectivity-derived structural certificate direction as originally scoped. The within-solve (single-level) boundary-shape memo Stage B surfaced is a distinct, narrower candidate — not scoped, soundness-tested, or recommended by either report — and would need its own investigation before any code change.
- Generic exact-state DFS memoization, repair-local experience memory, and the MustPass/MustCross lower-bound caches remain the codebase's actual "remember repeated information" toolkit; no new learned-failure mechanism is currently justified.
