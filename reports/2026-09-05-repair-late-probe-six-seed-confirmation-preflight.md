...
Decision: dispatch both arms with the **workflow's plain defaults** (`node_budget=50000000`, `strict_total_work_budget=false`, no `node_budget_advisory_only`) — no budget-input overrides needed. This still satisfies the underlying intent behind "a strict... ceiling sized from representative production behavior" (a real, bounded, production-representative stop) without the specific `node_budget_advisory_only=true` + borrowed-cap combination, which this population's own selection condition already rules out as viable. Each retained seed keeps the unmodified `REPAIR_LATE_PROBE_NODE_BUDGET` per the experimental contract; only `enable_flags`'s `repair_late_probe_multi_seed_retry_seed_count=6` differs between arms.

**Not yet done:** the population-scale 7-vs-6 dispatch itself. Held pending completion of the concurrently running goal-attraction-disabled-retry confirmation-002 GHA arms, per explicit scope instruction to avoid concurrent broad solver compute without a specific reason.
