...
## Root-cause investigation (2026-09-09, local, no further GHA dispatches)

Follow-up requested after the decision above: dig into *why* `R01273` regresses, using only local in-process reproduction (`Solver.solveLevel` directly, both `STRATEGY_PORTAL_COARSE_STATE_MERGE` on/off, same `nodeBudget=50,000,000`/`workBudget=67,000,000` as the frozen A/B envelope) — no additional GHA runs.

**Finding 1 — the technique-census attribution used above was stale.** The 6-of-12 "beam-isolated-winner" framing (R01273's only known isolated-T1 capability being `beam|score=objectiveFirst|...|retention=mechanic-buckets`) came from [`the technique census`](../reports/stress/technique-census/33717910218/level-technique-coverage.json), an artifact from an earlier solver commit. Reproducing that exact isolated beam config directly (`beamSearchFromGate`, width 5000, node budget 50M) found it does **not** solve R01273 at all — with the flag either on or off (474,127 / 402,590 nodes, both naturally exhausted well under budget, no solution either way). The census is out of date for this level (unsurprising: several flags were promoted to default-on earlier in this same session); the "6 concerning ids" list was a reasonable prioritization heuristic at the time but its causal story for R01273 specifically was wrong.

**Finding 2 — the real winning mechanism, and where it breaks.** Running the full production ladder locally (`Solver.solveLevel` with `attemptBudgetTelemetry: true` to see every attempt) shows R01273's actual solving mechanism is the `must-cross-neighbor-prune-disabled-retry` stage, not any single beam config in isolation:

- **Flag off (control):** that stage's 2nd attempt succeeds outright (`nodesExpanded=404,434`); the solve finishes at `workSpent=157,151,484` in 29 total attempts.
- **Flag on (treatment):** the *same* stage gets attempts with nearly identical node counts to control's (405,794 / 408,495 / 372,643 / 396,123 — evidently the same deterministic seed/config sequence) but **every one of them now fails**, plus two more time out; the ladder then falls through to `guidance-goal-distance-retry`, which also exhausts every attempt, and the whole solve runs out of budget (41 total attempts, `node-budget-reached`).

That is a clean, reproducible signature, not budget noise: same stage, same apparent seeds, same node counts — success under control, failure under treatment. The difference is entirely attributable to the merge key change, since it is the only thing that differs between the two runs.

**Interpretation.** This is not a reopening of the aliasing bug the flag was built to fix — `usedPortalPairs` is confirmed folded into `beamStateKey` (`modules/solver/search.ts` line 68), so candidates that used different portal pairs are never merged together. It is the *ordinary*, already-accepted risk that comes with *any* coarse-state merge: a merge collision keeps only the higher-scoring of two candidates at a shared (position, constraint-state) key, and the discarded one can still be the one a later search step needed. That risk already exists on portal-free levels (where this same merge has been default-on for a long time) and is judged acceptable there. Turning merge on for portal levels exercises that same risk on a population that never had it before, and R01273 is (at least) one case where it costs a solve that a retry tier used to rescue.

**Priority for any follow-up: low, not urgent.** The flag is opt-in and stays that way — there is no live-play or production risk from this finding, only a foregone opportunity (the 158 gains stay unrealized until/unless this is fixed). A real fix would need either (a) a narrower merge key or near-tie-retention interaction that also preserves an at-risk runner-up specifically inside retry tiers with a demonstrated rescue history (a nontrivial design question, not a quick patch), or (b) accepting R01273 (and any other confirmed losses) as a known, documented cost and re-evaluating net value with that accepted. Given portal capability restoration's other three items are now resolved (two promoted, one closed negative), this is not on the critical path for anything else queued — it's a candidate for a future, dedicated session if someone wants to pursue it, not urgent follow-up work.
