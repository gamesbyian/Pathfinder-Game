...
> **Last evidence:** 2026-08-28 — code migration, full 160-level `solver:bench --check` byte-identical, targeted unit tests, and a full `pos:1-10` capability-sweep-shaped before/after confirmation (0 solved-set gains/losses; see addendum)
> **Decision:** `dedup-near-tie-retry`'s own additive work pool is now sized from the solve's resolved `workBudget` (`scaledStageWorkBudget` in `budget-units.ts`) instead of re-deriving it from `timeBudgetMs` a second time via `legacyMsToWork`. This is rigorously behavior-preserving for live interactive play (the tier never runs there) and for the plain-default `solveLevel()` call shape (no explicit `baseWorkBudget`/`workBudget`). It is a genuine, deliberate dose correction — not byte-identical — for the offline capability-sweep/confirmation-workflow call shape, where it closes part of the "467x nominal `workBudget`" overshoot [`2026-08-28-additive-tier-participation-audit.md`](2026-08-28-additive-tier-participation-audit.md) found for this exact tier; the same 10-level population that audit used showed the intended ~2.5x drop in this tier's own `workSpent` with zero solved-set change.
> **Remaining gate:** none. Live-play and default-shape parity are proven; the capability-sweep-shape change is intentional, and the `pos:1-10` confirmation-scale population (the audit's own reference sample) found no solved-set change alongside the expected dose reduction. A check at the confirmation workflows' own much larger scale (`node_budget=50,000,000`, ~100x this population's) would further de-risk it but is not required to keep this migration — see the addendum's own scale caveat.
...
The full `pos:1-10` population — the exact 10-level sample [`2026-08-28-additive-tier-participation-audit.md`](2026-08-28-additive-tier-participation-audit.md) itself used, same `--corpus`/`--node-budget=500000` shape — completed after the initial merge. Before/after code identity, corpus, and flags were otherwise held fixed (before = commit `046097b` in a separate worktree; after = this migration's own commit).

**Solved set: identical.** 3/10 solved both before and after, and every single level matched on `ok`/`status`/`winningStageId`: `R00039` (via `repair-late-probe-multi-seed-retry`), `R00059` (via `repair-late-probe`), and `R00080` (via `connectivity-axis-exhausted-retry`) solved in both runs, via the identical winning stage each time; the other 7 (`R00001`, `R00044`, `R00046`, `R00050`, `R00073`, `R00082`, `R00088`) stayed `node-budget-reached` in both. **0 gains, 0 losses** — this is a real, confirmation-scale, zero-difference solved-set result, not just the small `pos:1-3` spot check.

**`dedup-near-tie-retry`'s own dose changed as predicted; nothing else moved meaningfully.** Its `totalWorkSpent` across the population dropped from 6,028,767 (before) to 2,394,858 (after) — a ~2.5x reduction, the expected signature of the tier's own work pool shrinking from the old effectively-infinite ms-derived number to the real `workBudget=670,000`-scaled one. Every other stage's `totalWorkSpent` and `levelsWon` stayed within noise (single-digit-ppm differences, e.g. `connectivity-axis-exhausted-retry` 12,168,302 → 12,168,456, `repair-late-probe` 59,855,302 → 59,856,706) — pre-existing minor nondeterminism unrelated to this migration (present in principle on the unmodified baseline too), not a new effect this change introduced.

This upgrades the capability-sweep-shape evidence from a 3-level spot check to the full 10-level population the participation audit itself used as its reference sample: **no solved-set change found at this scale**, alongside the intended, substantial dose reduction on the one migrated tier. It does not by itself prove parity at the confirmation workflows' own much larger scale (`node_budget=50,000,000`, ~100x this population's `nodeBudget=500,000`) — see Part 3's scale caveat in the participation audit — but it materially de-risks this migration beyond what was known at initial merge.
