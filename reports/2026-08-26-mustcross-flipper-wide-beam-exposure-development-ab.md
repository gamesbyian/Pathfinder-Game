...
> **Status:** inconclusive
> **Last evidence:** 2026-08-26 — `confirm-broad-004` run [`32955491247`](https://github.com/gamesbyian/Pathfinder-Game/actions/runs/32955491247) at solver revision `7fb675bcbf1e6ca1d36a0bb02a53c1560df83f25`, plus a local single-arm sanity check at the same revision; `confirm-broad-003` run [`32940910715`](https://github.com/gamesbyian/Pathfinder-Game/actions/runs/32940910715) at `36c0744a3eaa6cd2ff787e8221032062ebd85f9d`; development runs [`32931244624`](https://github.com/gamesbyian/Pathfinder-Game/actions/runs/32931244624)/[`32931246526`](https://github.com/gamesbyian/Pathfinder-Game/actions/runs/32931246526) at `dc781d371ba3d083948e97644e76d5b63f8768de`
> **Decision:** `STRATEGY_MUSTCROSS_FLIPPER_WIDE_BEAM_EXPOSURE` was +3/-0 in development (below). Two independent broad confirmation attempts at increasing scale — `confirm-broad-003` (256 fresh levels) and `confirm-broad-004` (1,200 fresh levels, ≈4.7x larger, sized from the measured 1.76% base rate) — both came back with **byte-identical aggregate `workSpent` in both arms**, i.e. zero measurable participation, not merely a null result. `P(zero participating rows at n=1,200 | p=0.0176) ≈ e^(-1,200×0.0176) ≈ 7×10⁻¹⁰` — not plausible as an unlucky draw; the true rate for *freshly generated* levels is evidently far below the 1.76% figure measured on Corpus 2's naturally-accumulated misses. A local control-vs-treatment sanity check directly on the three known development-positive levels (`R00817`/`R02010`/`R02151`) reproduced the original +3/-0 result exactly, ruling out a wiring/regression bug: the mechanism is real, but a broad uniform-random *generated* cohort cannot exercise it at any feasible scale. This is **not** confirmation evidence either way: do not promote to default-on, and do not treat it as a closed negative. See "Confirmation attempts" below.
> **Remaining gate:** the two-phase control-failure-residual design (option 2 in the original diagnosis) is now the prescribed next step, since scaling `count` further already failed decisively once. See "Confirmation attempts" for the full diagnosis and next-step plan.
...
## Confirmation attempts: `confirm-broad-003` and `confirm-broad-004` (2026-08-26, both inconclusive)
