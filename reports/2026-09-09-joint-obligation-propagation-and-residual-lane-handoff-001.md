...
> **Status:** active
> **Last evidence:** 2026-09-11 — the post-restoration refresh this report's own gate called for is now done (`2026-09-11-post-1029-residual-atlas-001.md`), and its trigger condition is met: a recurring reasoning/representation residue survives. Of the current 671 misses, 388 (57.8%) have no known rescuer at all after full cross-evidence reconciliation (T1 census, this run's own dispatch log, lifecycle reach/starvation, and hint-store provenance) — up from this report's own "no known rescuer" framing but now precisely counted rather than exploratory. That residue concentrates in exactly the predicted structure: 269/388 (69.3%) portal-bearing, 325/388 (83.8%) `intersection-heavy` routing, 130/388 (33.5%) the portal+must-cross+intersection-heavy triple-overlap. Portal coarse-state-merge salvage (the other atlas-nominated mechanism) is separately CLOSED NEGATIVE (`2026-09-11-portal-coarse-state-merge-r01273-collision-forensic-001.md`), so it is not a substitute lever for this residue. Admissible-order repricing is also deprioritized: the atlas found allocation/exposure a minority failure mode (57/671, 8.5%).
> **Decision:** per this report's own prespecified trigger, the observer-only joint-obligation propagation pilot below is now the atlas-justified next substantial-compute candidate for Workstream 1/2 — not started in this session (a materially new mechanism needs its own careful design/implementation/validation pass, not a rushed addition at the end of an already-large session). The residual allocation lane stays small and explicitly priced, as originally scoped.
> **Remaining gate:** implement and validate the observer-only pilot exactly as scoped below (build obligation clusters/boundary signatures, log pass/reject/abstain against real search without pruning anything yet, and only promote to an actual pruning pilot if it clears the stated promotion gates: zero live-prefix rejections, independently confirmed dead rejections, material dead-work capture, low relative cost).
