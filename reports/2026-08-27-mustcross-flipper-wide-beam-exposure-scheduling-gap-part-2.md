...
>
> **Correction (2026-08-27):** `confirm-residual-002`'s sealed "treatment" report was actually **control** — `solver-residual-confirmation.yml` referenced `matrix.arm` (nonexistent) instead of `matrix.shard.arm`, so the treatment arm never received `--enable-flags`. The "isolated `--workers=1` reproductions" this report compared it against explicitly passed `ablation: { STRATEGY_MUSTCROSS_FLIPPER_WIDE_BEAM_EXPOSURE: true }` — i.e. they were genuine treatment, with 6 main-loop configs instead of control's 4. `MAIN_LOOP_LATE_RESERVE_CONFIG_COUNT`'s reserve carve-out is sized from the total main-loop config count, so a 4-config run gives its early `dfs:objectiveFirst`/`dfs:intersectionHarvest` the full pool while a 6-config run reserves a slice for the (real, in that arm) trailing configs — a complete, mundane, deterministic explanation for the reported node-count discrepancy that requires no concurrency effect at all. **The "Open question" below did not need answering by a fresh investigation** — the comparison that raised it was never apples-to-apples in the first place. See [`2026-08-27-confirmation-workflow-treatment-flag-wiring-bug.md`](2026-08-27-confirmation-workflow-treatment-flag-wiring-bug.md) for the full correction and blast radius across all four affected confirmation cohorts. [`2026-08-27-worker-pool-concurrency-determinism-diagnosis.md`](2026-08-27-worker-pool-concurrency-determinism-diagnosis.md)'s own direct finding (real `--workers=4` concurrency does not change node/work trajectories, tested independently without this bug in the loop) stands regardless.
