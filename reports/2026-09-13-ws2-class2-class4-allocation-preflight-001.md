...
Freeze one ID file before either arm runs, using control-side facts only. A candidate row must contain a must-turn obligation, actually participate in ordinary `late-repair-search`, satisfy the current structural gates for the child retry, and reach the point where the child treatment would be legal if enabled.

Split into a **gain stratum** where control remains unsolved after the full ladder and a **collateral stratum** where control solves only after `late-repair-search` through a later production stage. The collateral stratum is mandatory because the must-turn tier sits before later specialists and can consume resources before them.
...
Use identical frozen IDs and production-shaped starting envelope in both arms. Control keeps the candidate flag OFF; treatment enables only `STRATEGY_REPAIR_LATE_MUSTTURN_BIASED_RETRY`.
...
### Required accounting and advancement

Preserve solved/referee-valid outcome, winning stage, stage-local and whole-solve `workSpent`, nodes/attempts by stage, wall time, target-stage accounting, censoring/stop reason, and whether downstream control-winning stages still received meaningful work in treatment.
...
Implement one new default-off retry tier with a stable explicit stage ID, portal-only eligibility, and **true final position after every currently promoted additive retry**. Run it through the existing whole-ladder retry architecture; enable `STRATEGY_PORTAL_COARSE_STATE_MERGE` only via the retry-local proxy override; give it a fresh work scope and explicit additive node reserve; do not carve resources from earlier stages; and preserve lifecycle telemetry distinguishing eligibility, entry, participation, and stage solve.
...
Before any population test, prove default-off behavior unchanged, non-portal zero participation, true-dead-last ordering, fresh/additive work, no changes before an earlier production solve, and explicit worker/sequential transport semantics. Freeze the exact additive node/work dose before canary outcomes are inspected. Use one production-sized retry dose, not 10x capability-sweep sizing.
