# Solver optimization workstreams pre-compaction snapshot 001

> **Status:** superseded
> **Last evidence:** 2026-09-25 — canonical queue reached the 21,000-byte agent-context maintenance trigger.
> **Decision:** preserve this exact pre-compaction queue state for archaeology while moving the live authority back toward its 10,500-byte steady-state target.
> **Remaining gate:** none

This report preserves the full canonical queue text immediately before the September-25 maintenance compaction. It is historical context, not a live priority authority. Current priority and next gates remain in [`docs/solver-optimization-workstreams.md`](../docs/solver-optimization-workstreams.md).
...
Plan: [solver research execution efficiency](solver-research-execution-efficiency-plan.md). Current handoff: [2026-09-25 execution-efficiency continuation](../reports/2026-09-25-research-execution-efficiency-session-handoff-001.md). Cross-program follow-through: [convergence backlog](cross-program-convergence-backlog.md). Starting evidence: [execution-topology census](../reports/2026-09-25-solver-research-execution-topology-starting-census-001.md).
...
| 1 | Automatic action selection | `supporting` | `implementation` | **SINGLE-STAGE PLAN RECOVERED / WRAPPER NEXT** | Fresh 24-parent Stage A (seed 2026092201) remains a valid closed result, but its original two-stage design was underpowered relative to its own breadth floor. The replacement is now fully frozen: N=160 fresh random parents, replacement seed `2026092591`, block `ws1-late-continuation-single-001`, ID prefix U, Stage-A-compatible `portfolio-solve-sweep.mjs --scheduler-mode=production` row semantics, and a >=3 independent-parent breadth floor. Recovered un-PR'd Claude commit `49a87732` had already generated the old `2026092501` population before final precommitment; that block is quarantined and must not be used for the decision-bearing confirmation. Exact Binomial(160,0.0516) gives P(X<3)≈0.99%; the prior >=8 wording would have passed only ~58.7% of draws at the historical rate and is retired. The thin one-shot GHA wrapper now exists at `.github/workflows/ws1-late-continuation-single-stage-confirmation.yml` and reuses canonical generation/portfolio/scoring owners; next gate is ordinary CI + plan-quality closure, then a single dispatch from merged main. [single-stage plan](../reports/2026-09-25-ws1-late-continuation-single-stage-acquisition-plan-001.md) · [precommitment recovery](../reports/2026-09-25-ws1-precommitment-overlap-recovery-001.md) · [quality contract](../reports/2026-09-25-ws1-late-continuation-single-stage-acquisition-plan-001.quality.json) · [Stage A result](../reports/2026-09-25-ws1-late-continuation-stage-a-opportunity-canary-result-001.md) | `WS1-ACTION-SELECTION-LEGAL-SIGNAL-CAPTURE` |
