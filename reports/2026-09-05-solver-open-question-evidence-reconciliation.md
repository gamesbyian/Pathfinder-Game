...
| Can CP-SAT solve native residuals current solver evidence misses? | Yes. A deterministic current-boundary join contains 26 current production-unsolved rescues, 13 without an isolated T1 winner, and 12 satisfying both predicates. | **Answered positive for exact rescue capability.** The integrity gate is repaired; a short/lower-load acquisition rule is exploratory and awaits a prespecified future holdout. |
...
5. **CP-SAT rescue selector:** exact rescue capability exists and the source cohorts are repaired. A short/lower-load rule is internally parity-stable but was selected on the same small positive population; validate only on a prespecified future temporal/new-label holdout before bounded acquisition.
