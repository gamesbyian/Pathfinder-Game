...
> **Status:** active
> **Last evidence:** 2026-09-24 — Remote PR validation on head `06294be...` produced green closeout, solver-evidence integrity, CI-topology and deep-verification evidence but red hostile/fast gates. Those reds exposed a malformed persistence-guard edit, stale/new physical-surface classifications, an invalid referee fixture, and a workflow-trigger-vs-execution false positive. All were repaired, followed by further hostile findings through #38 (narrow exception ancestor escape, stale/ambiguous review control planes, and fixed-point gaps in Phase 8/10 closeout). The latest branch head still requires its own complete remote run set.
...
> **Remaining gate:** Obtain one post-Finding-38 exact PR merge head where CI, hostile audit, closeout canary, solver-evidence integrity, CI topology, six-store referee/occurrence/reconstructability validation and Firestore-emulator evidence are all green together. Earlier green jobs remain supporting evidence only; connector-authored commits without a matching Actions run are not closure evidence.
