...
**Status as of 2026-09-09 ~06:50 UTC:** not currently dispatched. An earlier attempt (dispatched alongside the must-cross-neighbour-budget and coarse-state-merge A/Bs, 3 pairs running in parallel via distinct `concurrency_suffix` values) hit two problems in sequence: (1) shards sized off the workflow's default `target_wall_minutes=20` hit their 40-minute job-timeout ceiling before finishing — the shard planner's per-id runtime fallback (used for ids without historical telemetry) undercounted actual solve time for this population by roughly 2x; (2) after redispatching all 3 pairs in parallel with `target_wall_minutes=5`, this account's GitHub-hosted-runner capacity was exhausted (zero runs `in_progress` repository-wide for over an hour). This pair was cancelled back to a clean, undispatched state to let the must-cross-neighbour-budget pair (see [`its preflight`](2026-09-09-mc-neighbor-budget-portal-ab-001-preflight.md)) have the available capacity alone. Redispatch with `target_wall_minutes=5` once that pair completes and free runner capacity is confirmed — do not run more than one pair concurrently until capacity headroom is re-established.
