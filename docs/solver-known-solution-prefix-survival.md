...
**Prospective producer boundary:** historical/current compact outputs are not sufficient to reconstruct every replay-first/dependency-collapsed oracle-set view after the fact. `scripts/known-prefix-oracle-set-manifest.mjs` now provides an inactive compact manifest with stable path-set and conditioning-set identities, producer/version identity, and explicit ancestry/dependence metadata. Instantiate it only when a future decision-bearing prefix-survival study needs that sensitivity. Do not fabricate missing historical ancestry or dump full provenance into every row. The corresponding reopen condition is tracked in [`solver-future-work.md`](solver-future-work.md).
