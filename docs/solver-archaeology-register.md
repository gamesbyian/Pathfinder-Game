...
Historical conclusions are not trusted from labels alone. Before treating a historical null as a clean falsification:

1. normalize historical attempt, stage, routing, corpus, reference-model, and family identities through the current owning normalizer where one exists;
...
### Fragile vs robust hardness in variant families
...
Historical `DEDUP_NEAR_TIE_*` terminology was corrected by the naming cleanup to `COARSE_STATE_NEAR_TIE_*`: these mechanisms intentionally merge non-identical states and therefore concern representation loss, not exact duplicate removal.

The August near-tie history showed alternate merge/retention behavior owns different solve sets. A dead-last additive retry under the alternate representation eventually produced a clean positive population result after earlier subtractive-reserve designs caused collateral losses.

The September portal coarse-state merge produced a large positive basin but failed global promotion because R01273 lost a real capability. Forensic work localized the first-loss collision to omitted path-history information; bounded second-survivor and 1/2/4-hop predecessor extensions delayed extinction but did not converge, so that salvage form was correctly closed rather than expanded toward a full visited-set key.

**General lesson:** a compressed representation can create genuine capability while destroying another capability; alternate representations may be complementary rather than globally rankable.

### Mechanic-bucket retention / old diverse-beam language

The naming cleanup clarified that old `diverse` beam identities map to mechanic-bucket retention, not generic diversity. A later small Corpus-1 comparison found portal-use-signature diversity moved consistently between mechanic-buckets-only and plain-only wins across two prespecified pairs, but samples were too small and the line stopped as inconclusive. Preserve this as evidence for categorical-state distinctions, not as license to reopen generic WS4 retention experiments.

## Capability composition vs acquisition lessons

### Routing-regime / old archetype studies
...
**Permanent archaeology rule:** before trusting a historical/current comparison, prove normalized identity intersection and nonzero participating rows for each treatment.

## Exact/reference-model lineage
...
- nearby scalar future-feasibility features;
- generic scorer proliferation;
- universal beam widening;
