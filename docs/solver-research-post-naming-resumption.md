...
| --- | --- | --- |
| NC-P15-001 / NC-P15-008 | variant-family dataset-root CLI vs private source vocabulary | current commands use `--variant-family-dataset-root`; the shared parser alone accepts `--trove-root` during its external transition window; private source names are canonical-only |
...
| NC-P15-011 / NC-P15-012 | external case-format token and known-prefix source schema | canonical `reference-abstain` is emitted; `atlas-abstain` remains only as the owned input alias; authentic schema-v1 `oracle-abstain` known-prefix sources permanently normalize through the shared extractor to the canonical reference model |
