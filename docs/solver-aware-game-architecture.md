...
Production coarse state merge is intentionally lossy frontier-retention control, not semantic equivalence. Removing it lost solves. Fixed-width packing became unsafe as mechanic cardinalities grew. Current coarse state merge normally uses a per-level mixed-radix numeric fast path with a delimited-string fallback; the schema-valid 31/32-flipper numeric-base defect is tracked in [`solver-correctness-hardening.md`](solver-correctness-hardening.md). None of these coarse keys claims exact future-state identity.
