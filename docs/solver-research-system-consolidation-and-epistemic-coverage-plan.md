...
> **Execution note (2026-09-19, initial slice):** Phase 0 is applied in `solver-research-operating-model.md`. Bundle A now has one derived read-only inventory spine (`research:system-inventory`) over existing relation authorities, current package entrypoints, shared implementation dependencies, current-reference/document lifecycle routing, documentation cognitive roles/status-claim entropy, simple burden metrics and the existing integration audit; it renders focused architecture/lifecycle/diagnostic views plus front-door briefing inputs from that one source, and the briefing-input view is now exposed through cheap discovery. Bundle B now has an emitted-v3-result/declared-schema audit with local schema conformance, a reconciled v3 schema for maintained publisher semantics, delimiter-safe structured population identity hashing with adversarial regression coverage, independent-unit propagation through the shared publisher, typed recovery provenance carried through contract creation/validation/publication, a shared report-status constructor/enum, and a growing synthetic transaction/conformance fixture covering partial acquisition, combine-layer failure/recombination without solver compute, abstention/indeterminate outcomes, dependence units, content identity, confirmation consumption, selected-development evidence and report-status creation. The historical Lane-A identity failures are now pinned at their real seams: the existing newline-only expected-ID parser regression protects comma-bearing IDs, the shared signature-collision primitive has delimiter/Unicode/repeated-local-ID cases, and new explicit-prefix rows preserve structured `source.cutSignature` so current C0 analysis no longer depends on parsing `caseId`; delimiter recovery remains only as a legacy-reader path. Draft PR #1922 is the execution anchor; Bundles A and B remain in progress.
