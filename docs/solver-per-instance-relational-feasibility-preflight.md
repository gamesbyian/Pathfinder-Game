...
> **Status:** questions 1-3 EXECUTED. See [`Q1 result`](../reports/2026-09-17-lane-d-intersection-commitment-realizability-result-001.md), [`Q2 result`](../reports/2026-09-17-lane-d-constrained-event-feasibility-result-001.md), [`Q3 result`](../reports/2026-09-17-lane-d-residual-interface-commutativity-result-001.md).
...
> **Question 1 result:** on the 2 B2 parents with a same-parent LIVE/DEAD pair matched on identical remaining-intersection-deficit and remaining-length, every exact-DEAD state has zero confirmed-feasible future-intersection commitments while every exact-LIVE sibling has >=1 (107 queries, 0 correctness alarms). **Replicated at real scale (2026-09-17)** once the sibling-constructor handoff gate closed: 18 matched exact-DEAD states from one fresh multi-pick parent, 220 queries, 18/18 zero-feasible vs. 2/2 LIVE with >=1, 0 alarms. [`replication`](../reports/2026-09-17-lane-d1-multi-pick-realizability-result-001.md) Retained economics later identified an existing ranking consumer but insufficient decision-context evidence; the current next gate is the production-inert multi-parent [`D1 evidence preflight`](solver-d1-production-inert-evidence-preflight.md). No live prototype is authorized.
...
> **Question 3 result:** the archaeology-flagged gap (commuting-candidate legality was never actually measured) is closed: 12,277 same-obligation-multiset, different-order segment pairs spliced and validated with the native PLAY referee (0 CP-SAT, 0 unknowns) across 25 levels. Length-matched pairs are legal 46.6% of the time (1,453/3,121); every failure is geometric/adjacency or length mismatch, never a lost obligation. Not yet tied to LIVE/DEAD fate. A mechanic-conditioned breakdown found **0% legal on flipper-bearing levels** vs. 69.9% portal-only / 44.8% mechanic-free; a per-segment tightening then found this is **not** explained by the spliced segment itself touching a flipper cell (flipper-avoiding segments on those same levels are also 0%), and that the finding's real independent support is 3 levels, not 10 -- most flipper-bearing levels produced zero commuting candidates to test at all. [`breakdown`](../reports/2026-09-17-lane-d3-mechanic-conditioned-breakdown-result-001.md) · [`per-segment correction`](../reports/2026-09-17-lane-d3-segment-mechanic-attribution-result-001.md)
