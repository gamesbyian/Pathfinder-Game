...
The remaining **3** work-dose sites from the original nine-site inventory are unmigrated (`goal-attraction-disabled-retry`, `repair-elite-prefix-dfs-retry`, `guidance-goal-distance-retry`); each needs its own site-specific behavior-preservation check, not a batch conversion. `admissible-order-fallback` itself (the structural-gap tier above) remains separate and pending its own decision.
