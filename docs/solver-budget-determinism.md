...
The remaining **4** work-dose sites from the original nine-site inventory are unmigrated (`goal-attraction-disabled-retry`, `repair-elite-prefix-dfs-retry`, `must-cross-neighbor-prune-disabled-retry`, `guidance-goal-distance-retry`); each needs its own site-specific behavior-preservation check, not a batch conversion. `admissible-order-fallback` itself (the structural-gap tier above) is separate from this inventory and pending its own decision.
