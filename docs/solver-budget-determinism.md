...
**A distinct, unmigrated structural gap found the same day in the sibling plain `admissible-order` tier** (not the retry tier above — the earlier tier in the ladder): it installs no fresh `prep._workCap` at all under legacy semantics, unlike every other tier examined so far. It has no `legacyMsToWork` call to swap, so it does not fit this migration's "currency swap, prove behavior-preservation" shape — any fix is a genuine new allocation (the same class of change as `repair-fallback`'s own 2026-08-20 fix), which needs its own evidence first. See [`../reports/2026-08-28-admissible-order-work-cap-gap-discovery.md`](../reports/2026-08-28-admissible-order-work-cap-gap-discovery.md).
