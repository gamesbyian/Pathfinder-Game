...
The remaining legacy sites are in additive stage setup inside `orchestration.ts`: repair fallback, goal-attraction-disabled-retry, admissible-order-fallback, and promoted retry-stage work pools derived from `timeBudgetMs * stageFraction` before conversion to work. They are deterministic for identical inputs, but they violate the stronger target invariant that a non-binding deadline can change **only latency**, never search allocation.
...
**A distinct, unmigrated structural gap found the same day in the sibling plain `admissible-order-fallback` tier** (not the retry tier above — the earlier tier in the ladder): it installs no fresh `prep._workCap` at all under legacy semantics, unlike every other tier examined so far. It has no `legacyMsToWork` call to swap, so it does not fit this migration's "currency swap, prove behavior-preservation" shape — any fix is a genuine new allocation (the same class of change as `repair-fallback`'s own 2026-08-20 fix), which needs its own evidence first. A full-`pos:1-10` probe found the node reserve governs every level in that population regardless, so this is a confirmed structural gap with no demonstrated impact so far, not an active risk. See [`../reports/2026-08-28-admissible-order-work-cap-gap-discovery.md`](../reports/2026-08-28-admissible-order-work-cap-gap-discovery.md).

The remaining 5 sites in the original nine-site CI-ratchet inventory are unmigrated (`goal-attraction-disabled-retry`, `connectivity-axis-prune-disabled-retry`, `repair-elite-prefix-dfs-retry`, `must-cross-neighbor-prune-disabled-retry`, `guidance-goal-distance-retry`); each needs its own site-specific behavior-preservation check, not a batch conversion of this same pattern. `admissible-order-fallback` itself (the structural-gap tier above) is separate from this inventory and pending its own decision.
