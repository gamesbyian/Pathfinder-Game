...
- `test:hint-path-validation` (renamed from test:hint-path-oracle by batch 8A) is a surfaced Phase-8 package command that is **not directly reachable from the PR-CI package-command graph**, while the underlying `validate-hint-paths.mjs` (renamed from `hint-path-oracle.mjs`) has at least one CI-reachable test reference. The inventory therefore records it as **CI test reference**, not direct execution. Before the Phase-8 rename, the table-setting audit still needed to determine whether that indirect reference substantively exercises the command/runtime contract; its full command reads the split `data/hints/` artifact, which the Node-test sparse checkout intentionally does not materialize.
...
| 11 | geometry argument variant -> orientation / canonical-live | **Implemented and guarded.** Geometry and input-coordinate APIs use `orientation`; the former LevelUtils orientation surface was decomposed in Phase 14B. |
...
| 13 | normalized reqLen -> requiredLength / no-reference | **Implemented in Phase 13B; closeout pending.** Normalized/runtime code uses `requiredLength`; raw wire `reqLen` remains intentionally retained at the compatibility boundary. |
| 13 | normalized reqInt -> requiredIntersections / no-reference | **Implemented in Phase 13B; closeout pending.** Normalized/runtime code uses `requiredIntersections`; raw wire `reqInt` remains intentionally retained at the compatibility boundary. |
| 14 | former top-level core facade -> specific dependencies / mixed | **Implemented in Phase 14A; validation pending.** The mixed facade, `createCore`, and `SOUND_BUS` are removed; immutable vocabulary, audio, and cloning now have explicit owners. |
| 14 | `modules/level-utils.ts` -> direct domain imports / no-reference | **Implemented in Phase 14B; closeout pending.** The facade is deleted; pure/domain, level-data, input-coordinate, and editor-coordinate behavior now has direct owners. |
