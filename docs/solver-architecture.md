...
- **Resumability research:** current beam attempts discard their live frontier when a work cap ends. A bounded feasibility gate now asks whether one deterministic beam can pause at `W` and resume for `Δ` equivalently to an uninterrupted `W+Δ` run, using in-memory continuation state only. This is research, not current production behavior. See [`solver-search-resumability.md`](solver-search-resumability.md).
