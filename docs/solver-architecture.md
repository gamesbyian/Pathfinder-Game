...
Solver allocation's target currency is machine-independent work `applyMove + 12 * isConnected`. The main ladder and all inventoried additive tiers now divide work rather than deriving fresh work from ms-shaped stage fractions; `check:solver-budget-boundaries` prevents that debt from returning. Internal search loops may also honor technique/node caps. See [`solver-budget-determinism.md`](solver-budget-determinism.md).
