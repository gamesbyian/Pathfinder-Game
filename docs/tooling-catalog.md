...
| Hints/provenance | `npx tsx scripts/hint-query.mjs --id=<ID>`; `node scripts/hint-store-index.mjs`; `node scripts/hint-ingestion-receipt-query.mjs`; `node scripts/stress/hint-reconstructability-report.mjs`; query `hint` | Query accepted paths/events, index current stores, audit ingestion accounting, and inspect execution/source-run reconstructability before bespoke provenance joins |
...
| Research graph query | [`research-queryability.md`](research-queryability.md); `npm run research:query -- --entity=<type:id> [--direction=in|out|both] [--relation=<edge>] [--depth=N]`; semantic `--view=<answerability|impact|live-successors|closed-constraints|shared-measurements|multi-consumed-blocks|ownership-gaps|non-question-lineage|support-impact|coverage|system-findings|system-lineage>`; `--compare-ref=<git-ref>`; snapshot options | Derived read-only cross-family lookup, reverse traversal, recurring semantic views, and temporal comparison. Authored edges/sufficiency stay distinct from lexical discovery; structural diffs do not manufacture scientific causality. |
