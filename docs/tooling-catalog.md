...
| Research question dossier | `npm run research:dossier -- --question-id=<id>` | Derived read-only question view joining current authority matches, question relations, active premise/MO context, candidate resources, known frozen blocks, durable evidence and conservative acquisition guidance; never becomes a priority/evidence authority |
| Research relations | `npm run research:relations -- --list`; then `--relation=<name> [--query=<term>] [--discover]` | Read-only normalized access across questions, assets/resource contracts, evidence integrity, measurement opportunities, all active premise rows/edges, premise snapshots/admissions, queue/experiment authorities, durable evidence summaries and optional block discovery; reuse existing semantics rather than creating another warehouse |
| Research consumption lineage | `npm run research:record-consumption -- --block-artifact=<path> --question-id=<id> --decision-ref=<ref> --conditioning=<name> --out=<sidecar>`; matched selections may use `--selection-artifact=<matched.json>` | Append question-specific selection/consumption facts to a sidecar copy of the frozen block; preserves evidence-use ancestry without mutating the source block or converting semantic question relations into exposure relations |
...
| Research acquisition preflight | `npm run research:acquisition-preflight -- --question-id=<id> [--artifact=<path>]... [--control=<control-report>]` | Question-first conservative route selection: automatically discovers known research blocks, carries audit-grade Resource Contract conditioning/dependence/missingness alongside candidate assets, and names suitable existing generation source families only when generation is earned; never generates levels automatically |
...
| Research integration audit | `npm run research:integration-audit` | CI-backed referential-integrity checks across the active premise snapshot, measurement overlay, question IDs, asset/resource-contract relationships, generator suites, discovered blocks and durable evidence summaries; does not make semantic reopen/priority decisions |
