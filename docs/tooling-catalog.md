...
| Generate cross-generator challenge levels | `npm run stress:generate-topology -- --count=N --master-seed=S --out=tmp/...`; topology-first macro-maze/module construction independent of Corpus-1/2 `generateWitness()`. Use primarily for transfer/challenge after a candidate earns the expense; v0.1 mechanic scope is bounded. See [`solver-evaluation-evidence.md`](solver-evaluation-evidence.md). |
