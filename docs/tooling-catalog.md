...
| Attempt-shape signatures among unsolved levels | `node scripts/stress/cluster-unsolved-failures.mjs --in=<benchmark> --corpus=<levels>`; complementary to lifecycle telemetry, and its `beam-collapse` tag is explicitly not a search-quality signal |
| Which level features correlate with current solve/fail outcomes? | `node scripts/stress/feature-solvability-analysis.mjs --baseline=<compiled-baseline> --corpus=<levels>`; reports base-rate-corrected archetype enrichment, feature separation, and navDensity-controlled effects; observational diagnosis, not a causal routing rule |
| Connectivity-prune soundness after topology/prune changes | `node scripts/run-bundled.mjs scripts/stress/connectivity-soundness-check.mjs`; every stored valid-solution prefix must remain connected, exits non-zero on a counterexample |
| Must-cross prune soundness | `node scripts/run-bundled.mjs scripts/stress/mc-prune-soundness-check.mjs -- [--corpus=corpus2]`; validates shipped must-cross connectivity/forced-neighbor rejection rules over known-valid paths |
...
| Operational similarity of solver techniques/configs | Read [`solver-technique-operational-taxonomy.md`](solver-technique-operational-taxonomy.md), then reuse/extend the existing technique-census, `method-probe`, lineage, beam-research, and debug telemetry surfaces. The bounded taxonomy/cohort substrate and initial ordering/admissible/beam pilots are implemented; measure local ranking, branch/frontier retention, and first-divergence behavior rather than treating outcome-vector Jaccard as operational similarity. |
| Operational taxonomy/cohort rebuild | `node scripts/run-bundled.mjs scripts/technique-operational-similarity.mjs` rebuilds the compact machine-readable taxonomy, per-term source proxies, outcome join, and bounded exact inversion/control fixtures downstream of the existing census. It intentionally leaves unmeasured operational fields null; use `node scripts/operational-similarity-lib-node-test.mjs` for its pure ranking/slack/bounded-output reducers. |
| Bounded encountered-state ordering | Add `--ordering-profiles=harvestThenFinish,portalFirstTransfer --ordering-limit=4096` to `method-probe.mjs`. This observation-only seam reports top/full-ranking agreement, tie rate, margins/Kendall components, first divergence, and censoring without retaining states or changing active ordering. For admissible anatomy, include true `none` (for example `--ordering-profiles=none,default,mustCrossFirst`) to report equal-slack frequency and final slack-plus-tie-break orders. |
| Bounded beam trace | Add `--beam-trace-limit=512` to an isolated beam `method-probe`; it reduces existing beam-observer paths to bounded stage/depth hashes. Compare two outputs with `node scripts/compare-operational-beam-traces.mjs --left=<json> --right=<json> --out=<json>`. Jaccard is censored sample overlap whenever a bucket truncates, not exact full-frontier overlap. |
...
| Stress | `stress:generate*`, `stress:validate-witnesses`, `stress:benchmark*`, `stress:regression`, `stress:solve-one`, `stress:reduce-level`, `stress:rank-levels`, `stress:failure-inbox`, `stress:lifecycle-failure-map`, `stress:solution-profile*`, `stress:provenance-coverage`, `stress:tune-parallelism`; direct specialist diagnostics include `scripts/stress/cluster-unsolved-failures.mjs`, `feature-solvability-analysis.mjs`, and the `*-soundness-check.mjs` gates above; compact corpus discovery: `scripts/corpus-query.mjs`; [`../data/stress/README.md`](../data/stress/README.md) |
