...
| Research graph query | `npm run research:query -- --entity=<type:id> [--direction=in|out|both] [--relation=<edge>] [--depth=N]`; `--query=<term>`; `--stats` | Derived read-only cross-family entity lookup, reverse relationships and bounded traversal over authored stable edges; lexical discovery is not promoted to authority |
