...
| Research question dossier | `research:dossier -- --question-id=<id>` | Read-only question/premise/resource/block/evidence/acquisition join |
| Research relations | `research:relations -- --list`; `--relation=<name> [--query=<term>] [--discover]` | Authorities/relations, active premises, durable evidence, optional block discovery |
