...
| D1 production-inert observation | `npm run solver:capture-d1-decisions`, then `npm run solver:annotate-d1-decisions` | Freeze real beam cull decisions and eligibility under unchanged search, then attach exact D1 zero/nonzero/UNKNOWN evidence offline; development canaries may cap annotation, independent confirmation may not |
