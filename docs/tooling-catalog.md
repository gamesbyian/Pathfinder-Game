...
| Research question dossier | `npm run research:dossier -- --question-id=<id>` | Read-only question/premise/resource/block/evidence/acquisition join; owns no state |
| Research relations | `npm run research:relations -- --list`; `--relation=<name> [--query=<term>] [--discover]` | Read-only authorities/relations, active premises, durable evidence and optional block discovery |
| Research consumption lineage | `npm run research:record-consumption -- ...` | Append use/selection ancestry to a block sidecar; supports matched-selection artifacts |
...
| Research integration audit | `npm run research:integration-audit` | CI reference checks across research-system seams; no semantic decisions |
