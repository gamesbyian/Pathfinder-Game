...
| Research acquisition preflight | `npm run research:acquisition-preflight -- --question-id=<id> [--artifact=<path>]...` | Question-first conservative route selection: reuse existing evidence first, otherwise name the smallest justified acquisition route; never generates levels automatically |
