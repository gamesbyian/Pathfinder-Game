...
| Research question dossier | `npm run research:dossier -- --question-id=<id>` | Read-only join of question/authority matches, premise/MO context, resources, blocks, durable evidence and acquisition guidance; owns no state |
| Research relations | `npm run research:relations -- --list`; then `--relation=<name> [--query=<term>] [--discover]` | Read-only normalized authorities/relations, including active premise rows/edges, durable evidence and optional block discovery |
| Research consumption lineage | `npm run research:record-consumption -- ...` (`--selection-artifact=<matched.json>` supported) | Append selection/use ancestry to a block sidecar without mutating the source block |
...
| Research acquisition preflight | `npm run research:acquisition-preflight -- --question-id=<id> [...]` | Conservative route using discovered blocks, audit-grade Resource Contract signals and existing generator guidance; never auto-generates |
...
| Research integration audit | `npm run research:integration-audit` | CI referential-integrity checks across premise/MO/question/resource/generator/block/durable-evidence seams; no semantic decisions |
