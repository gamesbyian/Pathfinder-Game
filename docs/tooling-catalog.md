...
| Completed naming-cleanup status/history | `npm run naming:status [-- --phase=N|--batch=<id>|--json]`; reports terminal completion by default and exposes historical phase/batch rows on demand. This is a ledger/history view, not a naming authority or an active queue. |
...
| Technique niches / unsupported anatomy | `node scripts/analyze-technique-niches.mjs`; joins the reconciled frozen census to legal static descriptors and emits deterministic offline level/action classifications with input hashes. Development evidence only; replay IDs are never routing inputs. |
| Technique relative advantage | `node scripts/analyze-technique-relative-advantage.mjs`; compares prespecified close-action inversion populations (left-only/right-only/both/neither) over the committed niche base artifact and emits compact standardized descriptor differences. Outcome-selected development evidence for hypothesis triage, not a routing rule. |
...
| Join equal-work pricing to current production reach/work | `npm run solver:analyze-equal-work-production-reach -- --equal-work=<EW1 combined-cells.json> --production=<current solver report> --require-current-head --check`; requires a current-head production sweep with `--lifecycle-telemetry` and per-attempt `workSpent`; accepts both maintained `{summary:{commit,corpus},levels}` level-blind reports and flattened `{commitSha,corpus,levels}` stress/combined reports; canonicalizes mixed-era attempt/action/stage identities at ingestion and refuses decision-bearing status for missing/mixed commit or corpus provenance |
...
| Prior experiment evidence | `node scripts/research-status-index.mjs --compact --query=<term>` before opening reports; stage, routing-regime, attempt-config, and composite action queries expand through their owning historical-identity normalizers; historical evidence nominates current work only after comparability check |
...
| Families | `family:generate`, `family:index`, `family:show`, `family:query`, `family:coverage`, `family:analyze`, `family:boundary-report`, `family:parent-hint-replay`, `stress:family-pair-divergence`, `solver:winning-attempts`; use `--variant-family-dataset-root`; `family:index` reconciles historical/current aggregate snapshots by parent+mode+variant identity plus normalized evidence payload, canonicalizes recognized attempt identities, preserves differing observations, and reports partial-coverage/conflict diagnostics rather than suppressing evidence; [`variant-level-research.md`](variant-level-research.md) |
