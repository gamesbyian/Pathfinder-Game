...
- Archetype/sample workflows save compute but do not cure selection bias. If the same routing-regime population nominated/tuned the rule, obtain confirmation proportional to the selection pressure; use cross-generator challenge only when the claim/risk warrants it under [`solver-evaluation-evidence.md`](solver-evaluation-evidence.md).
