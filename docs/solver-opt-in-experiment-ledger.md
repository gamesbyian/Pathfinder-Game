...
| `STRATEGY_GOAL_ATTRACTION_LEGACY_DISTANCE_RETRY` | **VALIDATED POSITIVE, NOT YET PROMOTED** | 2026-08-23, `solver-level-blind-targeted-sweep.yml` (GHA, commit `95927c6df`) matched control/treatment on both populations plus a local published-corpus sweep: 73-level loss population 15/73 → 18/73 (**+3/-0**, new solves R02158/R02575/R03211, baseline's 15 solves all retained); 90-level gain population 90/90 → 90/90 (**0/-0**, confirming the retry tier structurally never fires on levels that already solve earlier in the ladder); published corpus 160/160 unchanged. Net +3, zero regressions across all three populations — smaller recovery than the closed-negative global-swap form's +9 (budget is shared additively with everything upstream, so the retry tier only gets a fraction of the node ceiling the global form had from move zero), but the full gain is captured at zero cost. Reopen for a larger reserve fraction only with evidence a bigger headroom recovers more of the remaining unsolved set without a matched-work regression check first. |
