...
| `STRATEGY_REPAIR_FALLBACK_NODE_RESERVE` | **NEW, OPEN — n=8/n=12 local pilot only, no regressions found across two revisions** | Two design flaws found and fixed locally before any population evidence was gathered (see `REPAIR_FALLBACK_NODE_RESERVE_FRACTION`'s own comment in `orchestration.ts` for the full derivation) — the mechanism itself is now unit-tested (`orchestration.test.ts`) with exact expected node counts. n=8 spot-check: the fallback loop moved from 0 attempts on 5/6 unsolved levels to 1 attempt on all 6, but produced no new solves at this budget/fraction; zero regressions on the 2 previously-solving levels. n=12 sample (R00602/R01856/R02823, the three levels that previously flip-flopped across the two design-flaw revisions): all three now byte-identical to the flag-off baseline. | A larger GHA sample (repair-gated population, ~919/1700 on corpus-2) is in progress — see the flag's own dispatch for current status. Do not promote on local-pilot evidence alone, per this ledger's standing bar. |
