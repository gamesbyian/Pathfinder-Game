...
| `STRATEGY_HIGHINT_STANDARD_INTERSECTION_HARVEST_BEAM_EXPOSURE` | **CLOSED NEGATIVE (2026-08-28, APPEND-LAST FORM).** Selected replay was +1/-0, but the prespecified mechanics-eligible 120-level strict-67M development A/B was control 56/120 vs treatment 55/120: 0 gains, 1 loss (`R02965`), new action reached 68/120, +29.0M aggregate work, no errors/deadline censoring. Appending the new beam changed the fixed five-config protected suffix and starved an existing `beam:objectiveFirst@beam5000` winner. Do not repeat append-last placement. See [development report](../reports/2026-08-28-highint-standard-ih-exposure-development.md). |
| `STRATEGY_HIGHINT_STANDARD_INTERSECTION_HARVEST_RESERVE_PRESERVING_EXPOSURE` | **ACTIVE DEVELOPMENT DESCENDANT.** Same missing `beam:intersectionHarvest@beam2000` and same very-high-intersection rules, but inserted immediately before the pre-existing protected five-config suffix so no old protected member is displaced and no reserve is widened. This is a materially new response to the parent’s observed allocation regression. First gate: selected replay must retain the parent’s `R02440` rescue and restore the `R02965` control win; if it passes, use the already-prespecified disjoint 120-level mechanics-eligible development cohort before any independent confirmation. |
