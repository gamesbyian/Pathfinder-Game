...
| `STRATEGY_HIGHINT_STANDARD_INTERSECTION_HARVEST_RESERVE_PRESERVING_EXPOSURE` | **CLOSED NEGATIVE (2026-08-28).** Same missing `beam:intersectionHarvest@beam2000` and same very-high-intersection rules as the append-last parent, but inserted immediately before the old protected five-config suffix. Selected mechanism replay run `33151801662` was still +1/-1: `R02440` remained a direct treatment rescue, but `R02965` still regressed. Preserving suffix membership did not preserve the old winner's usable allocation because the inserted beam consumed work before it; the 5K objective winner stopped about 134K work short of its known solve. Gate 2's disjoint 120-level cohort was therefore never executed. Do not repeat nearby placement tweaks without a materially new scheduler premise. See [development report](../reports/2026-08-28-highint-standard-ih-reserve-preserving-development.md). |
