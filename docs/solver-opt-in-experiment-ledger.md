...
When this ledger conflicts with stale wording in an older dated report, the dated report remains authoritative for the experiment it actually ran, but **this ledger is authoritative for the current disposition of the current implementation**. The same rule applies to broad queue text in `future-work.md` when a later implementation commit changed the exact mechanism under decision. Update both when practical; never silently reinterpret an old A/B as though it tested later wiring.

The reconciliation that created this ledger is recorded in [`../reports/2026-08-11-opt-in-experiment-status-reconciliation.md`](../reports/2026-08-11-opt-in-experiment-status-reconciliation.md). No new solving was performed in that reconciliation; it only aligned current code, historical evidence, and promotion state.
