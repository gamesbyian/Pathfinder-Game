...
| `STRATEGY_GOAL_ATTRACTION_LEGACY_DISTANCE_RETRY` | **UNDER VALIDATION** | 2026-08-23: dead-last additive whole-ladder retry tier reusing the legacy goal-attraction distances closed negative above, gated to only run after every earlier tier has already failed on a level (`goal-attraction-legacy-distance-retry` stage, order 125). Structurally cannot touch a level that already solves earlier in the ladder, so the 90-gain-population loss set from the global-swap form cannot recur. Population-scale A/B against the 73-loss and 90-gain sets plus the published corpus is queued next; update this row with the confirmed result before considering promotion. |
