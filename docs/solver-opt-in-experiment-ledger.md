...
| `STRATEGY_MUSTCROSS_FLIPPER_WIDE_BEAM_EXPOSURE` | **DEVELOPMENT POSITIVE (+3/-0); CONFIRMATION INCONCLUSIVE x2, MECHANISM VERIFIED INTACT, NOT NEGATIVE (2026-08-26).** Appends plain `beam:intersectionHarvest@beam5000` + `beam:objectiveFirst@beam5000` to attempts.ts's must-cross+flipper-heavy rule only. Development: prespecified `solver-archetype-sample-ab.yml` pilot restricted to `must-cross-heavy` (486-level fixed population): +3/-0, `R00817`/`R02010`/`R02151` gained, aggregate work +1.03%. Two broad confirmation attempts at increasing scale — `confirm-broad-003` (256 levels) and `confirm-broad-004` (1,200 levels, ≈4.7x, sized from the measured 1.76% base rate) — both came back byte-identical aggregate work in both arms (0/0 gains/losses, zero measurable participation). At `confirm-broad-004`'s scale, `P(zero participating rows) ≈ 7×10⁻¹⁰`, ruling out chance; a local sanity check directly on the three known development-positive levels reproduced the original +3/-0 result exactly, confirming the mechanism is real and not a wiring bug. Diagnosis: this rule's eligibility gate is a strict superset of `needsRepairFallback`'s gate, so a fresh cohort's eligible levels are almost always solved by the early repair probe before reaching this candidate's new configs; the development population was mined specifically from repair-resistant residual misses, and evidently the generator's raised-cap stratum does not reproduce that same repair-resistant texture even at scale. **Do not promote to default-on; do not close negative either** — absence of participation is not evidence of no mechanism. Remaining gate: `confirm-residual-001`, a two-phase control-failure-residual cohort (generate fresh, freeze the control-failure subset, confirm only on that subset) via the new `.github/workflows/solver-residual-confirmation.yml` (built, locally validated) — reserved but not yet materialized. See [`2026-08-26-mustcross-flipper-wide-beam-exposure-development-ab.md`](../reports/2026-08-26-mustcross-flipper-wide-beam-exposure-development-ab.md) and [`solver-optimization-current-queue.md`](solver-optimization-current-queue.md) Priority 1. |
