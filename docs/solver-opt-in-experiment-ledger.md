...
| `STRATEGY_MUSTCROSS_RESERVE_WIDEN_BEAM_EXPOSURE` | **PREPARED, NOT YET DISPATCHED (2026-08-26).** Widens `MAIN_LOOP_LATE_RESERVE_CONFIG_COUNT` by one (mirroring the validated 4->5 precedent) and appends the missing plain WIDE beam to attempts.ts's other two must-cross-heavy sibling rules (the ones `STRATEGY_MUSTCROSS_FLIPPER_WIDE_BEAM_EXPOSURE` deliberately left untouched because their reserve windows were full). Code + unit tests done. Local published-corpus check (160 levels, matched envelope): 160/160 both arms, 157/160 rows byte-identical, 3 rows (`P00140`, `P00154`, `P00157` — all route through one of the two modified rules) show small `nodesExpanded`/`workSpent` perturbation from the added trailing config with **no solve-status change** — this is the expected, scoped effect of adding a real config, not a silent no-op claim; nothing outside the two targeted rules moved. No `solver-archetype-sample-ab.yml` A/B has been dispatched yet — queued behind `STRATEGY_MUSTCROSS_FLIPPER_WIDE_BEAM_EXPOSURE`'s own confirmation so the two candidates' evidence doesn't get conflated. See [`solver-optimization-current-queue.md`](solver-optimization-current-queue.md) Priority 1. |
| `STRATEGY_MUSTCROSS_FLIPPER_WIDE_BEAM_EXPOSURE` | **DEVELOPMENT POSITIVE, AWAITING CONFIRMATION (2026-08-26).** Appends plain `beam:intersectionHarvest@beam5000` + `beam:objectiveFirst@beam5000` to attempts.ts's must-cross+flipper-heavy rule only. Prespecified `solver-archetype-sample-ab.yml` pilot restricted to `must-cross-heavy` (486-level fixed population): +3/-0, `R00817`/`R02010`/`R02151` gained, aggregate work +1.03%. Earns confirmation per the prespecified gate; do not promote to default-on before a fresh confirmation cohort (`confirm-broad-001`/`002` are spent on the unrelated diverse-IH candidate). See [`2026-08-26-mustcross-flipper-wide-beam-exposure-development-ab.md`](../reports/2026-08-26-mustcross-flipper-wide-beam-exposure-development-ab.md) and [`solver-optimization-current-queue.md`](solver-optimization-current-queue.md) Priority 1. |
