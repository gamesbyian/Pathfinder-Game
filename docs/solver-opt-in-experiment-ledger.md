...
| `STRATEGY_GOAL_ATTRACTION_DISABLED_RETRY_FRESH_WORK_POOL` | **ACTIVE, clean positive development result — nominated for independent confirmation, not yet promoted.** New 2026-09-02, directly motivated by the starvation telemetry below: gives the pass a fresh work pool (sized off `diversityBudgetFraction` of `workBudget`) instead of sharing the outer, already-depleting pool. Zero live-play risk (both real interactive callers zero `diversityBudgetFraction`, skipping the whole block). **2026-09-02, development A/B result:** 150-level Corpus-2 population (positions 1-150), `node_budget=50,000,000`: control (node reserve alone) 78/150 solved vs. treatment (node reserve + fresh work pool) 79/150 — **+1/-0** (`R00355` gained, none lost), aggregate work/nodes both slightly lower in treatment too. A local single-level reproduction of `R00355` confirmed the mechanism directly: control never dispatches a single `goal-attraction-disabled-retry` attempt (shared pool already spent), treatment gets two dispatches and wins the level on the tier's own beam action. Full result: [`../reports/2026-09-02-goal-attraction-disabled-retry-fresh-work-pool-development-ab-preflight.md`](../reports/2026-09-02-goal-attraction-disabled-retry-fresh-work-pool-development-ab-preflight.md). **This population was chosen for likely reach, not independently drawn — per the preflight's own pre-declared acceptance framing, this result nominates the mechanism for independent confirmation on a fresh, independently-drawn cohort; it does not itself promote anything.** Implementation details, validation, and why this is opt-in rather than a direct fix: [`../reports/2026-09-02-goal-attraction-disabled-retry-fresh-work-pool-implementation.md`](../reports/2026-09-02-goal-attraction-disabled-retry-fresh-work-pool-implementation.md). Starvation diagnosis: [`../reports/2026-09-02-goal-attraction-disabled-retry-work-pool-starvation.md`](../reports/2026-09-02-goal-attraction-disabled-retry-work-pool-starvation.md). |
