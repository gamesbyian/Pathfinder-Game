...
The first beam-routing expansion produced a population result of **+20 net Corpus-2 solves (828→848; 21 gained / 1 understood loss)**. A follow-up perimeter-beam expansion (adding `perimeterSweep` STANDARD-width beam configs) recovered all 29 of its newly routed local targets and was then confirmed at population scale (GHA run 32419836654, Corpus-2 848 → 863, **+16 gained / 1 lost, net +15**; the 1 loss, R03111, is consistent with ordinary non-deterministic search-order jitter — its own previously-winning DFS config was re-attempted and failed this run, not evicted by anything beam-related — rather than a config regression). `STRATEGY_REPAIR_LATE_PROBE` recovered 20 of 94 locally targeted gate-excluded repair winners, was confirmed at population scale (GHA run 32418694112, Corpus-2 828 → 868, **+20 gained / 0 lost**), and was **promoted to production default-ON 2026-08-20/21** — see [`solver-opt-in-experiment-ledger.md`](solver-opt-in-experiment-ledger.md).
...
| 7 | Cheap isolated-technique wins the ladder does not route to | **ACTIVE / SHIPPING GATE** | `STRATEGY_REPAIR_LATE_PROBE` promoted 2026-08-20/21 (population-confirmed +20/0); perimeter-beam expansion population-confirmed 2026-08-21 (+16/-1, net +15). Investigate remaining high-intersection/must-cross-heavy beam gaps per archetype rather than adding broad configs indiscriminately. |
...
**Shipping work, now confirmed (both already merged to main):**

- the first beam-routing expansion population-confirmed +20 net Corpus-2 solves (828→848);
- the follow-up perimeter-beam expansion (adding `perimeterSweep` STANDARD-width beam configs) recovered all 29 local newly routed targets and was then **population-confirmed 2026-08-21** via GHA run 32419836654, Corpus-2 848 → 863 (**+16 gained / 1 lost, net +15**). The 1 loss (R03111) was checked against its per-level record: it hit the 100,000,000-node budget cap after 72 attempts across every retry tier, and its own previously-winning DFS config (`dfs:perimeterSweep/sideCommitment`) was re-attempted and failed this run rather than being evicted by a beam config — consistent with ordinary non-deterministic search-order jitter (this run used `deterministic: false`), not a routing regression;
