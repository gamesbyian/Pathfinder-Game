...
**Not yet run**: the equal-budget full-ladder A/B this proposal needs. Analysis script (not committed — throwaway, reproducible from this description) mined `data/stress/hints*/`'s existing provenance; the actual routing change and its A/B are separate, larger work items requiring GHA capacity.
