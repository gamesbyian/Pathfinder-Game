...
**Local verification (2026-08-20): 22/22 sampled cheap oracle-union levels confirmed missed by the full production ladder, 0 recovered** (current defaults, 20,000,000-node declared budget — extended to 40–60M by the default-on retry tiers' additive extensions, so this is not a budget-size artifact). Every one of the cheapest oracle-union levels tested (all ≤~100,000 isolated nodes, up to ~30 sampled) came back `ok: false` under the real `solveLevel()` ladder — a 100% miss rate with no counterexample found.
...
**Reproducibility checked and resolved (2026-08-20, same day): these are reliable, deterministic finds, not stochastic luck.** 5 of the cheapest ladder-ineligible repair wins (`R00347`, `R03122`, `R03245`, `R02760`, `R03079`) were each re-run 3 times via the isolated `dfs:repair:repair` config with no explicit seed override — every trial came back byte-identical (same node count, same solve) across all 5 levels. Plain repair (no biased-variant, no multi-seed probe retry) is fully deterministic in this code path by default; the `seedSalt` stochasticity this session flagged elsewhere applies specifically to the repair-*probe* retry tiers' multi-seed widening, not an ordinary single repair attempt. The reproducibility caveat is closed.
