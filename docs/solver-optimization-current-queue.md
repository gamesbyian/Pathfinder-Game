...
| 7 | **Repair-fallback eligibility gate too narrow (new, 2026-08-20, root-caused same day, see section 7 below)** | Root cause found for the census's cheap-solve oracle-union misses (20/20 sampled confirmed missed by the real ladder): `needsRepairFallback` requires `mustCross ≥ 2 AND mustPass ≥ 3` (or very-high reqInt) — of **158 currently-unsolved levels where repair wins in the census, 94 (59%) never get repair tried at all** under this gate, **18 of those solving in ≤1,000,000 nodes**. Not a budget/ordering problem — the ladder structurally never attempts repair on this population. Next: cross-validate reproducibility (repair is stochastic) and a before/after cost check on a proposed narrower eligibility rule before any promotion. | A validated eligibility-rule widening with net solve gain and an acceptable full-corpus cost increase — the same promotion bar as every other shipped mechanism this session. |
...
small hand-built sample). The other 4 beam-only cases and the 4 repair-only cases remain fully
un-root-caused. Scratch scripts (`scripts/tmp-stale-solve-mine.mjs`, `scripts/tmp-stale-solve-verify.mjs`,
and both bisections' probe scripts) were deleted after use, not committed — their logic and full
results are captured here and in the local scratchpad.
...
| solves within (nodes) | oracle-union levels |
|---:|---:|
| 1,000,000 | 111 |
| 5,000,000 | 134 |
| 10,000,000 | 164 |
...
**111 of the 246 — 45% of the whole oracle union — solve in under 1,000,000 nodes**, several in the low hundreds or thousands (e.g. `R01129` via `ida:mustCrossFirst` in 159 nodes; `R00347` via `dfs:repair:repair` in 7,232). A level in this cheap-solve subset staying unsolved by the current production ladder cannot be explained by budget size at all — the winning technique needs a rounding error of any reasonable node budget. If the ladder still misses a meaningful fraction of these, that is real evidence of an ordering/gating defect (the right technique/gate combination never actually gets tried, or gets deprioritized past where it would win), not a budget question — a fundamentally different and more surgical kind of fix than anything else on this queue.

**Local verification (2026-08-20): 20/20 sampled cheap oracle-union levels confirmed missed by the full production ladder** (current defaults, 20,000,000-node declared budget — extended to 40–60M by the default-on retry tiers' additive extensions, so this is not a budget-size artifact). Every one of the 20 cheapest oracle-union levels tested (all ≤~100,000 isolated nodes) came back `ok: false` under the real `solveLevel()` ladder.
...
**Caveat before treating this as ready to ship:** repair's own search is not fully deterministic (uses `seedSalt`, per this session's earlier finding) — the census's single-run wins for this population have not been cross-validated across repeated runs the way the Priority 0 mining pass required (≥3 finds, tight spread) before trusting them as reliable. Cost risk also needs measuring, not assumed: `needsRepairFallback`'s narrow gate exists because repair is expensive when it fails (census: 85.3% of repair's *isolated* full-budget attempts on the broader unsolved population hit the 50M-node cap without solving — Priority 1's own finding). Broadening eligibility to this 94-level population specifically looks favorable (cheap when it wins, per the 18-under-1M subset), but that is a different, narrower question than "make repair eligible everywhere," and needs its own before/after cost comparison (nodes/work on the full corpus, not just the win-count on this one population) before promotion — the same discipline every other mechanism on this queue has been held to. **Next decision-bearing step:** cross-validate a sample of the 94-level population's repair wins for reproducibility (repeat runs, not just the census's single pass), then a local before/after cost check (matching this queue's established pattern) on a proposed narrower eligibility rule — e.g. `mustCross ≥ 2` alone, or `mustCross ≥ 1 OR mustPass ≥ 1` — rather than shipping a broadened `needsRepairFallback` on this evidence alone.
