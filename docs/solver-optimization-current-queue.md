...
|---:|---|---|---|
| 0 | Restart/randomization and learned-failure search | **ASAP / EQUAL-WORK RESTART GATE** | Build the narrow research harness around canonical `workSpent`: compare seed 0 continued to `W` against seed 0 to `W/2` plus fresh seed 1 to `W/2`, charging all failed work. First pass is accounting correctness, then a development residual population frozen against one baseline contract. Existing multi-seed gains motivate this but do not answer it. [`restart audit`](../reports/2026-08-24-restart-continuation-value-audit.md), [`learned-failure audit`](../reports/2026-08-24-learned-failure-certificate-audit.md) |
