...
- **Full validation before commit**: `npx tsc --noEmit` clean; full `modules/solver` Vitest suite 402/402 passing (four pre-existing tests updated to assert the new — first template-only, then beam-last — ordering, since they previously encoded the old no-beam/beam-position behavior); `solver:bench --check` PASS (160/160, no regressions) plus the same-commit worktree cost comparison above; end-to-end `Solver.solveLevel()` (not just the isolated census attempt) run against all 69 previously-zero-beam oracle-union levels, 20 recovered through the real production ladder. **Not yet run**: a full corpus-2 (1,700-level) stress refresh to confirm the corpus-wide solved-count improvement at scale — the 69-level oracle-union population is the complete, exact set this change can possibly affect (every level whose census winner needed one of the three patched rules), so this is expected to be a lower bound on the eventual `solver-stress-refresh.yml` corpus-2 delta, not a population sample; run that workflow to confirm before citing a corpus-2-wide number.
...
**Recommendation (not yet implemented): a new LATE-tier, node-capped repair attempt, independently gated from `needsRepairFallback`, appended after the existing ladder (including the existing early probe and fallback) rather than widening either of them.** Concretely: a new opt-in-tested ablation flag (e.g. `STRATEGY_REPAIR_LATE_PROBE`, default-off in production until validated, matching `STRATEGY_REPAIR_TURN_BIAS`'s existing opt-in convention) that appends one fixed-budget (2–5M node, per the table above) repair attempt to the very end of `getAttemptConfigs()`'s returned list specifically when `!needsRepairFallback(f)` — i.e. only for levels the existing mechanism structurally never reaches — with its own independent node budget the way `runRepairProbe` already has, not sharing the main loop's per-attempt allocation. This is a genuinely new scheduler-position tier, not an eligibility-threshold edit, and implementing it correctly inside `orchestration.ts`'s budget-composition machinery (which has a documented history of subtle starvation/reserve bugs — see the CLAUDE.md gotchas on `ADMISSIBLE_ORDER_NODE_RESERVE_FRACTION` and the repair-probe node-budget-starvation fix) is real work, not yet done. **Next steps, in order**: (1) implement behind the flag; (2) validate correctness on the 94-level gap population plus a control sample of currently-solved levels (confirm zero regression, since they should never reach the new tier); (3) run the same `solver:bench --check` + full-corpus `nodesExpanded`/wall-time before/after comparison this doc's promotion contract requires everywhere else, using the 314-level ineligible-unsolved population's own timing as the primary cost signal; (4) promote only if the recovery justifies the measured cost. Not yet started.
