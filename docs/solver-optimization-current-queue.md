...
| 2 | Beam score/retention at proven extinction boundaries | Re-run the nine B/D rows that formerly abstained now that CP-SAT supports flipping filters; then test K versus 2K only on held-out family-namespaced extinction cases while collecting score descriptors. | Recurrent exact-live/exact-dead separation across unrelated parents; a scorer change must beat widening at equal work. |
| 3 | Canonical-inclusive family-boundary retest | Cold-run the eight ETT-028 nominated canonical parents and selected siblings on current main, then replay/ablate the configurations around any reproduced disagreement. | Reproduced, parent-clustered solver boundary that identifies a generic technique or representation change. |
...
Existing lineage work says generic widening is a weak lead: some winning families disappear because the score ranks an exact-dead child above an exact-live sibling. Flipping-filter support now removes the main abstention blocker for nine B/D cases. Re-run those exact cases before inventing descriptors.
