...
| 6 | Mechanics-conditioned technique routing | **Re-measured 2026-08-19 with true repair eligibility (see section 6 below): the density split IS real within the truly-eligible population, but a direct 3-level trace found the proposed reserve mechanism's case is weaker than the correlation alone suggested — 1/3 shows real cost waste, 1/3 shows stale corpus data, 1/3 shows no issue.** Not yet strong enough to justify building the mechanism. Next: either a larger local sample, or wait for the technique census (in flight) to answer the underlying isolated-capability question directly. | A mechanics-only rule improves a matched population A/B; no exact-level winner lookup. |
...
it, and none of the 5 solved.** That rules out "just disable the flag" as a fix: something else changed
in the ~19 days after `d87338d0f` that compounds with it, not yet identified. Next step: bisect one of
these 5 a second time with `PRUNE_MC_RESERVED_WALL` forced off throughout, to isolate the remaining gap
— a smaller, more targeted search than starting over. The other 4 beam-only cases and the 4 repair-only
cases remain fully un-root-caused. Scratch scripts (`scripts/tmp-stale-solve-mine.mjs`,
`scripts/tmp-stale-solve-verify.mjs`, and this bisection's probe scripts) were deleted after use, not
committed — their logic and full results are captured here and in the local scratchpad.
...
**Revised assessment: 1 of 3 direct traces shows a real cost-inefficiency signal, 1 shows stale corpus data, 1 shows no issue at all.** That is not strong enough evidence to justify writing a new production reserve mechanism yet — the sample is too small, and where the effect IS real (`R02434`), it currently looks like a **wasted-cost** story (an already-solved level spending far more than it needs to) rather than a **missed-solve** story. Do not build `STRATEGY_ADMISSIBLE_ORDER_DENSITY_RESERVE`-shaped code on the strength of this alone. The right next step is either (a) a larger local sample (10-20 more direct traces) before committing to a mechanism, or (b) waiting for the technique census (already in flight, full isolated-technique-budget data across every level) to answer the underlying question directly — does `admissible-order` genuinely fail to reach solutions that `repair`/other techniques also can't reach on these levels, at the SAME full budget each — rather than continuing to infer it indirectly from ladder-order-biased provenance and a 3-level spot check.
