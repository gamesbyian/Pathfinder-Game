...
| 7 | **Cheap oracle-union solves the full ladder currently misses (new, 2026-08-20, see section 7 below)** | The technique census's 246-level "oracle union" (currently-unsolved levels some isolated technique DOES solve at 50M nodes) splits sharply by cost: **111 of the 246 solve in ≤1,000,000 nodes** by some single technique — trivially cheap relative to any production budget, so a level in this subset staying unsolved is not explained by budget size at all. Local sample verification against the real `solveLevel()` ladder (20M-node budget, production defaults) is in progress to establish how many of these the current ladder genuinely fails to reach despite the cheap solve existing. | A verified subset of genuinely-missed cheap oracle-union levels, with a concrete allocation/ordering explanation (not a new scoring change) that a matched population A/B can validate. |
...
**Local verification in progress (2026-08-20):** re-running the full production `solveLevel()` ladder (current defaults, 20,000,000-node budget — already 20x the median cost of the cheapest tier) against a sample of the cheapest oracle-union levels, to establish how many the ladder genuinely fails to reach despite the near-free isolated solve existing. Results pending; this section will be updated with the verified miss rate and, if the signal holds, a concrete next step (most likely: read off exactly which gate/technique/ordering the ladder needs to reach sooner for the missed subset, rather than a scoring change).
