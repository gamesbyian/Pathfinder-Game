...
Use full `npm run ci` for the solver/deep-change classes listed in [`testing.md`](testing.md). Browser changes may additionally require `npm run ci:full` or focused Playwright coverage.
...
| Package scripts, CLI entrypoints, renamed/moved scripts | Update live consumers and lifecycle references; run the dead-script/package-script checks through `ci:fast`. |
| GitHub workflows, local workflow entrypoints, path filters, maintained action versions | Run the workflow-action validator through `ci:fast`; keep local and Actions entrypoints semantically aligned. |
...
If an authority crosses its warning threshold, prefer replacement, compaction, links to specialist detail, or archival of chronology. If it crosses a hard maximum, compact it as part of the same change rather than waiting for CI to reject the branch.
