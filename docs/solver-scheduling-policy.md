...
**Rung 2 tooling (built 2026-09-02, see [`solver-optimization-workstreams.md`](solver-optimization-workstreams.md) Workstream 2 and [`2026-09-02-static-portfolio-construction-pilot.md`](../reports/2026-09-02-static-portfolio-construction-pilot.md) for the full evidence trail):** a real fixed-work static-portfolio execution mode exists and needs no `attempts.ts`/production orchestration change. `technique-census-cell.mjs`'s `runCell` runs an ordered technique-key list against one level under one cumulative shared work budget with early-exit-on-success; its opt-in `cell.perTechniqueWorkCap` (and per-key map variant `cell.perTechniqueWorkCapByKey`) bounds each technique's own share, required whenever a non-naturally-terminating technique (e.g. `repair`) could otherwise consume the whole shared budget and starve later list positions. `scripts/build-static-portfolio-plan.mjs` constructs a plan (population + named ordered-technique-key arms + shared budget); `scripts/combine-static-portfolio-shards.mjs` aggregates shard results into per-arm coverage/work versus a named control arm. `.github/workflows/static-portfolio-confirmation.yml` dispatches this at population scale (artifact-only, no commit). `scripts/stress/select-random-sample.mjs` draws a deterministic seeded UNIFORM population sample (`--exclude-ids-from` keeps it disjoint from an already-mined discovery sample) for a portfolio-cardinality question not scoped to one mechanic/regime.
...
**Mechanized since 2026-09-09:** the `node_budget_advisory_only=true` + `strict_total_work_budget=false` combination described above is no longer merely discouraged in prose — `solver-level-blind-targeted-sweep.yml` now rejects that dispatch outright with an explanatory error before planning or solving happens (see `docs/solver-correctness-hardening.md`'s "unbounded advisory-node-budget timeout sizing" entry). The same workflow's `--min-participating-levels` gate (an absolute floor, trivially satisfied on a large population) gained an additive `--min-participation-rate` (a fraction of the dispatched population) in `validate-solver-sweep-integrity.mjs`, wired through as the optional `min_participation_rate` dispatch input — both default to off/no-op, so no existing dispatch's behavior changes unless a caller opts in. Separately, `ci.yml` now runs a tiny fixed 9-level solver capability canary (`scripts/solver-bench.mjs --check --baseline=logs/solver-canary-baseline.json`) on every PR — previously nothing caught a wholesale solver regression before merge; every corpus-scale check remained a manually-dispatched workflow.

### Shard-level runtime estimation, uncertainty, and timeout recovery (hardened 2026-09-10)

**Root cause (chronology detail: `reports/2026-08-28-additive-tier-participation-audit.md`, `reports/2026-09-05-admissible-order-non-default-retry-repricing-confirmation-005.md`).** Telemetry written by `solver-highbudget-unsolved-sweep.yml` at its own large `--node-budget` was linearly extrapolated down to smaller callers' budgets; several additive/retry tiers have a FLAT or budget-fraction cost rather than a proportional one, so that extrapolation systematically underestimated real cost at small budgets, with no record of which ablation flags were active. Separately, the shard-timeout estimator used `sum(predicted)/workers`, correct only if every id's duration matched, not the true bottleneck (the largest single job in a concurrent queue). Both caused real 2026-09-10 incidents (`connectivity-volume-portal-ab-001`).

**Fix, in four parts, all in `scripts/plan-highbudget-shards.mjs` (regression coverage: `scripts/combine-solver-sweep-reports-node-test.mjs`):**

1. **Real makespan estimation.** `simulateMakespan(durationsMs, workerCount)` is a cheap longest-processing-time-first (LPT) list-scheduling simulation of the actual `workers`-way concurrent queue (assign jobs longest-first to whichever virtual worker currently has the least load; makespan = the max worker load), replacing `sum(predicted)/workers` for both bin-packing capacity checks and the reported/timeout wall time. `packByMakespan` packs ids into bins under a makespan-capacity ceiling (plus an optional hard group-size cap) rather than a naive count- or sum-based heuristic.
2. **Uncertainty-aware fallback, not an ordinary median.** `classifyTelemetry(id, ...)` tiers each id into `confident` (matching config + node-budget scale within tolerance), `scale-mismatch` (matching config, budget ratio beyond `--scale-tolerance`, default 3x), `config-mismatch` (telemetry's `configKey` — see part 3 below — doesn't match this dispatch's resolved ablation flags), or `none` (no telemetry at all). Two multipliers, empirically calibrated (not arbitrary) via `scripts/backtest-shard-runtime-policy.mjs` against real incident data in `data/stress/shard-runtime-backtest-cases.json` (`scaleMismatchMultiplier=8` from a pure scale-mismatch case observed at 6.52x its naive prediction; `noTelemetryOrConfigMismatchMultiplier=11` from a scale-and-config-mismatch case observed at 9.2x, floored at the scale multiplier), inflate the naive telemetry-scaled prediction for anything less than `confident`. A self-referential-fallback guard (`hasGroundedFallback`) prevents multiplying an already-unevidenced placeholder fallback (when a population has zero confident ids) into an unbounded number unrelated to real evidence — in that case the multiplier is not applied and `--max-uncertain-group-size` (part 3) is the sole defense instead.
3. **Bounded uncertain-shard packing, not solo-sharding.** `--max-uncertain-group-size` (default `workers*3`) caps how many low-confidence ids can share one shard regardless of makespan headroom — small multi-wave shards (roughly 2-3 waves of `workers`) rather than one-level solo shards. Confident-telemetry shards are not subject to this cap and can still pack as large as the makespan model allows. Solo-sharding (one id per shard) remains available only via the pre-existing, explicit `--fixed-group-size` opt-in, never the default.
4. **Diagnostics.** `shard-plan.json`'s top-level `planning.uncertaintyPolicy` records the resolved `configKey`, `scaleTolerance`, both multipliers, `maxUncertainGroupSize`, and a pointer to the calibration source; every shard object carries `confidence` (`'high'`/`'low'`/`'bypassed'`), `telemetryCoverage` (per-tier id counts), and `uncertainIds`. The CLI prints a `⚠ LOW-CONFIDENCE PLAN` warning when applicable, so a low-confidence plan is obvious in GHA logs before any shard runs.

**Automatic timeout-only recovery.** `solver-level-blind-targeted-sweep.yml`'s job graph now separates `combine-first-pass` → `plan-recovery` → `solve-recovery-shards` → `combine-final` (previously one terminal `combine` job). `combine-first-pass` runs `scripts/derive-timeout-recovery-population.mjs`, which distinguishes "missing ids only" (exit 0, potentially timeout-caused) from duplicate/unexpected/malformed rows or no result at all (exit 3/4 — a genuine correctness problem, never auto-retried). `plan-recovery` only runs when that population is missing-ids-only AND `needs.solve-shards.result == 'success'` (every shard job itself succeeded — GHA's own matrix aggregate is `'failure'` the moment any shard exits for a reason other than its own timeout wrapper, so a genuine shard crash can never reach recovery) AND the new `auto_recover_timeouts` dispatch input (default `true`) isn't disabled. The recovery pass forces `--fixed-group-size=workers` and doubles `min_timeout_minutes` (floored at 60m) — deliberately not solo-sharding, and not a broad global timeout increase — so previously-timed-out ids get materially more per-shard headroom while staying truly concurrent. `combine-final` runs the same hard, must-pass population-completeness validate the old `combine` job did, over the union of both passes' shard artifacts.

**Telemetry config-provenance.** `scripts/update-highbudget-telemetry.mjs` now tags every written entry with `configKey` (`canonicalConfigKey(enableFlags, disableFlags)`, the same function the planner uses) and starts a fresh EMA rather than blending across a config change, so a future flag that changes pruning/retry behavior can't silently contaminate an existing config's estimate. `solver-highbudget-unsolved-sweep.yml` (telemetry's sole writer) has no ablation-flag dispatch inputs at all, so `configKey` there is always the production-default `'|'` today — this is a correctness contract for the future, not a behavior change now. That workflow already folds a round's completed rows into telemetry regardless of whether the round's own population was complete (no validate gates that step), so valid rows from a timeout-truncated run were already being learned from; no separate change was needed for that half of the ask. `solver-level-blind-targeted-sweep.yml` remains deliberately un-wired to telemetry writing: its own header comment documents it as artifact-only / never-commits-to-main, and it is exactly the workflow most likely to vary ablation flags (A/B experiments) — writing its telemetry back would make it the highest-risk contamination source of all, not a safe extension of "learn from partial runs."
