...
**Root cause history.** `logs/solver-stress-refresh/corpus2-runtime-telemetry.json` is written EXCLUSIVELY by `solver-highbudget-unsolved-sweep.yml` at its own `--node-budget` (historically ~1.2 BILLION nodes), as an EMA of `elapsedMs / (nodeBudget/1e9)` for ids that hit the node ceiling. `scripts/plan-highbudget-shards.mjs` then linearly extrapolated that figure to whatever `--node-budget` a DIFFERENT caller requested (e.g. 50M for `solver-level-blind-targeted-sweep.yml` — 24x smaller). That linear-scaling assumption is false: several production additive/retry tiers have a FLAT node-count ceiling or a fixed budget-FRACTION cost rather than a `--node-budget`-proportional one (`reports/2026-08-28-additive-tier-participation-audit.md`), so a flat-cost tier is relatively far more expensive at a small requested budget than telemetry's own calibration point implies — extrapolating down in scale systematically UNDERESTIMATES real cost. Telemetry also carried no record of which ablation flags were active when a sample was taken. Separately, the shard-timeout estimator itself used `sum(predicted)/workers`, which is only correct when every id's duration is identical — a real `workers`-way concurrent queue is bottlenecked by its single largest job, not the average. Together these produced repeated real incidents on 2026-09-10 (`connectivity-volume-portal-ab-001`: a canary-vs-plan-job-timeout failure, a canary-picks-a-hard-population-id failure, and a systemic telemetry-vs-real-cost mismatch across the whole treatment arm) and echo the earlier 2026-09-05 admissible-order confirmation timeout investigation (58/76 hard shards ran past their timeout with zero output, `reports/2026-09-05-admissible-order-non-default-retry-repricing-confirmation-005.md`).
