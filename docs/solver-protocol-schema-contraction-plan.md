...
> **Progress (2026-09-20, PR #1937):** rebased conceptually onto the post-#1930/#1933/#1934/#1935 mainline. Wave 1 is active. A shared research-artifact envelope decoder now owns researchBlock/populationIdentity compatibility reads, three general consumers route through it, conflicting duplicate locations fail closed, and current search-loss capture producers write the canonical top-level populationIdentity instead of duplicating it inside specialist population metadata. Historical nested search-loss captures remain fixture-backed readable. Recent edge-hardening's typed recombination lineage, provenance validation, artifact byte binding, and workflow-contract checks are treated as canonical infrastructure for later contraction rather than duplicated here. C1 has also started: candidate-path wire decoding is now separated from a packed-key-only canonical referee, with the old multi-encoding validator retained only as the compatibility/import adapter while internal callers are migrated. B1 is now active too: a canonical Hint-record mutation helper owns the bare-path projection, and the complete-enumeration, corpus-expand, diversification, workbench, and published-import producers have stopped writing both sibling fields independently; interactive UI/editor writers remain. New research consumption/enrichment links also assert the canonical envelope before write, so the shared decoder is now paired with a current-producer guard rather than serving only as a tolerant reader. D has started: the worker client validates/normalizes raw levels before transport, so direct solveLevel() and worker SOLVE now share one normalized-level contract; the worker refuses non-normalized SOLVE messages. The obsolete same-release TRAP/TRAP_PROGRESS/TRAP_RESULT false-goal worker dialect has been removed entirely. E1 has also materially advanced: fresh scheduler input is canonical-only, historical scheduler spellings moved behind an explicitly named historical decoder, the portfolioExperiment alias is gone, phase-6 portfolio CLI aliases fail with canonical replacements, all deprecated renamed SolveOpts override fields/fallbacks have been removed, and the now-empty solveopts transport-parity guard was retired. F1 has started: solver sweep shard envelopes and flattened recombination reports now cross one pure, non-mutating ingress adapter and the combiner core operates on one internal {summary, levels} shape.
