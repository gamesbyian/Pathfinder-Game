...
> **Progress (2026-09-20, PR #1937):** current APIs/producers are increasingly canonical-only while historical compatibility is being pushed behind named ingress readers. Fifteen registry seams are now closed. This session closed the hint mutable-representation seam (canonical Hint records are the sole write authority; bare paths are derived), the corpus-document seam (explicit {levels, metadata, storageShape} end to end; LEVEL_WRAPPERS and array facades removed), and the SolveOpts work-budget seam (baseWorkBudget only on current input). It also removed residual stage-budget option aliases that contradicted PSC-022's earlier closeout and repaired the resulting stale CI fixtures/types without restoring retired compatibility. Remaining work is concentrated in sweep-envelope finalization, explicit corpus write-set semantics, durable level addressing, family-attempt archival strategy, the ablation/stage historical-reader census, population-integrity identity-output contraction, hint-provenance old-field isolation, and the research-consumption sidecar model.
