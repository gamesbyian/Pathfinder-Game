...
> **Progress (2026-09-21, PR #1937):** implementation is in late closeout. The CI-driven caller audit strengthened the completion rule: central deletion is necessary but not sufficient; maintained entrypoints, shared helpers, workflows, and exercised tests must also prove no current consumer remains. PSC-001/002 are now re-closed after B3 replaced implicit read-time hint-array identity with an explicit `changedHintLevels` write-set contract. `setLevelHintRecords()` is the sole current hint mutation boundary, `.hints` is derived, `UNTOUCHED_HINTS_STATE` is gone, explicit corpus documents are used throughout maintained tooling, and the concurrency regression is preserved without stale-snapshot clobbering. PSC-016/018/019/014 remain closed as previously documented. PSC-007 has one current `{summary, levels}` write envelope with historical flat ingress only. PSC-004 durable addressing has been tightened so explicit `pos:` is selection-only and level-blind persisted research identity uses stable IDs. PSC-015/017 have named historical Attempt/provenance ingress and canonical-only current projection. PSC-022/023 have no known current aliases after the exercised caller migrations. Remaining work is primarily final no-current-consumer census/CI verification for PSC-004/007/015/017/022/023 and any residue those checks reveal. Frozen evidence remains unchanged.
