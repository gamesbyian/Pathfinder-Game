...
3. **Admissible-order retry `1.0 → 0.18`.** **Closed negative on testability, not on the merits (2026-09-10).** A matched-work confirmation of this fraction is architecturally unreachable by any CLI/workflow configuration: `admissibleOrderNonDefaultConfigs` dispatch exclusively through the plain `admissibleOrderSearch` primitive, which never consults the per-tier soft work cap the fraction scales — only the raw node ceiling and (under strict mode) an unrelated whole-solve cumulative cap can ever stop it, code-verified across 7 local probes. A genuine confirmation needs a prerequisite code change (make that search primitive consult its own work cap, mirroring `admissibleOrderSearchLDS`'s existing pattern) before any dispatch, not further budget-sizing. Production stays `1.0`. See [`matched-work methodology`](../reports/2026-09-10-admissible-order-non-default-retry-matched-work-methodology-001.md) (root cause), [`artifact recovery`](../reports/2026-09-10-admissible-order-confirmation-006-artifact-recovery.md), [`work-cap gap discovery`](../reports/2026-08-28-admissible-order-work-cap-gap-discovery.md) (sibling tier's related, earlier finding). [`Confirmation 006`](../reports/2026-09-05-admissible-order-non-default-retry-repricing-confirmation-006.md)
