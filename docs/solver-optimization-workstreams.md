...
**State:** LEGAL-SIGNAL CAPTURE ACTIVE. Next: retained-artifact action-boundary capture; the first gate is existing-data analysis and requires no new solver acquisition. [`oracle`](../reports/2026-09-21-prewinner-work-oracle-census-001.md) · [`preflight`](../reports/2026-09-21-action-selection-legal-signal-capture-preflight-001.md)
...
| 2R | Parity response signature | `supporting` | **DEFERRED / STATIC FORM CLOSED** | No active acquisition; reopen only on a materially different parity mechanism from success/prospective seam evidence | `WS2-PARITY-RESPONSE-SIGNATURE` |
...
| 1 | Automatic action selection | `supporting` | **LEGAL-SIGNAL CAPTURE ACTIVE** | Existing-data only: freeze action-boundary data from retained production artifacts; test simple legal signals with rare-winner protection | `WS1-ACTION-SELECTION-LEGAL-SIGNAL-CAPTURE` |
