...
2. **Portal-aware beam coarse-state merge.** **Closed negative on promotion (2026-09-09) — stays opt-in.** `BeamNode.usedPortalPairs` preserves exact used-pair identity in the merge key behind `STRATEGY_PORTAL_COARSE_STATE_MERGE`; the frozen 954-level population A/B found 158 gains / 12 losses (net +146, all gains referee-valid) but the rare/specialist-retention check confirmed a genuine capability regression on `R01273` (its sole isolated-technique winner still fails under treatment even at 10x the matched envelope). A strongly positive net does not override that. **Next (optional, not urgent):** root-cause why this level's beam trajectory is discarded by the new merge key before any future promotion attempt. See [`preflight`](../reports/2026-09-09-portal-coarse-state-merge-ab-001-preflight.md), [`beam preflight`](../reports/2026-09-09-portal-beam-state-identity-preflight-001.md).
