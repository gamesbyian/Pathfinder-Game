...
3. **Admissible-order repricing: DEFERRED, atlas decided NO for now.** The atlas found only 21/671 (3.1%) misses with an offered-but-starved known rescuer and 36/671 (5.4%) reached-with-comparable-work failures — allocation/exposure (classes 2+3 combined, 8.5%) is a minority failure mode, materially smaller than the atlas's dominant class: 388/671 (57.8%) have no known rescuer at all after full cross-evidence reconciliation. Per the gate's own rule ("if the atlas instead exposes a larger representation/search-policy/capability gap, pursue that higher-value mechanism first"), do not spend the next population-scale run on admissible-order repricing. If a matched-work `1.0 -> 0.18` confirmation is ever run, use the tier-scoped enforcement path and prove target-stage participation first.
4. **No-known-rescuer residue (388/671, 57.8%): NEXT, atlas-justified.** This is the larger gap gate 3's own rule points to. It is exactly the recurring reasoning/representation residue [`2026-09-09-joint-obligation-propagation-and-residual-lane-handoff-001.md`](../reports/2026-09-09-joint-obligation-propagation-and-residual-lane-handoff-001.md) pre-specified as the trigger for its **observer-only joint-obligation propagation pilot** (build obligation clusters/boundary signatures from must-cross/intersection/must-pass/portal obligations; log pass/reject/abstain against real search without pruning anything; promote to an actual pruning pilot only after clearing its stated zero-false-rejection/material-dead-work-capture gates). Structurally concentrated exactly as that report predicted: 269/388 (69.3%) portal-bearing, 325/388 (83.8%) `intersection-heavy` routing, 130/388 (33.5%) portal+must-cross+intersection-heavy triple-overlap. Not started this session — a materially new mechanism needs its own design/implementation/validation pass. The atlas's class-1 menu-expansion candidate (26 never-offered levels, two recurring `width=5000`/`mechanic-buckets` beam configs) remains available as a smaller, cheaper, parallel-track WS1 action for a session that wants a bounded win before or alongside the pilot.
...
|---:|---|---|---|
| 2 | Residual capability + fixed-work allocation | **ACTIVE / FIRST PRIORITY** | Atlas done; portal salvage done/CLOSED NEGATIVE; admissible-order repricing deferred (atlas: allocation is 8.5% of residual) -> observer-only joint-obligation propagation pilot on the 388-level no-known-rescuer residue is the atlas-justified next gate. |
| 1 | Automatic action selection | **ACTIVE / PARALLEL ANALYSIS** | Atlas's 26-level never-offered `width=5000`/`mechanic-buckets` beam cohort is a small, cheap, well-evidenced menu-expansion candidate that can run in parallel with the joint-obligation pilot; protect low-multiplicity capability before routing changes. |
| 6 | Repair reachability/reconstructability | **SUPPORTING** | Static block count is an exploratory signal only (AUC 0.69 on 8 vs 22 cases). Run a fresh prespecified confirmation (real CP-SAT+rollout+`closeLengthGap` compute on a prespecified sample) only if current residual evidence makes that non-cheap test worth buying — do not use `bestBadnessTechnique`/repair-dominance in lifecycle telemetry as a substitute signal; it is not cross-technique comparable (beam/DFS attempts rarely populate `bestProgress` the way repair's iterated-local-search does, per `search.ts`'s own comment on `finalBadness` not being a tracked best-ever minimum outside repair). |
