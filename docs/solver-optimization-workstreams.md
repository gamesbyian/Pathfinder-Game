...
**State:** active. The 671-miss residual atlas is done (provenance-audited rerun); portal coarse-state salvage is done and CLOSED NEGATIVE. Allocation/exposure is only 8.5% of the residual, so admissible-order repricing is deferred. The joint-obligation propagation flag `PRUNE_MC_PORTAL_FORCED_NEIGHBOR` is PROMOTED (2026-09-11, default-ON); see gate 4 below. First-loss phenotyping (gate 5) concluded negative for cross-action recurrence; beam-specific, routed to WS1/WS4. Repair-side natural exposure/operator-reachability work is also resolved; its 4/28 reconstructable cases are a Card-E handoff nomination, not an open operator-capability gate. The simple same-policy resumable-tranche salvage is closed NULL and is no longer active queue work.

#### Current production boundary

Post-restoration run `34531412380` is **99/102 Corpus 1 + 1,029/1,700 Corpus 2**, net +55/-0 across both corpora, with zero errors/truncation. Corpus 2 therefore has **671 misses**. Counts from the older 975/1,700 boundary are historical sizing only. The later +21 joint-obligation promotion has not yet been folded into a full refreshed production boundary in this authority, so do not silently treat 1,050 as the canonical baseline until such a refresh exists.
...
- **Portal-aware beam coarse-state merge:** CLOSED NEGATIVE/default-OFF, incl. capability-safe salvage (2026-09-11). Frozen A/B +158/-12; exact `R01273` repro found the coarse key's blind spot (trailing visited-cell identity), but every bounded discriminator only delayed the failure. This remains an unusually strong complementary-capability signature for offline capability-memory analysis without reopening the rejected production form. [`preflight`](../reports/2026-09-09-portal-coarse-state-merge-ab-001-preflight.md), [`R01273 forensic`](../reports/2026-09-11-portal-coarse-state-merge-r01273-collision-forensic-001.md)
...
7. **Repair-side coverage: DONE (2026-09-11), both exposure and operator-reachability resolved.** Extended `census-repair-rollback-windows.mjs` with `--only=<ids>` and ran it against the full 28-level frontier population at two matched-work node budgets (30k, 300k — a 10x escalation): repair's own randomized-restart search never approaches the known-live trajectories (best natural common-prefix ~19% as deep as beam's own cull depth) and 10x more work bought zero improvement — dose-insensitive, same shape as the beam finding. Separately, seeded `searchCompletionFromPartialPath` directly at each level's exact beam-cull-depth state (liveness free from the known-live hint itself, no CP-SAT needed): **4/28 (14.3%) reconstructable, 24/28 (85.7%) operator-incapable**, referee/replay-verified — a rate statistically indistinguishable from the general-population Sept-2 program's own 21.4%/78.6% (n=28). This population's deficit is exposure, not generic operator incapability. The 4 reconstructable cases are a candidate Card-E (continuation/handoff) trigger nomination, not yet an implementation — n=4 is too small to size any mechanism. [`report`](../reports/2026-09-11-repair-side-first-loss-exposure-001.md)
...
Use capability, lifecycle, provenance, profile, variant, census, trace, accepted-path and capability-memory evidence. For hinted failures, audit provenance/dedup, select structurally diverse basins, locate where all known-live basins disappear, and classify loss as allocation/exposure, search policy, prune/state merge/representation, or other reasoning failure. Cross policies with lifecycle/census exposure so `exposed-and-failed` remains distinct from `not exposed`.

Stored paths, provenance, profile/family labels, historical policy signatures and same-level outcomes are offline diagnostics, not production routing inputs. Capability-memory overlap may nominate a policy contrast; the production selector still needs a legal generic current-level/current-solve signal and independent confirmation proportional to how much history was mined. [`evidence-layer upgrade`](../reports/2026-09-09-hint-provenance-evidence-layer-upgrade-001.md), [`capability memory`](solver-capability-memory.md)

**Frontier + structural-response characterization: ACTIVE / OFFLINE SUPPORT.** Use class 4 as the primary control for class 5, and screen broader technique niches for temporal stability plus generic-difficulty confounding before escalating to existing-family flips, source-controlled profiles or traces. Structural-response work has two exits: stable technique-relative evidence remains WS1; the same known-live extinction mechanism recurring across materially distinct actions becomes a shared-capability hypothesis and hands off to WS2/the earned specialist owner. Capability-memory signatures from materially distinct historical policies are eligible nomination evidence for such recurrence, but must be reconciled before being treated as current action capability. [`structural-response audit`](../reports/2026-09-11-structural-technique-response-extension-audit-001.md), [`capability map`](../reports/2026-09-11-future-feasibility-capability-map-001.md)

**Structural technique-response stage 3: DONE (2026-09-11), succeeds for both frozen pairs.** Bounded isolated-technique resolve (4 exact configs, matched 20M-work budget, existing family material only, one parent per pair — `R02687`/objectiveFirst, `R02094`/intersectionHarvest) finds real per-sibling `plain`-vs-`mechanic-buckets` response flips across 4/5 existing family transform modes for each pair, with the two parents flipping in opposite net directions (structure-dependent, not a universal retention ranking). Two cheap mediator candidates (object counts, portal-use timing) both fail to separate winners. Next earned stage: source-controlled solution-space mediation (must-cross order / basin descriptor) for these two pairs, not a third pair. [`report`](../reports/2026-09-11-ws1-stage3-isolated-technique-resolve-001.md)
...
|---:|---|---|---|
| 2 | Residual capability + fixed-work allocation | **ACTIVE / FIRST PRIORITY** | Joint-obligation hard-prune promoted; first-loss phenotyping concluded negative for cross-action recurrence and repair-side exposure/operator-reachability follow-up is complete. Use current residual/capability-memory evidence to size genuinely complementary capability before additional allocation work; the 4 reconstructable beam-cull states are only a Card-E handoff nomination until recurrence/headroom is demonstrated. |
| 1 | Automatic action selection | **ACTIVE / PARALLEL ANALYSIS** | Run frontier + temporal/difficulty-controlled response screening; distinguish technique-relative advantage from shared extinction. First-loss phenotyping nominates a beam score-width rank-retention pattern as WS1/WS4 follow-up. Capability-memory policy contrasts may nominate generic selector/mechanism questions, never exact-level routing. Class-1 compact-beam-menu: naive form closed, budget-dilution overlap; see `solver-future-work.md`. |
...
| 3 | Generalization/holdout discipline | **METHOD COMPLETE** | Concrete methodological failure; capability claims use grouped parents/symmetry controls and proportional confirmation. Capability-memory mining increases selection pressure rather than reducing the confirmation burden. |
...
- Preserve a material capability signature separately from a promotion disposition: a closed treatment may remain useful offline nomination evidence without reopening the tested form or retaining its failed code.
- Historical gain/loss intersections with a current residual are nominations until reconciled under current code/protocol; exact same-level history never becomes production routing input.
...
- After a material capability promotion or provenance reinterpretation, refresh/rejoin the production residual before treating old family counts, class membership, capability-memory residual intersections, or attribution shares as current.
...
- Do not wait on expensive jobs when offline analysis, source diagnosis, test hardening, docs reconciliation, or capability-memory joins can advance a separate gate.
