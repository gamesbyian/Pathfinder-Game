...
**Class 1-3 rejoin:** class 1 has no free menu headroom; class 3 is exposed-and-failed. Six genuine must-turn-guidance rows remain (`R03049` is dose/allocation). A 7M pilot and the real ladder reproduce referee-valid gains on `R02768` and `R02180` only after plain late repair fails. The next gate is a **60-row participant-aware economics/collateral A/B**: 40 control-unsolved + 20 control-downstream-solved actual participants, excluding the development pair. [`pilot`](../reports/2026-09-13-must-turn-biased-repair-dose-pilot-001.md), [`preflight`](../reports/2026-09-13-ws2-class2-class4-allocation-preflight-001.md)

**Class 4:** corrected capability-memory union is **179 rows** (`14/28/21/116/0` across classes 1-5). Portal coarse-state merge nominates 113/123 class-4 rows; global merge stays closed after 12 losses. An 8-level freshness replay solved **8/8, referee-valid**. Next: a **default-off true-dead-last additive whole-ladder retry** enabling portal coarse-state merge only inside that retry. Canary: the 8 freshness rows + `R01273` + non-portal controls; only a pass earns the 113-row allocation test. [`freshness`](../reports/2026-09-13-class4-portal-coarse-freshness-replay-001.md), [`preflight`](../reports/2026-09-13-ws2-class2-class4-allocation-preflight-001.md)

**Class 5:** generic width/retention rescue, `swap`/`cs`, bounded known-capability composition and base-T1 harvest are closed. Six scalar future-feasibility summaries over **31 exact-labelled states / 15 levels** overlap LIVE and DEAD. The connectivity/dead-cause rejoin is also closed: only **1/12** deterministic current Class-5 rows had >=25 records in the historical cluster, despite real cross-state recurrence inside `R03046`. Do not build cause reuse for this sparse population. [`feasibility`](../reports/2026-09-12-future-feasibility-descriptor-rejoin-result-001.md), [`dead-cause result`](../reports/2026-09-13-class5-dead-cause-current-population-rejoin-result-001.md)

**Archaeology-derived acquisition order:** dead-cause reuse and the August low-cardinality full-pool quota projection are closed in their tested forms. The live Class-5 gate is the **homotopy/winding completion-class observer** with puncture-robustness checks. Future-crossing commitments or arbitrary-target feasibility remain later mechanism-specific microscopes. [`dead-cause result`](../reports/2026-09-13-class5-dead-cause-current-population-rejoin-result-001.md), [`categorical correction`](../reports/2026-09-13-class5-categorical-projection-archaeology-correction-001.md), [`register`](solver-archaeology-register.md)

**Next WS2 gates, in parallel:**

1. **Class-4 allocation:** code the dedicated dead-last retry; require the 8-row freshness canary plus `R01273` and non-portal controls before the 113-row population. [`preflight`](../reports/2026-09-13-ws2-class2-class4-allocation-preflight-001.md)
2. **Class-2 economics:** materialize the frozen 60-row participant cohort and run matched control/treatment arms for the 7M `late-repair-must-turn-biased-retry`. Require target-stage participation, gains/losses, downstream displacement, incremental `workSpent`, wall cost and symmetric censoring. [`preflight`](../reports/2026-09-13-ws2-class2-class4-allocation-preflight-001.md)
3. **Class-5 acquisition:** reconstruct the smallest offline homotopy/winding observer on current exact-labelled extinction cases. Use robust puncture choices; no in-loop topology feature, quota, or scorer change until a recurring relation appears. [`dead-cause result`](../reports/2026-09-13-class5-dead-cause-current-population-rejoin-result-001.md)
...
|---:|---|---|---|
| 2 | Residual capability + allocation | **ACTIVE / FIRST** | Class-4 dead-last canary then 113-row allocation; Class-2 60-row 7M economics A/B; Class-5 homotopy/topological observer. |
...
| 4 | Beam retention | **CLOSED IN TESTED FORMS** | Generic forms and August low-cardinality quota keys stay closed; new independent categorical/topological evidence may support a changed premise. |
