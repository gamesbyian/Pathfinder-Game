...
**First-loss program:** two disjoint 14-level samples reproduce the same beam phenotype (score-width culls); DFS falsifies a shared-scorer failure. [`development`](../reports/2026-09-11-bounded-class4-class5-first-loss-phenotyping-001.md), [`confirmation`](../reports/2026-09-11-ws2-independent-first-loss-confirmation-001.md). **Repair-side:** natural repair search barely approaches the same trajectories even at 10x budget; seeded at beam-cull state only 4/28 reconstruct — too small to justify a mechanism. [`report`](../reports/2026-09-11-repair-side-first-loss-exposure-001.md) **Two bucket-key canaries, both CLOSED NEGATIVE** on the same population: frontier bucketed by `ints` ([`report`](../reports/2026-09-12-ints-bucket-retention-canary-001.md)), and the existing must-cross-heavy-only `mechanicBucketRetention` applied here (verified non-degenerate: all 28 have real mustCross counts) ([`report`](../reports/2026-09-12-mechanic-bucket-on-intersection-heavy-canary-001.md)) — zero new solves either way, deltas inside noise.
