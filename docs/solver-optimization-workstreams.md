...
| 1 | Automatic action selection | `supporting` | `bounded-compute` | **LATE-CONTINUATION SIGNAL / CONFIRMATION PREFLIGHTED** | Freeze an independent-parent family block per the preflight; Stage A = 24-parent opportunity canary, Stage B = 96-parent confirmation only if earned; exact frozen model, no refit. [preflight](../reports/2026-09-22-ws1-independent-continuation-confirmation-preflight-001.md) | `WS1-ACTION-SELECTION-LEGAL-SIGNAL-CAPTURE` |
