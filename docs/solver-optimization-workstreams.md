...
1. **Must-cross neighbour-budget propagation.** Portal derivation is closed and the shadow helper's post-August dense-index drift is repaired. Correctness gates clean (0 alive false rejects on the full 5,518-branch oracle-labelled atlas restricted to the 922 portal+must-cross branches; 0 soundness violations replaying every stored solution across all 3 corpora with the flag forced on). Population/envelope/acceptance rule frozen for the deterministic 530-level portal+must-cross A/B (preflight below). **Next:** dispatch in progress — control arm complete (193/530 solved); treatment arm's main-body run is in progress, gap-fill subset shows one confirmed gain (R02915) over control so far. Combine per-level once the treatment main body (+ any needed gap-fill) lands and apply the frozen acceptance rule. See [`preflight`](../reports/2026-09-09-mc-neighbor-budget-portal-ab-001-preflight.md), [`evidence hardening`](../reports/2026-09-09-portal-restoration-evidence-hardening-001.md).
2. **Portal-aware beam coarse-state merge.** `portalJumps` + `lastWasPortalJump` capture counted length/transient state but not which portal pairs were consumed. Aliasing under that proposed key is measured **material** (1.7% of merge-candidate groups, 2.2% of grouped candidates, 80/80 sampled portal levels) — see [`aliasing measurement`](../reports/2026-09-09-portal-beam-used-pair-aliasing-measurement-001.md). Implemented behind opt-in `STRATEGY_PORTAL_COARSE_STATE_MERGE`: `BeamNode.usedPortalPairs` preserves exact used-pair identity in the merge key; unit-verified. **Next:** dispatch the frozen fixed-work A/B against current no-portal-merge control (population/envelope/acceptance rule frozen in [`preflight`](../reports/2026-09-09-portal-coarse-state-merge-ab-001-preflight.md)), reporting gains/losses and specialist retention. See [`beam preflight`](../reports/2026-09-09-portal-beam-state-identity-preflight-001.md).
