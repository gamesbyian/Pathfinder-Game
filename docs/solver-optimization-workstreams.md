...
1. **Post-1,029 residual atlas: DONE.** Full per-level five-class rejoin of the 671 misses. Result: not-offered 26 (3.9%), offered-unreached/starved 21 (3.1%), reached-comparable-work-failed 36 (5.4%), no-T1-winner-but-provenance-rescuer 200 (29.8%), no-known-rescuer 388 (57.8%). Portal-bearing structure dominates every class (497/671, 74.1%). Also nominates a small, cheap WS1 menu-expansion candidate: 26 never-offered levels, two recurring `width=5000`/`mechanic-buckets` beam configs. Any menu experiment must be rebuilt from this current 26-level population under fixed total work; do not replay the older pre-1,029 8/9 four-beam nomination. [`atlas report`](../reports/2026-09-11-post-1029-residual-atlas-001.md), [`audit reconciliation`](../reports/2026-09-11-audit6-action-selection-handoff-reconciliation.md)
...
4. **No-known-rescuer residue (388/671, 57.8%): NEXT, atlas-justified.** Run the **observer-only joint-obligation propagation pilot** with class-4/composition near-controls and its existing zero-false-rejection/material-dead-work gates. In parallel, bounded all-known-basin first-loss work should classify evidence into allocation, joint-feasibility, residual-interface, state-equivalence, rank/retention, repair-reachability, correctness, or `unknown` without forcing one label per level. Cross-action recurrence at the same residual-state boundary is the gate for a shared-capability claim; otherwise keep technique/config research primary. [`joint-obligation`](../reports/2026-09-09-joint-obligation-propagation-and-residual-lane-handoff-001.md), [`frontier contrast`](../reports/2026-09-11-algorithmic-frontier-class4-vs5-contrast-001.md), [`capability map`](../reports/2026-09-11-future-feasibility-capability-map-001.md)
...
| 1 | Automatic action selection | **ACTIVE / PARALLEL ANALYSIS** | Run frontier + temporal/difficulty-controlled response screening; distinguish technique-relative advantage from shared extinction. The 26-level never-offered beam cohort remains a small parallel fixed-total-work candidate. |
...
- Production-baseline provenance is tri-state: missing provenance stays `unknown` and cannot populate an unsolved/frontier cohort.
- Reusable benchmark/census rows require compatible protocol identity; do not splice partial reuse into across-level parallel runs when that changes the contention regime.
- A forced retry that does not change effective behavior is not a distinct treatment and must not buy or reserve a second dose of work.
