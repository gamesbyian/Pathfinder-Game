...
| 1A | Remaining-length allocation | `supporting` | `bounded-compute` | **INTRA-SOLVE BRIDGE PREFLIGHTED / IMPLEMENTED INERTLY** | Frozen Stage A: same elite-prefix candidate multiset and matched node budgets, legacy order vs ascending remaining length on the calibrated 20-level closest-miss set. Research-only switch + attribution trace are implemented but not dispatched. [preflight](../reports/2026-09-22-ws1-remaining-length-intrasolve-bridge-preflight-001.md) | `WS1-REMAINING-LENGTH-INTRA-SOLVE-BRIDGE` |
