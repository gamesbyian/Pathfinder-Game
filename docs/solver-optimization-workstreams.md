...
| 2I | Capability invention demand | `active` | `bounded-compute` | **ACQUISITION / COMPACT CONTRAST EXHAUSTED** | Fresh 1,802-parent compact evidence has been reduced against solved-parent failed controls. Once conditioned on the same stage failing, stage-level work/cap profiles are broadly similar, so do not buy more generic compact telemetry. Advance to mechanically sampled first-loss / operational-divergence evidence outside capability-selected Class 3, using the fresh compact denominator for sample freezing/stratification and solved controls; exact/reference annotation stays downstream. [fresh reconciliation](../reports/2026-09-22-canonical-stress-refresh-failure-evidence-reconciliation-001.md) | `WS2-CAPABILITY-INVENTION-DEMAND` |
