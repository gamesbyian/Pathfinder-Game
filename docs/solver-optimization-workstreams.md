...
| 2I | Capability invention demand | `active` | `existing-data` | **ACQUISITION / FRESH FAILURE DENOMINATOR AVAILABLE** | First run a contrastive parent-level digest over fresh compact response: current misses vs solved parents with failed/capped attempts, stratified by normalized action/stage and protocol. Use it to choose whether broader first-loss sampling is still the cheapest discriminator; do not launch another generic failure-data acquisition first. [fresh reconciliation](../reports/2026-09-22-canonical-stress-refresh-failure-evidence-reconciliation-001.md) | `WS2-CAPABILITY-INVENTION-DEMAND` |
