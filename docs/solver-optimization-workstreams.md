...
|---:|---|---|---|
| 2 | Residual capability + fixed-work allocation | **ACTIVE / FIRST PRIORITY** | Joint-obligation hard-prune promoted; first-loss phenotyping concluded negative for cross-action recurrence — beam-specific, routed to WS1/WS4. Next: independent-sample repeat and/or repair coverage before further capability-map escalation. |
| 1 | Automatic action selection | **ACTIVE / PARALLEL ANALYSIS** | Run frontier + temporal/difficulty-controlled response screening; distinguish technique-relative advantage from shared extinction. The 26-level never-offered beam cohort remains a small parallel candidate. First-loss phenotyping nominates a beam score-width rank-retention pattern as WS1/WS4 follow-up. |
