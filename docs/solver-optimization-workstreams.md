...
**Class-4 capability-memory aggregate corrected.** The published six-source union of 65 was arithmetically impossible because portal coarse-state merge alone has 137 current-residual historical nominations. Existing-data reconstruction of all six named sources yields an exact **179-row union**, distributed **14 / 28 / 21 / 116 / 0** across residual classes 1–5. Portal coarse-state merge alone nominates **113/123 class-4 rows**; the full panel nominates **116/123**, while still reaching zero class 5. Its globally enabled form remains correctly closed negative because of 12 losses and the hard `R01273` regression, but its positive basin now owes the capability-memory contract's cheap freshness test before class 4 is treated as exhausted. Historical nominations are not solve claims. [`correction`](../reports/2026-09-12-capability-memory-union-reconciliation-001.md) The six-source union is now also a regenerable machine manifest (5/6 sources; the sixth, production-boundary displaced solved-set losses, is not yet relocated) — reproduces **167/652** exactly. [`manifest`](../reports/2026-09-13-capability-memory-manifest-hardening-001.md)

**Class-4 freshness CONFIRMED.** An 8-level prespecified stratified sample of the 113 class-4 portal-coarse-state-merge nominations (spanning all three represented routing regimes) was replayed locally under the existing default-off flag, current code, and production-matched work semantics: **8/8 solved, 8/8 referee-valid**. The September 9 positive basin is fresh, not stale. The globally enabled merge form stays closed (`R01273` regression); the live next question is the least-disruptive changed-treatment exposure form — a dead-last additive whole-ladder retry (same `runWholeLadderRetryTier` pattern as the promoted `must-cross-neighbor-prune-disabled-retry`/`connectivity-axis-exhausted-retry`/`guidance-goal-distance-retry` tiers) that cannot regress any currently-solving level because every additive tier only runs when `!result.solution`. Not yet implemented. [`freshness result`](../reports/2026-09-13-class4-portal-coarse-freshness-replay-001.md)
...
1. **Class-4 allocation form (freshness answered fresh 8/8):** implement and validate the dead-last additive whole-ladder retry described above — the least-disruptive exposure form, not the closed global merge. Confirm it cannot touch `R01273` or any other currently-solving level (structural, by tier ordering) before any population-scale measurement. Do **not** reopen the globally enabled merge.
