...
**Current WS2 gate:** the frozen **53-parent compact-response acquisition** (redispatched with durable persistence after PR #1927 fixed an artifact-access gap) serves failure-response Stage A and Class-3 dose (23 residual + 30 solved controls); analysis pending. **Reserve-starvation Stage-1 CONCLUDED POSITIVE:** 2/40 independent parents (R01154 at 179.0M nodes, R03270 at 172.7M) show a genuine reserve-starvation opportunity (solved above the 75M reserve, within the 300M total envelope), clearing the `>=2` bar to design (not yet dispatch) a matched-total-work `admissibleOrderNodeReserveFractionOverride` A/B. First-loss/rejection/2x2 still require explicit selection. [`freeze`](../reports/2026-09-20-ws2-class3-shared-acquisition-freeze-001.md) · [`reserve-starvation result`](../reports/2026-09-20-admissible-order-reserve-starvation-probe-result-001.md) · [`future`](solver-future-work.md)
