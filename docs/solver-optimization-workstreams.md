...
**Current WS2 gate:** HARVEST's repair node-cap seam is implemented ([`seam + A/B preflight`](../reports/2026-09-25-ws2-repair-deadline-allocation-node-cap-seam-and-ab-preflight-001.md)); run the execution-family canary, then dispatch the predeclared matched-work A/B. Reserve repricing stays separate. ACQUISITION runs beam freshness, then uses the fresh 1,802-parent compact failure-response corpus as the existing-data gate before expanding first-loss sampling or buying prefix/orientation traces. A candidate residual phenotype must differ from solved-parent failed-attempt controls at the parent level before it earns richer capture. BC1 advances to inert safety/economics. Forced-work prevalence is 25.33%, but expansion/pruning is already paid when forcedness is known; the global-phase compression route is now closed negative (0.25% singleton-outcome phases, 1,234 canonical work units of discovery cost), confirming the reservoir is local-parent forcedness inside a still-branching beam, not global collapse. [`fresh failure evidence`](../reports/2026-09-22-broad-capability-refresh-failure-evidence-reconciliation-001.md) · [`invention`](solver-capability-invention-program.md) · [`forced-work`](../reports/2026-09-21-forced-work-prevalence-result-001.md) · [`phase census`](../reports/2026-09-25-forced-work-capture-economics-phase-census-result-001.md)
...
|---:|---|---|---|---|---|---|
| 2 | Residual capability + premise acquisition | `active` | `bounded-compute` | **HARVEST** | Node-cap seam implemented (`earlyRepairSearchOrdinaryNodeBudgetOverride`/`earlyRepairSearchBiasedNodeBudgetOverride`); run the execution-family canary, then dispatch the predeclared 53-parent matched-work A/B (treatment caps 21M/38M). [preflight](../reports/2026-09-25-ws2-repair-deadline-allocation-node-cap-seam-and-ab-preflight-001.md) | `WS2-REPAIR-DEADLINE-ALLOCATION` |
...
| 2I | Capability invention demand | `active` | `design` | **T1 COHORTS BLOCKED / EW1 ROUTING-GAP SAMPLE REGISTERED** | Do not buy more generic compact telemetry. The full **83 T1-rescuable / 47 singleton-supported** cohort needs the technique-census combine artifact, which this environment's network policy currently blocks (GHA artifact host); reopen when a broader allowlist or a durable repo-side join exists. Meanwhile, a fully local existing-data sample (EW1 pricing x production-reach join) found 2 exact-config F8 routing/deployment misses (`CID-0027`/`CID-0028`, one singleton-T1-supported); design the smallest matched-work routing-exposure test for those before any broader campaign. [EW1 routing-gap sample](../reports/2026-09-25-capability-invention-demand-ew1-routing-gap-sample-001.md) · [fresh reconciliation](../reports/2026-09-22-broad-capability-refresh-failure-evidence-reconciliation-001.md) · [census closeout](../reports/2026-09-22-technique-census-broad-evidence-closeout-001.md) | `WS2-CAPABILITY-INVENTION-DEMAND` |
