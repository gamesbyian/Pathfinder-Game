...
| 1A | Remaining-length allocation | `supporting` | `bounded-compute` | **INTRA-SOLVE BRIDGE PREFLIGHTED** | Before any production consumer, test the missing transport step: same 12 elite-prefix completion candidates, same 15k/candidate + 90k shared cap, legacy order vs global ascending remaining length on the frozen 20-level closest-miss set. [preflight](../reports/2026-09-22-ws1-remaining-length-intrasolve-bridge-preflight-001.md) | `WS1-REMAINING-LENGTH-INTRA-SOLVE-BRIDGE` |
