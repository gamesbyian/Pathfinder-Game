...
1. **Class-4 allocation:** code the dedicated dead-last retry; require an 8-row freshness canary + `R01273`/non-portal controls before the 113-row current class-4 allocation population. Global enablement remains closed. [`preflight`](../reports/2026-09-13-ws2-class2-class4-allocation-preflight-001.md)
...
|---:|---|---|---|
| 2 | Residual capability + allocation | **ACTIVE / FIRST** | Class-4 dedicated dead-last canary then 113-row allocation; Class-2 frozen 60-row 7M economics A/B; Class-5 dead-cause rejoin, then categorical full-pool if null. |
