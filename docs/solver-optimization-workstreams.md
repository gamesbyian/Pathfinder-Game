...
3. **Admissible-order retry `1.0 → 0.18`.** Matched-work test with nonzero target-stage work required. Production stays `1.0`. Confirmation-006's result was non-participating (forensic correction: [`artifact recovery`](../reports/2026-09-10-admissible-order-confirmation-006-artifact-recovery.md)); a follow-up local probe fixed participation but found the tier's own stacked NODE ceiling, not the work fraction under test, binds at small scale — see [`matched-work methodology`](../reports/2026-09-10-admissible-order-non-default-retry-matched-work-methodology-001.md) for the open sizing question before any next dispatch. [`Confirmation 006`](../reports/2026-09-05-admissible-order-non-default-retry-repricing-confirmation-006.md)
