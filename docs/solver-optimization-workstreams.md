...
**State:** active. The 671-miss residual atlas is done; portal coarse-state salvage is done and CLOSED NEGATIVE. Allocation/exposure is only 8.5% of the residual, so admissible-order repricing is deferred; the provenance-relevance correction expands the atlas-justified observer-only joint-obligation population to the 419-level no-known-rescuer residue. The simple same-policy resumable-tranche salvage is closed NULL and is no longer active queue work.
...
1. **Post-1,029 residual atlas: DONE, provenance-corrected.** Full per-level five-class rejoin of the 671 misses. Result: not-offered 26 (3.9%), offered-unreached/starved 21 (3.1%), reached-comparable-work-failed 36 (5.4%), no-T1-winner-but-deterministic historical production-context candidate 169 (25.2%), no known deterministic historical or T1 candidate 419 (62.4%). The former 200/388 split admitted 31 randomized research discoveries as production-context rescuers. The 169 retained rows are longitudinal nominations, not evidence that current production can rescue those levels. Portal-bearing structure still dominates the full residual (497/671, 74.1%). Also nominates a small, cheap WS1 menu-expansion candidate: 26 never-offered levels, two recurring `width=5000`/`mechanic-buckets` beam configs. [`atlas report`](../reports/2026-09-11-post-1029-residual-atlas-001.md), [`provenance audit`](../reports/2026-09-11-hint-provenance-evidence-relevance-audit-001.md)
...
3. **Admissible-order repricing: DEFERRED.** Atlas classes 2+3 (allocation/exposure) are only 8.5% of the residual, materially smaller than the 62.4% no-known-rescuer class — per the gate's own rule, do not spend the next population-scale run here. If ever run, use the tier-scoped enforcement path and prove target-stage participation first.
4. **No-known-rescuer residue (419/671, 62.4%): NEXT, atlas-justified.** Run the **observer-only joint-obligation propagation pilot** with corrected class-4/composition near-controls and its existing zero-false-rejection/material-dead-work gates. In parallel, bounded all-known-basin first-loss work should classify evidence into allocation, joint-feasibility, residual-interface, state-equivalence, rank/retention, repair-reachability, correctness, or `unknown` without forcing one label per level. Cross-action recurrence at the same residual-state boundary is the gate for a shared-capability claim; otherwise keep technique/config research primary. [`joint-obligation`](../reports/2026-09-09-joint-obligation-propagation-and-residual-lane-handoff-001.md), [`frontier contrast`](../reports/2026-09-11-algorithmic-frontier-class4-vs5-contrast-001.md), [`capability map`](../reports/2026-09-11-future-feasibility-capability-map-001.md)
...
|---:|---|---|---|
| 2 | Residual capability + fixed-work allocation | **ACTIVE / FIRST PRIORITY** | Joint-obligation observer + bounded class-4/class-5 first-loss phenotyping; route only recurring cross-action mechanisms to generic capability work. |
