...
1. **Must-cross neighbour-budget propagation.** Next: corrected reference-labelled portal branch set with zero alive false rejects, then frozen A/B on the 530 portal+must-cross levels; enumerate gains/losses and keep published regression green. [`Evidence`](../reports/2026-09-09-portal-restoration-evidence-hardening-001.md)
2. **Portal-aware beam coarse-state merge.** Measure aliasing from omitted consumed-pair identity on a prespecified observer population, using diverse validated portal prefixes as positive-oracle coverage. If material, preserve exact used-pair identity; then fixed-work A/B versus current no-portal-merge control. [`Preflight`](../reports/2026-09-09-portal-beam-state-identity-preflight-001.md)
3. **Connectivity volume check.** Existing screen: 266,320 valid paths / 21.8M prefixes, zero rejects. Next: matched-work A/B on all 954 portal levels. Keep false-goal mirror separate pending zero-loss endpoint differential. [`Evidence`](../reports/2026-09-09-portal-restoration-evidence-hardening-001.md)
4. **Same-parity portal parity prune/gate.** Next: bounded implementation plus unit/differential/published-regression coverage, including zero rejection of representative known-live portal prefixes.

Primary catalog: [`portal carve-outs`](../reports/2026-09-09-portal-carveout-and-additive-tier-solve-rate-catalog-001.md).

After material restorations, repair the two-stage lifecycle instantiation projection before refreshing production/capability telemetry; the current projection can falsely mark retry tiers mechanically ineligible. [`Telemetry gap`](../reports/2026-09-09-stage-lifecycle-instantiation-projection-gap-001.md)
