...
**Class 5:** prior generic capability forms and fixed-endpoint homotopy are closed as documented. A research-only lifted open-path phase observer is now implemented: it integrates along the actual prefix without endpoint closure or integer rounding, conditions on endpoint geometry, uses only board-derived obstacle punctures, abstains on portal jumps, and passes endpoint-fixed homotopy/gauge fixtures. Next: apply it to existing exact-labelled material within endpoint/geometry-controlled strata; stop if those data remain contrast-starved before considering a small human-parent intervention. [`homotopy result`](../reports/2026-09-13-class5-homotopy-prefix-census-result-001.md)
...
3. **Class-5 acquisition:** run the implemented observer on existing exact labels within endpoint/geometry controls. If a clean causal contrast is missing, stop before any label campaign; only then consider the smallest human-parent controlled family.
