...
> **Status:** CONCLUDED / NEGATIVE (both tested regimes). See [`B2 result`](../reports/2026-09-17-lane-e-repair-retreat-commitment-probe-result-001.md), [`multi-pick result`](../reports/2026-09-17-lane-e-multi-pick-bisection-result-001.md).
...
> **First observer result:** CP-SAT bisection on B2's only 2 qualifying same-parent dead/live pairs (sharing a literal common prefix) cleanly locates the exact point of no return in both, but both land 1-2 moves from the trajectory's own end -- a regime where rollback distance and a compact commitment predict the same thing, so neither case discriminates the premise. Critical move in both cases was a plain, non-mechanic-obligation move.
> **Second observer result (fresh multi-pick population, once the sibling-constructor handoff gate closed):** 4 more matched pairs, divergences starting much earlier (depth 4 of 11, 7 moves of rollback distance available) -- the point of no return again lands EXACTLY at the naive divergence point in all 4 (2 independent divergence points, one replicated 3x by differing downstream continuations, all agreeing). 6/6 tested pairs across two structurally different regimes show no daylight between naive divergence and the true causal point. Premise closed in both tested regimes.
