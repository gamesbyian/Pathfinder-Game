...
Local `ci` runs `check`, `test:coverage`, then `test:node`. The major phases remain serial because each has internal concurrency; running coverage/check/Node concurrently on one 4-core machine can oversubscribe it. Heavy implementation proofs are ordinary deep Vitest files locally: the two deadlock root subtrees and the enabled/disabled R02560 repair-regression halves can therefore use Vitest's existing file worker pool inside the single coverage phase. `npm run test:deep-proofs` is the canonical explicit four-file partition used when those proofs need separate ownership. GitHub Actions uses two authoritative PR-gate lanes plus one non-gating shadow-routing job: `fast-gate` owns non-lint checks, lint, Node/CLI contracts, and the production build on one runner/install; `deep-verification` owns the single covered ordinary Vitest population followed by the explicit four-file deep-proof command on a second runner/install; `impact-shadow` computes the proposed scoped validation plan from the tested PR merge diff without installing dependencies or suppressing any existing validation. The coverage step skips only those four delegated proof files, so the proof population remains complete and non-duplicated. `package.json` is authoritative for the local graph.

The two authoritative gate lanes remain intentional tail-latency control. The lightweight `impact-shadow` job is observational infrastructure, not a third validation lane, and should not acquire dependency installation or duplicate software tests. In August 2026 the repository briefly benefited from splitting cheap validation layers across six hosted runners because checkout/setup/install were consistently cheap. Subsequent Actions runs showed identical cached `npm ci` work ranging from seconds to many minutes, plus occasional long hosted-runner assignment queues. With many independent jobs, PR completion becomes the maximum of several noisy setup times: the gate waits for the unluckiest runner. Do not split `fast-gate` or `deep-verification` into additional hosted-runner jobs merely because the commands are logically independent. Require current measurements showing a reliable wall-time win after checkout/setup/install variance before adding another PR-gate runner.
...
`deepTest` is for expense intrinsic to an implementation proof: exhaustive soundness, real regression rescue, or real cross-tier budget behavior. `SOLVER_DEEP_TESTS=0` skips them; `test:unit:fast`/`ci:fast` set it. Every PR still runs every deep proof. The expensive deadlock-soundness proof is represented by two deep Vitest files, one for each root move of the fixed 5x5 must-cross fixture; those subtrees are disjoint and collectively exhaustive, and each file repeats the negligible must-turn fixture so it independently exercises every helper/control. The provisional R02560 partial-path-completion integration witness is two deep files sharing the same 900,000-node boundary: production-default enabled must solve, while disabling only that feature must not. It remains only because the earlier synthetic trap failed to isolate this mechanism correctly; replacement by a smaller faithful witness is preferred. Local coverage runs all four through Vitest's normal worker pool. In Actions, `deep-verification` first runs coverage with proof-specific skip flags and then runs those same four files through `npm run test:deep-proofs` on the same runner, so the proof population is unchanged, not duplicated, and does not pay a second checkout/setup/dependency installation.
...
- Actions has two authoritative PR-gate jobs plus dependency-free `impact-shadow`. `fast-gate` runs `check:nonlint`, `check:lint`, `test:node`, and `build` serially after one checkout/setup/install. `deep-verification` runs covered ordinary Vitest and then `test:deep-proofs` after one checkout/setup/install. The two jobs race each other; cheap subcommands inside each lane stay serial because their internal concurrency already uses the runner and extra hosted-runner fan-out has high tail-latency variance.
