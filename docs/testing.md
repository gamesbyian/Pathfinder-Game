...
|---|---|---|---|
| Smoke | `npm run stress:smoke` | 14 pins, <60s | Obvious breakage across every mechanic family + historical bug regressions. Four stale stress ids were replaced with current, purpose-matched pins on 2026-08-07; see `future-work.md`'s completed smoke-set entry. |
| Regression (pinned) | `npm run stress:regression` | 8 levels: 5 solved historical canaries + 3 current known-hard targets; minutes because repair fallbacks can exceed the nominal per-level budget | Solved canaries regressing; known-hard levels becoming improvements. Refresh matching solved timings with `npm run stress:regression -- --update-baselines`; expectations never change automatically. |
