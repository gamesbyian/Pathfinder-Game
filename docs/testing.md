...
- Actions has independent `fast-gate` static/validator/lint work, an impact-scoped reusable `production-build` lane, two execution-owner `node-contracts` shards, impact-scoped `deep-verification` coverage, `deep-services`, and dependency-free `impact-shadow` planning. As of the 35-second critical-path audit, the current full-impact layout is a **baseline to beat, not a protected topology**: broad phases may be split or raced when same-runner/hosted-runner measurements show that parallel execution reduces end-to-end wall time without weakening failure semantics.
...
- Data-consuming lanes initially omit the large runtime level/hint trees from sparse checkout. The shared runtime-data action derives exact Git-blob identity, restores an exact cache when available, otherwise restores the newest prior `runtime-data-v2-` generation and reconciles changed/added/deleted blobs against HEAD before use. Fast Gate and the Node shards consume that action; coverage is intentionally data-free after real-data integrity checks moved to explicit Node contracts. `build` no longer performs a second independent materialization of the same tree.
