...
Local `ci` runs `check`, `test:coverage`, then `test:node`. The local phases remain serial because each has internal concurrency and a single 4-core workstation can oversubscribe when broad phases race each other. Heavy implementation proofs remain ordinary deep Vitest files locally; `npm run test:deep-proofs` is the canonical explicit four-file partition used when those proofs need separate ownership.

GitHub Actions now has one universal PR lane plus an impact-scoped deep lane. `fast-gate` always runs package/script reachability, textual invariants, the validator population, lint, the full Node/CLI contract population, the solver capability canary, and the production build. `impact-shadow` computes the semantic impact plan from the tested PR merge diff and is authoritative only for whether `deep-verification` is required. `deep-verification` runs covered ordinary Vitest, the explicit four-file deep-proof command, and the Firestore persistence boundary when the plan selects the deep lane. Manual workflow dispatch always runs deep verification. If the impact planner itself fails, deep verification runs conservatively rather than being skipped. The broad `main-push-validation.yml` workflow remains the integration/direct-main backstop.

The coverage step excludes only the four delegated proof files, so a selected deep lane still executes the complete intended proof population exactly once. Impact routing changes cadence, not the meaning of a green selected lane. `package.json` remains authoritative for the local graph.
...
The CI impact-routing program is tracked in [`ci-impact-routing-plan.md`](ci-impact-routing-plan.md). Semantic ownership is now live infrastructure: `scripts/validation-groups.json` classifies the existing `check:validators` and `test:node` members into `repo`, `game`, `persistence`, `solver`, `research`, `data`, and conservative `shared` groups. The current production activation scopes only the expensive `deep-verification` lane; the universal fast gate is intentionally not dependency-local or surface-scoped yet.
...
For targeted local diagnosis, semantic group aliases exist for validator and Node-test families, for example `npm run check:validators:repo` and `npm run test:node:repo`. These remain diagnosis/profiling surfaces; ordinary PR `fast-gate` still executes the full validator and Node/CLI populations.
...
`deepTest` is for expense intrinsic to an implementation proof: exhaustive soundness, real regression rescue, or real cross-tier budget behavior. `SOLVER_DEEP_TESTS=0` skips them; `test:unit:fast`/`ci:fast` set it. PRs whose semantic impact requires deep verification run every deep proof; unrelated PRs may skip the entire deep lane under the fail-safe impact plan. The expensive deadlock-soundness proof is represented by two deep Vitest files, one for each root move of the fixed 5x5 must-cross fixture; those subtrees are disjoint and collectively exhaustive, and each file repeats the negligible must-turn fixture so it independently exercises every helper/control. The provisional R02560 partial-path-completion integration witness has two distinct roles and they deliberately run at different cadences. The **enabled** file is a cheap positive solver-regression witness: production-default behavior must still solve the historical case within the published 900,000-node ceiling. The **disabled** file is an expensive causal/effectiveness characterization: with only that feature disabled, the historical case did not solve within the same ceiling. The disabled negative is not a merge-safety invariant — a future solver improvement may legitimately make it false — so ordinary PR CI does not run it. It remains available through `npm run test:solver-effectiveness-characterizations` for intentional mechanism attribution/research checks. The earlier synthetic trap failed to isolate this mechanism correctly, so replacement of the enabled historical witness by a smaller faithful fixture is still preferred. Local ordinary coverage skips the explicit proof files. In Actions, `deep-verification` runs `npm run test:deep-proofs` only when the impact plan selects the solver deep-proof capability; coverage-only/game and Firestore-only/persistence changes do not pay solver-proof cost.
