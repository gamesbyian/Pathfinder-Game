...
GitHub Actions now uses semantic impact routing in both lanes. `fast-gate` remains the always-materialized runner so lint/build/startup costs do not acquire another hosted-runner dependency, but it computes the merge-diff plan locally and runs only the selected validator and Node/CLI groups. If that local router fails, Fast Gate fails safe to the full `check:validators` and `test:node` aggregates. Package/script reachability and textual invariants remain separate deterministic steps. `impact-shadow` computes the same plan independently as an inspectable routing artifact and owns whether `deep-verification` is required. `deep-verification` independently selects covered ordinary Vitest, the two-file hard-prune soundness command, and the Firestore persistence boundary. Planner failure remains conservative. The broad `main-push-validation.yml` workflow remains the integration/direct-main backstop.
