...
| A3 main-seeded ESLint cache | **merged / measured green** | #2054 seeded the default-branch generation. Main run 35961565813 restored it in ~1 s and lint completed in **2 s**, versus ~15 s on the first cold seed. |
| A4 250k solver canary | **merged / measured green** | #2056 retained the exact nine fixtures. Main run 35961565813 completed the production canary in **2 s**; hosted probe solved 9/9 in ~1.5 s / 1,303,532 nodes. |
