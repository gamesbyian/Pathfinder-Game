...
| A3 main-seeded ESLint cache | **merged / default-branch seed proven** | #2054 merged. Main-push run 35961515300 missed the first generation, ran full lint, then saved `eslint-Linux-...-55ee4ea...`; next unrelated PR/main run is the inheritance proof. |
| A4 250k solver canary | **merged / measured green** | #2056 merged. Exact original 9-level fixture set is retained; hosted evidence solves **9/9 in ~1.5 s / 1,303,532 nodes** at 250k. Production main timing is being observed in run 35961565813. |
| A2 exact Node 22.23.2 | **full-contract rehearsal green; production migration ready** | #2057 run 35961318833 passed planner, validators, lint, Node/CLI, canary, build, coverage, deep proofs, and Firestore. setup-node was ~1-3 s. Production migration must seed/use a Node-22 Firebase CLI cache generation rather than reuse Node-20 npx materialization. |
