...
| A1b fast runtime-data cache-miss recovery | **merged / measured green** | #2067: exact-current miss recovered from seeded `HEAD^1` cache in **2 s**, one changed blob overlay in **1 s**, current-generation save in **2 s**; three whole-tree materialization shapes at 52–56 s and one partial-clone archive shape at ~102 s were rejected. |
...
| A2 exact Node 22.23.2 | **merged / measured green** | #2066 pins PR/main/scoped to exact Node 22.23.2 after full-contract rehearsal; setup-node measured **0–3 s** and the Node-22 Firebase CLI generation is isolated from Node 20. |
| C exact dependency-tree restore | **merged / measured green on hit** | #2069: exact OS+arch+Node+npm+lockfile restore measured **2 s** in fast and deep with `npm ci` skipped; full validators/Node/coverage/proofs/Firestore contract stayed green. |
| A5 remove planner dependency edge | **merged / positive+negative cases green** | #2070: full-impact deep starts concurrently with fast/planner and preserves all deep obligations; #2073 docs-only rehearsal had both planners select `deep_job_required=false` and deep exited in **7 s** before runtime-data/dependency/test setup. |
| B2 coverage sharding | **measurement rehearsal in progress** | Current warm covered Vitest remains ~30 s, above the ≤19 s useful-work budget. Two Vitest shards now collect native blob+coverage reports and a merge runner enforces the unchanged canonical thresholds. |
