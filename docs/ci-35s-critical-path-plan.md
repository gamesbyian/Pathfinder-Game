...
| B1c hint-occurrence import boundary | **measured green / implementation ready** | #2078 evidence: synthetic contract **15.8 s → 0.1 s**; closeout canary reran full-corpus occurrence acceptance successfully. |
...
Post-hint-consolidation Node/CLI profiling exposed `test:hint-occurrence-acceptance` at **15.8 s** even though its test body uses only tiny synthetic records.

Root cause: the test imported one pure helper from a CLI module that also executed the full three-corpus acceptance audit at module load.

Measured fix:

- pure occurrence semantics live in a side-effect-free library;
- CLI imports/re-exports the helper and retains direct corpus-scale behavior;
- synthetic Node contract imports the pure library;
- ordinary CI: **15.8 s → 0.1 s** for the contract;
- hint closeout canary independently reran the real full-corpus occurrence acceptance CLI successfully.
