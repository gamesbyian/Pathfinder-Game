...
| D1 two-way Node sharding | **post-hermetic rerun in progress** | #2088 timing was viable (**29 s** first-shard-start → both complete) but exposed a tracked-hint mutation race. #2091 is merged and removes that shared state. This rerun uses the same current 204-contract **49.6/49.7 child-s** partition. |
