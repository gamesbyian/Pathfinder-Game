...
| C exact dependency-tree restore | **seed run green; hit-side proof in progress** | #2068 run 35965107782: fast miss install **8 s** + save **3 s**; deep miss install **9 s**; full contract green. Documentation-only follow-up now tests shared exact-tree restores in both lanes with `npm ci` skipped. |
