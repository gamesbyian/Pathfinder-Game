...
Follow-up implementation now lives in the combiner-topology PR. `combine-solver-sweep-reports.mjs` exports `combineSolverSweepReports(argv, { root })` while retaining the same direct CLI guard. The Node contract keeps one real CLI smoke for executable/argument/file-output integration, but routes the remaining **13 combiner semantic cases in-process** through the exported seam. The **8 planner subprocesses remain real CLI executions** in this first cut because later cases intentionally inspect planner/recovery stdout and exit behavior. Decision gate: keep the seam only if semantic CI stays green and the measured combiner contract or total Node wall improves; planner extraction is a separate second step, not assumed.
