...
1. **Crossing-slack instrumentation.** Shadow-log `crossingSlack = freeInt - forcedFutureNeighbourRevisits` on oracle-labelled branches and known-solution prefixes, controlled by depth and remaining must-cross count. Treat it as a diagnostic representation first, not a score.
