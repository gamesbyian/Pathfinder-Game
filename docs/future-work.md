...
### 4. Main-loop late-reserve full population A/B — RUN, CONFOUNDED, promoted anyway pending a direct sweep (2026-08-12)

**Status: promoted to production default-ON at fraction 0.15, but the A/B evidence behind it was found confounded after the fact. A single full corpus-1+corpus-2 sweep with everything correctly default-on is the pending follow-up.**

The frozen full-population level-blind A/B ran (all 4 arms `workers=1`, `deterministic=true`, full 1700/1700 C2 + 102/102 C1 coverage confirmed each arm): Corpus-2 solved control 617 → 0.05: 687 → 0.10: 692 → 0.15: 694, Corpus-1 saturating at 94/102 from 0.05 onward, aggregate nodes/work decreasing monotonically across all four arms. This initially looked like a clean win, but the control-vs-treatment comparison was later found confounded: the control arm's blank `enable_flags` left `ablation=null` (keeping `PRUNE_MC_NEIGHBOR_BUDGET` OFF under its then-unfixed opt-in read site) while every treatment arm's non-null ablation object read `PRUNE_MC_NEIGHBOR_BUDGET` as ON via the Proxy default-fallback — mixing a large share of that flag's own already-known +54 Corpus-2 effect into the 617-vs-694 gap. The unconfounded 687→692→694 treatment-vs-treatment trend still supports a real, if smaller, effect. Full mechanism and decision rationale: [`../reports/2026-08-12-main-loop-late-reserve-population-ab.md`](../reports/2026-08-12-main-loop-late-reserve-population-ab.md).

`scripts/ablation-config.mjs` no longer lists `STRATEGY_MAIN_LOOP_LATE_RESERVE` in `OPT_IN_FEATURES`; `MAIN_LOOP_LATE_RESERVE_FRACTION` in `modules/solver/orchestration.ts` is `0.15`, kept as the best current evidence. The mechanism remains a strict no-op without a finite `nodeBudget`, so this only affects offline batch tooling, not interactive Play/Editor/Review solves. **Pending**: dispatch a single full corpus-1+corpus-2 sweep with a genuinely blank `enable_flags` (both flags' read sites are now fixed, so this correctly gives both their real production-default state) to directly observe the achieved solved count under the fully-promoted configuration.
