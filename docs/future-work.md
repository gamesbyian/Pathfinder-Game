...
### must-cross-heavy diverse-beam gaps blocked on reserve-slot budget

2026-08-22 routing-gap mining (docs/solver-optimization-current-queue.md Priority 7) found two remaining must-cross-heavy census gaps needing a diverse WIDE beam (`beam:objectiveFirst@beam5000(diverse)` for R02299 via the must-pass-heavy rule; `beam:intersectionHarvest@beam5000(diverse)` for R02159 via the default catch-all) that neither rule offers at all — not gated by `POLICY.HIGHINT_MC_DIVERSE`/`mcDiverseThread` (that helper isn't used by either rule), just genuinely absent. Not fixed in the same pass as the sibling perimeter-direction gaps on these same two rules: both rules' `MAIN_LOOP_LATE_RESERVE_CONFIG_COUNT`-4 trailing reserve slots are already fully occupied by that just-added, already-validated perimeter fix (R02131/R02515/R02788 confirmed solved at 50M production budget locally), and appending a 5th trailing config would push one of those out of the protected-reserve window rather than add cleanly. Reopen with either evidence that the rule has room for a 5th protected slot (e.g. a `MAIN_LOOP_LATE_RESERVE_CONFIG_COUNT` increase, tested for wall-time cost the way the original reserve-size choice was) or a rule restructuring that doesn't just append.
