...
### 1b. Investigate worker-count solve-outcome sensitivity — MOSTLY RESOLVED (2026-08-12)

**Status: the corpus-scale gap that escalated this is resolved and was NOT a worker-count effect. A separate, genuine worker-count/contention bug was found and fixed in the same investigation. One narrow single-level question remains open.** See [`../reports/2026-08-12-worker-count-sensitivity-repair-probe-wallclock.md`](../reports/2026-08-12-worker-count-sensitivity-repair-probe-wallclock.md)'s "Corpus-scale directionality, resolved" and "Still open" sections (the originating report, [`../reports/2026-08-12-worker-count-solve-outcome-sensitivity.md`](../reports/2026-08-12-worker-count-solve-outcome-sensitivity.md), now carries a pointer to this resolution rather than restating it).
...
**Still open**: (1) why `R02823` solved once for the originating report's author and never reproduced across five local configurations here — points to execution-context (Node version/CPU architecture) rather than worker count per se, not identified; (2) whether worker count has ANY measurable effect on corpus-scale solved-count once the ablation-flag confound is controlled for (same commit, same explicit `--enable-flags`/`--disable-flags` on both arms, workers as the only declared difference) — a fresh, well-posed, not-yet-attempted question, distinct from the original "root cause unknown" framing. Neither blocks other work; downgraded from ESCALATED since the corpus-scale directional claim that drove the escalation no longer holds.
