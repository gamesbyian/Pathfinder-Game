...
**Decision-bearing next action**: none required — the CP-SAT question itself is answered well enough to stop iterating on it for now (the answer is "it depends," which is itself the finding). If repair-search improvement work resumes, the concrete, not-yet-tried idea this surfaces is gating the existing `closeLengthGap`/`enableElitePrefixDfs` bounded-backtrack mechanisms on "small demonstrated rollback" specifically (a cheap, already-computed signal) rather than applying them indiscriminately — those mechanisms were found net-negative/marginal at population scale when applied broadly, but were never tested restricted to the population this broadening suggests is actually favorable. That is a new mechanism proposal, not something evidenced here.
