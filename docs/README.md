...
| 5 | Add static typing gradually | **In progress (per ADR 0009)** — check-only `// @ts-check` + `tsc --noEmit` gate (`check:types`) in the `check` CI group, strict over a curated allowlist of **25 modules**: the whole pure domain rule layer (incl. `isValidMove` + `path-validator`), the runtime command/effect layer + `game-rules`, and most of the solver — primitives, the hot core (`search-state`), pruning (`topology`/`lower-bounds`), the move scorer (`scoring`), the policy config (`policy`, validated against the scorer contract), the attempt-config ordering (`attempts`), the DFS/beam search driver (`search`), and the per-level `PrepLevel` builder (`prep`) — with shared `NormalizedLevel`/`MoveState`/`SolverSearchState`/`PrepLevel`/`UndoToken`/`ScoringProfile`/`StructuralTemplate`/`AttemptConfig` typedefs. No build step. Remaining (documented in `typing.md`): solver `orchestration`/`trap-search`/`normalization`; state/persistence layers (import-graph permitting); runtime boundary validation. |
