...
| [`level-id-unification-plan.md`](level-id-unification-plan.md) | Proposed: give every level in every corpus a permanent id independent of array position, closing the real risk that reordering the published corpus today would silently corrupt local hint-file join keys — current-state audit, proposed design, sequencing recommendation, and open questions needing a maintainer decision before implementation starts |
