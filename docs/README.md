...
| [`solver-shadow-eval-harness.md`](solver-shadow-eval-harness.md) | Shared shadow-mode evaluation harness for candidate reasoners: probe contract, telemetry, residual decomposition, and CP-SAT-labelled branch atlas. The atlas was subsequently expanded by the full sweep to 5,518 labelled branches across the eligible population; the separator-resource, obligation-tour-mutex, and goal-approach-envelope probes have all been rescored at scale and closed as sound but too weak/sparse for production integration. Use this doc for the measured results, not the original 623-branch prototype snapshot. |
...
| [`repair-search-stagnation-escape-plan.md`](repair-search-stagnation-escape-plan.md) | Active investigation record: Stages 1–3 and turn-aware bias were prototyped and measured; exact relinking was rejected, the surviving soft prototypes remain default-off, and Stage 4 was re-scoped toward an extend/detour operator. The next evidence step is descent-aware shadow probing, not another constant-tuning pass. |
