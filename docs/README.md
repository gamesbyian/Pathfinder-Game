...
| 6 | Rationalize tests into tiers | **Largely done** — `ci` grouped into `check`/`test:core`/`test:app`/`test:solver`; `ci:full` (PR gate + browser e2e); visual harness; full script→tier map + per-script triggers in `testing.md`. **Shared test-lib landed** (`scripts/test-lib/`: a register/run `harness.mjs` + `fixtures.mjs` with `makeRawLevel`/`createFakeScheduler`, self-tested by `test:test-lib`); 25 suites migrated off their hand-rolled pass/fail boilerplate across domain/state/solver/app/persistence/UI. Remaining (optional): migrate the ~9 bespoke-harness suites; coverage reporting; emulator-backed rule tests. |
