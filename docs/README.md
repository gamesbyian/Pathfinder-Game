...
| 5 | Add static typing gradually | **In progress (per ADR 0009)** — check-only `// @ts-check` + `tsc --noEmit` gate (`check:types`) in the `check` CI group, strict over a curated allowlist of **17 modules**: the whole pure domain rule layer (incl. `isValidMove` + `path-validator`), the runtime command/effect layer + `game-rules`, and solver primitives (`encoding`/`distance`/`archetype`/`solution`), with shared `NormalizedLevel`/`MoveState`/`SolverSearchState` typedefs. No build step. Remaining (documented in `typing.md`): grow `SolverSearchState` for the large solver search modules; the state/persistence layers (bottom-up, import-graph permitting); runtime boundary validation. |
