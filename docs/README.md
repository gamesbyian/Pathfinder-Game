...
| [`repair-search-nogood-cache-plan.md`](repair-search-nogood-cache-plan.md) | **Proposed, not started.** CDCL-inspired nogood/dead-state cache for `repair-search.ts`'s stagnation plateau — a cheap Stage 0 premise check (does repair-search's own restart loop actually revisit exact dead states often enough to matter?) gating a staged Stage 1 build (incremental Zobrist-style hashing, scoped to repair-search only, with an explicit soundness/differential-testing plan) |
