...
| [`solver-optimization-current-queue.md`](solver-optimization-current-queue.md) | **Start here for solver tuning.** Canonical ranked queue for optimizing existing techniques: lifecycle-informed allocation, beam retention, family boundaries, deep repair editing, state-conditioned must-cross anchoring, mechanics-conditioned routing, required gates, and closed forms not to repeat. |
...
| [`future-work.md`](future-work.md) | Detailed solver evidence and disposition record through 2026-08-15, including capability/lifecycle results, experiment history, compatibility anchors, and the closed/do-not-repeat list. Use `solver-optimization-current-queue.md` for current ranked priorities. |
