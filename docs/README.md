...
| 2 | Make engine state transitions explicit | **Partial** — state-action boundary + per-slice ownership/derived typedefs done; derived-nav invariant test added (`test:path-state-invariants`); undo restoration extracted from `engine.js` into the unit-tested `PathNavigator.applySnapshot`. Remaining: command/effect transitions for the other correctness-sensitive flows (reset, win, hazard, level-load). |
