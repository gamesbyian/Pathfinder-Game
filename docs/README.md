...
| 3 | Real UI/component layer | **Partial** — focus-trap, dialog semantics, boot-time DOM builders (sprite/palette/close-icon/guide-cards), pixel-stable modal/overlay component classes, documented static-shell contract (`ui-accessibility.md`). Remaining: fuller primitive set; shrink `index.html` further. |
