...
| 5 | Add static typing gradually | **In progress (per ADR 0009)** — check-only `// @ts-check` + `tsc --noEmit` gate (`check:types`) in the `check` CI group, strict over a curated allowlist of **29 modules**: the whole pure domain rule layer (incl. `isValidMove` + `path-validator` + `landmark-rules`), the runtime command/effect layer + `game-rules`, and the **entire `modules/solver/` directory** — primitives, the hot core (`search-state`), pruning (`topology`/`lower-bounds`), the move scorer (`scoring`), the policy config (`policy`), the attempt-config ordering (`attempts`), the DFS/beam driver (`search`), the per-level `PrepLevel` builder (`prep`), the `solveLevelV2` driver (`orchestration`), the trap-spot search (`trap-search`), and the raw→`NormalizedLevel` builder (`normalization`) — with shared `NormalizedLevel`/`MoveState`/`SolverSearchState`/`PrepLevel`/`UndoToken`/`ScoringProfile`/`StructuralTemplate`/`AttemptConfig` typedefs. No build step. The pure `domain`/`runtime`/`solver` layers are essentially complete; remaining (documented in `typing.md`): state slices, persistence DTOs, runtime boundary validation. |
