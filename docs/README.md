...
| 5 | Add static typing gradually | **In progress (per ADR 0009)** — check-only `// @ts-check` + `tsc --noEmit` gate (`check:types`) in the `check` CI group, strict over a curated allowlist of **20 modules**: the whole pure domain rule layer (incl. `isValidMove` + `path-validator`), the runtime command/effect layer + `game-rules`, and a large solver chunk — primitives (`encoding`/`distance`/`archetype`/`solution`), the hot core (`search-state`), and pruning (`topology`/`lower-bounds`) — with shared `NormalizedLevel`/`MoveState`/`SolverSearchState`/`PrepLevel`/`UndoToken` typedefs. No build step. Remaining (documented in `typing.md`): solver `scoring`/`prep`/`orchestration` (config/builder-heavy); state/persistence layers (bottom-up, import-graph permitting); runtime boundary validation. |
