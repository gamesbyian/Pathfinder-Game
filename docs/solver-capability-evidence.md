...
## What capability evidence is not

Capability evidence is **not**:
...
```bash
node scripts/analyze-solver-capability-evidence.mjs \
  --manifest=tmp/capability-evidence-manifest.json \
  --out=tmp/capability-evidence.json \
  --summary-out=tmp/capability-evidence.md
```
...
Capability evidence feeds existing owners rather than becoming a new workstream.
...
8. If aggregate capability-evidence counts are published, are they generated from a reproducible manifest/result with exact source identities, and do basic set invariants hold?

Most experiments will still close with no further machinery. Capability evidence exists to prevent the scientifically interesting exceptions from becoming tombstones.
