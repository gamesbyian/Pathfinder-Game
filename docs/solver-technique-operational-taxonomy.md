...
This audit found and corrected one concrete example: `modules/solver/ablation-config.ts` described `PROFILE_default` as "all weights = 1.0" even though current `POLICY_PROFILES.default` is a non-uniform tuned vector. The registry now describes all named profiles explicitly as scoring profiles and avoids implying bespoke procedural algorithms where none exist.
