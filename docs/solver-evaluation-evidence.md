...
v0.1 has an intentionally narrower mechanic/topology envelope; the current machine-owned support set is `scripts/stress/topology-generation-support-lib.mjs`, and generated topology corpora carry that same envelope. Therefore
...
| Candidate's activation/routing rule requires a mechanic the producer support envelope marks unsupported | **Do not use a null/non-participating topology run as evidence against the candidate.** Either choose another independent source that represents the mechanic or expand the generator first. |
...
Success-selected Hint history now has three deliberately different evidence grains. Persisted Hint provenance owns the accepted path plus semantic discovery event and, for modern acquisitions, bounded execution identity and physical occurrence lineage. `hint-discovery-process` binds only exact complete stored paths to solved result rows and can recover the failed attempts preceding the winner under the canonical source-run/protocol envelope. `hint-ingestion-receipts` records cross-harvester success-ingestion accounting, including already-represented observations and exact path/event/occurrence additions where the producer can measure them; the older `hint-harvest-selection-provenance` resource remains a level-blind-harvester-specific compatibility view. None of these success-selected resources substitutes for the originating attempted population.
