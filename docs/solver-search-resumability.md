...
> **2026-09-03: rung 3 (shared beam frontier among multiple beam policies), simplest 2-policy cyclic-alternation form, is concluded-negative.** Generalizing rung 2's single switch into a repeating `[A, B, A, B, ...]` schedule (same segment size/profiles/width/population as rung 2, for direct comparability) solved exactly the same 4/60 sampled levels as a single one-time switch — 0 net wins in either direction, despite the two schedules' search trajectories genuinely diverging (45% of levels exhaust at a different segment count). One handoff already captures whatever complementarity this profile pair offers; further alternation adds nothing measurable. A follow-up segment-size sweep (5,000–60,000-work segments, a 12x range) found a small suggestive edge for alternating at the finer sizes, but it did NOT replicate on an independent confirmation sample at the same size (0/0 net wins) — net across all three `segment=5,000` samples (90 levels) is 2 alternating-only wins to 0 switch-only wins, noise-level. See [`2026-09-03-beam-alternating-policy-schedule-pilot-001.md`](../reports/2026-09-03-beam-alternating-policy-schedule-pilot-001.md). This closes cyclic alternation across a 12x segment-size range, not rung 3 as a whole — a staged (broad-then-specialist, non-cyclic) schedule is the most informative untested candidate if rung 3 is revisited; rung 4 (below) is a materially different mechanism, not gated on this result.
...
3. **attempted, concluded-negative for its simplest (2-policy cyclic-alternation) form (2026-09-03)** — shared beam frontier among multiple beam policies. See the status-block addendum above; a staged or segment-size-swept form remains untested, not closed.
