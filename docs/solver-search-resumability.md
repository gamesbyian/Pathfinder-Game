...
> **2026-09-03: rung 3 (shared beam frontier among multiple beam policies), simplest 2-policy cyclic-alternation form, is concluded-negative.** Generalizing rung 2's single switch into a repeating `[A, B, A, B, ...]` schedule (same segment size/profiles/width/population as rung 2, for direct comparability) solved exactly the same 4/60 sampled levels as a single one-time switch — 0 net wins in either direction, despite the two schedules' search trajectories genuinely diverging (45% of levels exhaust at a different segment count). One handoff already captures whatever complementarity this profile pair offers; further alternation adds nothing measurable. See [`2026-09-03-beam-alternating-policy-schedule-pilot-001.md`](../reports/2026-09-03-beam-alternating-policy-schedule-pilot-001.md). This closes the cyclic-alternation form specifically, not rung 3 as a whole — a staged (broad-then-specialist, non-cyclic) schedule or a segment-size sweep are the two most informative untested candidates if rung 3 is revisited; rung 4 (below) is a materially different mechanism, not gated on this result.
