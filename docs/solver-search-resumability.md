...
> **2026-09-03: rung 2 (same beam frontier, changed beam policy: fixed-work complementarity test) is concluded-positive at small/narrow scope.** A further opt-in `captureContinuationOnBudgetExit` param lets a pause land at `prep._workCap`'s own exact work ceiling (rung 2's currency is work, not phase count) — but only reliably fires at `beamWidth <= 256`, because a separate mid-phase check (every 256 frontier nodes) otherwise always wins the race first; see the report for the full mechanism and a real corpus2 calibration. Two independent 30-level uniform corpus2 samples (60 levels) found `intersectionHarvest`→`objectiveFirst` frontier inheritance solves 2/60 levels that neither policy alone nor a fresh-restart switch could solve at the same total work, and 0/60 the other direction — real, one-directional signal, but rare (~3%) and not yet cross-generator-confirmed. See [`2026-09-03-beam-policy-switch-complementarity-pilot-001.md`](../reports/2026-09-03-beam-policy-switch-complementarity-pilot-001.md).
> **2026-09-03: rung 3 (shared beam frontier among multiple beam policies), simplest 2-policy cyclic-alternation form, is concluded-negative.** Generalizing rung 2's single switch into a repeating `[A, B, A, B, ...]` schedule (same segment size/profiles/width/population as rung 2, for direct comparability) solved exactly the same 4/60 sampled levels as a single one-time switch — 0 net wins in either direction, despite the two schedules' search trajectories genuinely diverging (45% of levels exhaust at a different segment count). One handoff already captures whatever complementarity this profile pair offers; further alternation adds nothing measurable. A follow-up segment-size sweep (5,000–60,000-work segments, a 12x range) found a small suggestive edge for alternating at the finer sizes, but it did NOT replicate on an independent confirmation sample at the same size (0/0 net wins) — net across all three `segment=5,000` samples (90 levels) is 2 alternating-only wins to 0 switch-only wins, noise-level. See [`2026-09-03-beam-alternating-policy-schedule-pilot-001.md`](../reports/2026-09-03-beam-alternating-policy-schedule-pilot-001.md). This closes cyclic alternation across a 12x segment-size range, not rung 3 as a whole on its own — but a follow-up staged, non-cyclic three-policy pilot (`intersectionHarvest`→`objectiveFirst`→`perimeterSweep`, one switch each, never returning to an earlier policy) also concluded negative: three-stage solved exactly the same 4/60 levels as rung 2's own two-stage switch, 0 net wins either direction. See [`2026-09-03-beam-staged-three-policy-pilot-001.md`](../reports/2026-09-03-beam-staged-three-policy-pilot-001.md). Both tested rung-3 schedule shapes (cyclic and staged) are now closed negative for this profile family — a single handoff already captures all its measurable complementarity value. Rung 4 (below) is a materially different mechanism, not gated on this result; reopening rung 3 again needs a structurally different premise (e.g. a profile pair differing in retention/ordering-bias, not scoring-weight magnitude), not another schedule-shape variant of this same profile family.
> **2026-09-03: rung 4 (bounded beam → DFS handoff from selected frontier states), simplest single-inherited-state form, is concluded-negative.** Beam and DFS already share one state representation (`SolverSearchState`), so handing DFS beam's exact live residual state costs zero extra work — but on the same two 30-level populations, the handoff solved only 1/60 levels versus 4/60 for DFS started fresh from the gate (with 3 clean losses — levels fresh-gate DFS solves that the inherited-state handoff does not — and the handoff's one solve confounded by beam alone also solving it). Cross-method handoff here has a compatible state *representation* but an evidently incompatible search *shape*: a position beam's width-culled breadth search considers good enough may be a poor commitment for DFS's single continuous depth-first descent. `undoMove` was exposed via `SOLVER_TESTING_API` (small, safe, additive) so this external DFS reimplementation could replicate `dfsFromGate`'s backtracking without touching the production DFS loop; plain best-first DFS could not solve anything on this population even at 20,000,000 work, so the pilot's DFS needed its own minimal LDS wrapper before results were trustworthy — see [`2026-09-03-beam-to-dfs-handoff-pilot-001.md`](../reports/2026-09-03-beam-to-dfs-handoff-pilot-001.md) for the full mechanism, two work-accounting bugs found and fixed during calibration, and the complete result. This closes single-state, no-selection beam→DFS handoff for this exact profile pair — a genuine selection strategy (preferring a residual state that looks DFS-friendly over an arbitrary one) remains untested, not closed.
...
3. **attempted, concluded-negative across both tested schedule shapes (cyclic and staged, 2026-09-03)** — shared beam frontier among multiple beam policies, for the `intersectionHarvest`/`objectiveFirst`/`perimeterSweep` profile family. See the status-block addenda above; a structurally different profile family (retention/ordering-bias, not scoring-weight magnitude) is the next legitimate premise, not another schedule-shape variant of this family.
4. **attempted, concluded-negative for its simplest (single inherited state, no selection) form (2026-09-03)** — bounded beam → DFS handoff from selected frontier states. See the status-block addendum above; a genuine selection strategy (preferring a DFS-friendly residual state over an arbitrary one) remains untested, not closed.
