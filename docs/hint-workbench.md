...
| `candidate-grid` | Forced-first-neighbor (every real graph neighbor of the gate, not just the 4 cardinal directions `ablation-full`'s cascade forces) x strategy-ablation-flag grid, plus corner-flip mutation of a sampled subset of existing hints (`--seeds`). Ported from the standalone `hint-candidate-search.mjs`, but wall-clock-bounded (`--wall-ms`) so it always persists partial progress instead of losing an interrupted run. | You want candidate-search's finer-grained per-neighbor forcing technique, or corner-flip local mutation of existing hints, integrated with the shared acceptance/write pipeline. |
