...
4. **Execution-family canary:** before large matrices or many shards, run the smallest representative cell/level for each materially different execution family under the exact cap/flags/selector mode. Verify work/node/deadline stop semantics and expected output fields before scaling. **Mechanized (2026-09-09) in `solver-level-blind-targeted-sweep.yml`**: an "Execution-family canary" step runs `level-blind-capability-sweep.mjs` against exactly one representative level (the first resolved id) under the run's fully-resolved corpus/budget/flag config, then `scripts/verify-canary-cell.mjs` asserts no crash/error, nonzero attempts (catches a flag combination that eliminates every attempt config before any solve is tried), well-formed output fields, and — since this workflow's own `budget_ms` default is intentionally non-binding — that the wall-clock deadline never fires, all *before* the shard planner or the fan-out runs. Deliberately does NOT check target-stage participation rate: a single level can legitimately not reach every ladder stage even under a correctly-wired config, so that stays `validate-solver-sweep-integrity.mjs`'s job on the full population (gate #7's schema/feature assertions and this workflow's own `min_participating_levels`/`min_participation_rate` inputs). No other workflow has this wired yet; a workflow gaining a comparable single-resolved-config-before-fan-out shape should follow the same pattern (see `scripts/verify-canary-cell.mjs`'s own header comment) rather than inventing a bespoke check.
