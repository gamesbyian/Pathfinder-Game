...
28. **Stop required acquisition when a frozen decision is irreversibly locked.** For prospectively declared monotone gates, continuing rows need not delay the decision once every possible completion yields the same disposition. Example: under a frozen zero-loss promotion gate, the first genuine independent-unit loss permanently blocks promotion. Use existing completion-order cancellation where practical; record the lock reason, observed independent support, remaining units, elapsed wall and in-flight work. Keep `coverageComplete` / `decisionValidComplete` false for an early-stopped population and claim only the narrow locked disposition, not a complete effect estimate. Optional characterization may continue only under a separately stated reason. Do not derive a stop rule retrospectively from the observed sequence. See [`2026-09-20-solver-research-batch-decision-latency-audit-001.md`](../reports/2026-09-20-solver-research-batch-decision-latency-audit-001.md).
