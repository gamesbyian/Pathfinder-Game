...
28. **Stop required acquisition when a frozen decision is irreversibly locked.** For prospectively declared monotone gates, stop once every unseen completion yields the same disposition. Use completion-order cancellation where practical; retain the lock reason, independent support, remaining units, elapsed wall and in-flight work. Early-stopped populations keep `coverageComplete` / `decisionValidComplete` false and support only the locked disposition; optional characterization needs a separate reason. Never derive the rule retrospectively. See [the decision-latency audit](../reports/2026-09-20-solver-research-batch-decision-latency-audit-001.md).
