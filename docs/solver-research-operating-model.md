...
- `research-evaluation-evidence-role-lib.mjs` — canonical development/confirmation/transfer roles for generated/research-block evaluation evidence; broader report roles remain separate.
...
22. **Treat declared rival sets as decision-bounded unless exhaustiveness is separately established.** A discriminator can separate the named alternatives without proving that the named alternatives span the explanation space. Record omitted/unknown alternatives when they can change the decision, and do not turn `liveRivals` into an ontology-completeness claim by implication.
23. **Freeze decision-bearing quantitative design with the scientific contract.** Thresholds, caps, sample sizes, action/config identities, and outcome cut points that determine interpretation belong in the same frozen machine artifact as the precommitted question/analysis contract. Human prose may explain them; CLI overrides must not silently create a different experiment or reducer.
24. **Calibrate measurement reactivity when instrumentation touches the measured path.** Measurement support is not enough if telemetry changes search order, allocation, work, timing, memory pressure, or participation. Use parity plus bounded overhead/volume evidence appropriate to the instrument; otherwise scope the instrument as research-only and treat production-equivalence claims as unsupported.
25. **Run a closure pass after the first productive abstraction.** Before declaring an audit lens saturated, ask what can go stale, what other authorities must transition with it, which alternatives were omitted, whether observation perturbs behavior, who owns decision-bearing numbers, what target the evidence actually covers, what additional fact is only available through composition with another resource, how earlier results adaptively selected the current design, what downstream state notices invalidation, and whether tests assert the semantic fact rather than a proxy. See `reports/2026-09-20-research-session-local-optima-retrospective-001.md`.
