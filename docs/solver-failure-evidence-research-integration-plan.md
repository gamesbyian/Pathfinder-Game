...
- **Phase 4A — zero-compute freeze ready / acquisition-blocked:** current Class-3 membership + exact rescuer identities are mechanically derivable from committed boundary/census assets, and downstream exact-rescuer classification is implemented. Remaining scientific input is shared-production exact-action dose under [the Class-3 preflight](../reports/2026-09-19-class3-exact-action-dose-acquisition-preflight-001.md).
