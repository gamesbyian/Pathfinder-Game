...
- **W1 first candidate:** [development result 002](../reports/2026-09-21-connectivity-cut-certificate-shadow-development-result-002.md) found 650/15,019 confirmed scheduled-call hits on 24 hard C2 parents, 612 across different exact states, 0 false positives, and hits on 9 parents. Reusable implication-level proof is therefore a real reservoir. The naive 64-entry linear consumer is closed: 806,088 certificate scans + 311,919 boundary checks for a best-case 7,800 canonical work units (~0.064% of total work) replaced. The earned next shadow now deduplicates exact proof templates and measures current-position candidate fanout before any behavioral consumer.
...
The next decision-bearing connectivity evidence is selector economics, not recurrence: quantify exact proof-template duplication, hit concentration, boundary sizes and current-position fanout under the deduplicated shadow. Close the proof family if no cheap selector emerges; only then consider unscheduled/earlier rejection. W2's paired-beam proof-overlap probe and W3's BC1 later-disposition microscope remain independent next measurements.
