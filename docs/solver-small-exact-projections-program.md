...
The production-inert **beam later-disposition shadow** nominated by [BC1 removable-work economics seam audit 001](../reports/2026-09-21-bc1-removable-work-economics-seam-audit-001.md) is built (`modules/solver/bc1-shadow-disposition.ts`, hooked at `modules/solver/search.ts`'s ordinary-gauntlet-survivor seam) and has run against a live, exhaustive, 24-parent population (the same frozen Stage-B sample). [Pilot result 001](../reports/2026-09-26-bc1-beam-later-disposition-shadow-pilot-result-001.md): zero production-inertness violations, zero solution-safety alarms, 100% parent recurrence (up from the 22/24 sampled screen), full disposition closure (0% survive the observation window), and excellent per-catch construction economics (12 work units to expose a median 2,800-17,600 units of later work). Aggregate per-check economics are only qualified: an unconditional-per-candidate shadow costs roughly 70% of the entire campaign's own canonical work, dominated by redundant `isConnected()` recomputation the ordinary gauntlet may have already paid for.

**Next gate:** reuse the ordinary gauntlet's already-computed connectivity result (when available that phase) instead of unconditionally re-running `isConnected()` inside `computeBc1ShadowConflicts`, re-measure aggregate shadow cost against dominated work, then implement the smallest BC1 consumer and test at matched work on a disjoint population. Do not add BC1 to the hot path or generalize to k-cut/flow before that behavioral test.
