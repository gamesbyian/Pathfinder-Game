...
- `externalModelSupport`: `exact` means the current maintained external model can express the solver/game semantics needed for the mechanic when its stated supporting constraints are enabled; `relaxed` needs extra state/constraints; `unsupported` has no current exact encoding. This is an **encoding-capability** field, not a claim that every mechanic combination has completed bidirectional validation; validation confidence is tracked in [`../reports/2026-08-23-solver-reference-model-capability-audit.md`](../reports/2026-08-23-solver-reference-model-capability-audit.md).
...
| **Regular filter** | static | n/a | n/a | yes, precompiled | yes, static | no | no | unsupported in current full CP-SAT; deliberately skipped until there is a validation population/question |
| **Flipping filter** | global crossing parity + per-object used bit | published max 22; stress max 8; schema ≤32 | used bits yes; parity derived | yes: legal axis depends on global flipper order | yes, dynamic | no | yes | exact in current full CP-SAT via global order/parity encoding; validation tracked separately |
...
| **Must-turn** | per-object satisfied bit | schema ≤30 | yes | hard legality no; `mustTurnDeadlocked` is prune-only | no | yes | yes | exact in current full CP-SAT via turn/chirality variables; validation depth tracked separately |
| **Adjacent-turn** | per-object satisfied bit | schema ≤30 | yes | no | no | yes | yes | exact in current full CP-SAT via turn chirality + OR across neighbors; validation depth tracked separately |
...
- Regular filters are static/exact in native Pathfinder semantics but are intentionally **unsupported by `cpsat-full-probe.py` today** because neither stress corpus supplies a validation population. Do not infer external-model exactness merely because the native mechanic itself is static.
