...
- Flipper parity is `popcount(flipperUsedMask) % 2`, derived from the used mask; no separate parity state is needed. Flipper cardinality uses a different representation from the `(1 << n) - 1` initial masks below; do not reuse the ≤30 mask bound for it. That different representation had its own now-fixed analog of this bound bug: `search.ts`'s beam-search numeric coarse-state/mechanic-bucket keys derived their flipper radix as `1 << flipperCount`, unsound at the schema's real ≤32 flipper cap for the identical int32-shift reason (`1 << 31` negative, `1 << 32` wraps to `1 << 0`) — fixed 2026-09-02 to `2 ** flipperCount` with `flipperUsedMask` unsigned-normalized before composition; see `docs/solver-correctness-hardening.md`'s "Closed correctness defect: 31-32 flipping-filter beam identity".
