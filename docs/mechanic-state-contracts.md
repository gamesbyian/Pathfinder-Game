...
`(1 << n) - 1` where `n` is that mechanic's object count on the level. In JS, `1 << 32` wraps back
to `1` (the shift count is taken mod 32), so this expression silently produces a **wrong, non-
all-ones mask** once `n` reaches 32, and is already suspect at `n` in the high 20s depending on
what downstream code assumes about unused high bits. The comment on `flipperUsedMask` in
...
This is not a hypothetical: must-pass and must-cross are safe today only because a **documented**
design maximum (4 each, from `data/levels.json`'s own authored corpus and the in-envelope stratum's
caps) sits far under 31, and both stress-corpus generators respect it. **Surround, must-turn, and
adjacent-turn have no equivalent documented maximum anywhere** — nothing currently stops a
procedurally-generated or hand-authored level from placing, say, 40 surround landmarks, at which
point `initialSurroundMask` silently miscomputes and every downstream `surroundMask`-gated check
(win condition, `surroundLowerBound` pruning, scoring) operates on a wrong initial value. No
generator currently produces landmark-heavy levels at that scale — this is a **latent** gap, not a
reproduced bug — but it is exactly the shape of gap the beam-dedup incident already demonstrated
turns into a real one when a generator's cap changes without a corresponding cap check elsewhere.

**Recommendation, not yet done:** either (a) add an explicit assertion in `prep.ts` (or
`validateRawLevel`) that surround/must-turn/adjacent-turn counts stay ≤ 31 per level, matching the
same bound already implicit in must-pass/must-cross's bitmask encoding, or (b) if a level is ever
expected to legitimately need more than 31 of one landmark type, migrate that mechanic's mask to a
`bigint` or a typed-array-backed bitset before any generator is allowed to exceed 31. Either is a
small, targeted fix — flagged here rather than made, since no current corpus or generator triggers
it and CLAUDE.md's "measure before build" discipline argues against speculative hardening with zero
reproducing case.
