...
Stages 1-2 leave 2/8 credible frozen pairs. Stage 3 succeeds for both: matched isolated resolves on `R02687`/objectiveFirst and `R02094`/intersectionHarvest show real `plain` vs `mechanic-buckets` sibling-response flips across 4/5 existing family transform modes, with opposite net directions. Static object counts and simple portal-use timing do not mediate them. [`report`](../reports/2026-09-11-ws1-stage3-isolated-technique-resolve-001.md) Stage 4 (must-cross-order / portal-must-cross basin mediation) is now also closed negative for both pairs — non-separating for `R02094`, structurally inapplicable for `R02687` (zero must-cross cells in its whole family). The line is active WS1; its next earned step is bounded operational first divergence, which needs new small per-node divergence instrumentation that does not exist yet. [`result`](../reports/2026-09-12-ws1-stage4-solution-space-mediation-result-001.md)
