...
This is not a second queue. Carry forward only unexplained residue plus a concrete reopen condition. Closed work reopens only for a changed premise/treatment. Existing variants precede new generation when they can answer the question; human/editor parents fill source/contrast gaps.

For speed, current-head profiling must earn implementation. [`speed authority`](solver-architectural-speed-opportunities.md)
...
| Class-3 exact-action dose | Triage depends on comparable-work failure; atlas rows lack per-attempt `workSpent` |
...
Not current queue items. Reopen after profiling shows material cost and favorable expected value: neighbor/candidate allocation, replay/DFS transposition/cull selection, compact native/WASM kernels, or runtime-sensitive retests. Evaluate representative latency plus total CPU/work over the retained solve population. [`methodology`](../reports/2026-09-15-speed-negative-methodology-review-001.md)

## Recovered architectural descendants

Evidence-gated descendants, not independent queue items. [`reconciliation`](../reports/2026-09-16-recovered-solver-avenues-reconciliation-001.md) [`moonshots`](../reports/2026-09-16-assumption-breaking-solver-development-moonshots-001.md) [`post-merge`](../reports/2026-09-16-post-merge-moonshot-recovery-and-prioritization-001.md)

| Question | Reopen condition |
|---|---|
| Bidirectional / backward abstraction | H1 or another observer yields a compact sound residual/interface signature without recreating full path history |
| Learned conflicts / nogoods | H1/H3 or DEAD-core diagnosis finds recurring compact DEAD relations across unrelated parents |
| Bounded exact current-state production query | A small sound offline query answers an earned question; bound runtime and prove economics |
...
| Adversarial counterexample generation | A frozen hypothesis can be attacked by solver-belief vs exact-reality disagreement without construction leakage |
| Cooperative blackboard portfolio | At least two solver families expose reusable within-invocation artifacts with marginal value |
| Per-level search-plan compilation | Two different architectures show complementary legal niches predictable from current-input structure |
...
H1 optionality and CEGAR planning remain H1 descendants. Conflict-driven search follows only from compact recurring DEAD cores. Generic exact computation is not intrinsically illegal; historical labels/witnesses/identity are illegal runtime steering inputs.

## Capability-acquisition frontier

Post-Class-4-promotion boundary: **390 Class-5** misses with no known admissible/T1 candidate, **71 Class-4** historical-candidate misses; residual 531, Classes 1-5 = 17/30/23/71/390. [`refresh`](../reports/2026-09-16-post-promotion-production-boundary-refresh-001.md)

Closed neighbors: naive `swap`/`cs`, generic scorer tuning, six scalar future-feasibility summaries, connectivity/dead-cause reuse, August low-cardinality quotas, fixed-endpoint homotopy. Card-E sizing is resolved and tested state-selection descriptors closed negative after depth control.
...
|---:|---|---|
| 1 | **Controlled open-path topological completion contrast** | **PREFLIGHT DESIGNED.** Run the frozen same-board/same-endpoint human-parent topological-fork pilot; exact-label both prefixes and apply stop/advance rules. [`preflight`](solver-class5-controlled-topology-acquisition-preflight.md) |
| 2 | **H1 residual event-feasibility / completion regimes** | **PRESPEC FROZEN; strongest independent acquisition premise.** Test a tiny relational event vocabulary: individual/joint realizability and nominated obligation orders. No feature sweep. [`prespec`](../reports/2026-09-16-h1-event-feasibility-prespec-001.md) |
| 3 | **Minimal DEAD-core / minimum-relaxation diagnosis** | If 1/2 expose DEAD contrasts without a compact relation, relax a tiny semantic commitment set and seek recurring rescue/core families across independent parents |
| 4 | **H2 marginal information velocity / structural stasis** | Bounded observer; advance only if structural novelty per `workSpent` predicts marginal value beyond generic difficulty/work |
| 5 | **H3 dependency-defined repair commitment interface** | Bounded WS6 premise generator; require recurring coupled commitments smaller than rollback distance |
| 6 | **Behavioral-state quotient / residual-language probe** | Tiny exact-probe population first; advance only if future behavior admits a compact equivalence relation materially coarser than syntactic path history |
| 7 | **Separator/decomposition census** | Census small separators, interface width, mechanic leakage and obligation split; build no decomposing solver unless low-complexity interfaces are common |
...
| 9 | **Forced-chain traversal census** | Only after WS7 profiling finds material dynamic cost in post-prune one-successor chains |

Closed descendants remain history, not queue slots: August full-pool categorical projection was negative over 207 pools; Card-E confirmed 17/156 reconstructable-but-unexposed but found no descriptor family beating depth control. Partial-order/commuting replay remains nested under H1.

## Secondary solve-harvest trigger

Class 1/2/3 has no already-legal no-contention batch. Must-turn-biased late repair retains placement evidence on `R02768` + `R02180`, but its frozen 60-row 7M A/B is **CLOSED NEGATIVE**: identical 17/60 solved sets, zero marginal gains, +6.5% work. Reopen only for a different placement/selector premise or allocation contract. [`result`](../reports/2026-09-16-class2-must-turn-economics-result-001.md)
