...
Source-stratified hint/path mining, solution-basin complementarity, and a bounded forced-decision/backdoor-depth test for the active portal cohort are now promoted to WS1's existing-evidence tranche; see [`hint/provenance evidence-layer upgrade`](../reports/2026-09-09-hint-provenance-evidence-layer-upgrade-001.md). They are no longer generic future-work questions.
...
The broad structural-risk signal and several pairwise technique niches are already useful; extend them by evidence quality rather than by feature count. The execution-ready program is in [`2026-09-11-structural-technique-response-extension-audit-001.md`](../reports/2026-09-11-structural-technique-response-extension-audit-001.md): **temporal niche stability -> difficulty-stratified relative advantage -> existing-family controlled flips -> source-controlled solution-space mediator -> bounded operational first divergence**. Stop at the first stage that does not preserve a compact recurring distinction. The tested richer generic topology bundle remains closed as a selector extension; new static descriptors should be mechanism-nominated.

## Deferred questions

| Question | Reopen condition / boundary |
|---|---|
| **Variant-family capability robustness** | Current census evidence already supports temporal and budget-edge fragility by capability multiplicity/technique family, that multiplicity predicts real *production* success within each corpus separately (6.1% at solverCount=0 → 99.5% at 11+, fresh 1,700-level run), and that doubleton (`solverCount=2`) redundancy is disproportionately same-family (58.5%, no strong structural signature of its own) rather than genuinely cross-technique. Singleton exposure is concentrated by count in `repair`/`beam` (84.6%) despite DFS having the highest per-claim fragility rate. See [`2026-09-04-census-multiplicity-predicts-production-success-001.md`](../reports/2026-09-04-census-multiplicity-predicts-production-success-001.md), [`2026-09-05-multiplicity-production-success-robustness-by-corpus-001.md`](../reports/2026-09-05-multiplicity-production-success-robustness-by-corpus-001.md), [`2026-09-04-doubleton-intra-family-redundancy-001.md`](../reports/2026-09-04-doubleton-intra-family-redundancy-001.md), [`2026-09-05-doubleton-structural-signature-null-001.md`](../reports/2026-09-05-doubleton-structural-signature-null-001.md), [`2026-09-05-singleton-family-plurality-001.md`](../reports/2026-09-05-singleton-family-plurality-001.md). WS1 may mine already-persisted variant-parent replay provenance now. For structural-technique claims, query the existing family dataset only after temporal/difficulty-controlled screening nominates a pair; use whole parents and controlled response flips rather than bulk sibling counts. Do not rerun the answered temporal/budget-edge/production-success joins or generate variants first. |
| **Four-space triangulation** across input geometry, solution structure, technique response, and live trajectories | Reopen per nominated contrast, not as a broad campaign: require a temporally credible and difficulty-robust technique-response distinction first, then test existing-family flips and source-controlled basin/profile mediation before buying trajectory evidence. |
| **Mechanic-composition transfer / generator expansion** | The current topology generator omits portals, so it cannot test the active triple interaction. Expand only after a concrete intervention survives within-corpus gates and materially needs cross-generator transfer. |
| **Richer static graph/placement selectors** | The tested 18-feature bundle adds no material held-out value. Reopen only for a mechanism-specific descriptor derived from a demonstrated trace/solution distinction. |
| **Stability-aware portfolios** | Reopen only if the promoted WS1 basin-complementarity analysis finds a compact, held-out decision signal not captured by current work/coverage evidence. Basin diversity by itself remains an offline label, not a production selector. |
| **Latent response dimensions / biclusters** | Reopen only if simpler pair/cohort analyses leave stable unexplained response structure after temporal and generic-difficulty controls. |
...
| **Generator/editor-envelope technique niches** | Corpus1-vs-corpus2 ease, repertoire, regime composition, and structural risk are answered; the risk ranking replicated on natural holdouts, while the added topology bundle did not improve prediction. A true generator/editor field is still absent. Reopen only with a joinable finer envelope or an envelope-specific mechanism claim. See [`corpus comparison`](../reports/2026-09-04-corpus1-corpus2-stage-share-comparison-001.md), [`regime composition`](../reports/2026-09-05-routing-regime-composition-by-corpus-001.md), [`risk replication`](../reports/2026-09-04-production-structural-risk-factors-full-replication-001.md), and [`topology follow-up`](../reports/2026-09-08-cpsat-rescue-cohort-regeneration-and-selector-001.md). |
| **Minimal technique-niche counterexamples** | Use reducer + exact/reference validation when a temporally/difficulty-robust claimed niche needs causal isolation; retained exact paths should adjudicate known-live state loss rather than reopen broad CP-SAT acquisition. |
...
| **Queryable analytical layer** | Reopen only if recurring joins among run identity, telemetry, static features, families, labels, arms, and the new provenance evidence report still require bespoke scripts after existing join helpers are extended. Generated views must be rebuildable from canonical evidence and cannot become production-policy truth. |
