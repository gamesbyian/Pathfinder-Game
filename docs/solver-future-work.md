...
**Ladder complete for both frozen pairs; closed.** Stages 1-2 left 2/8 credible frozen pairs. Stage 3 succeeded for both: matched isolated resolves on `R02687`/objectiveFirst and `R02094`/intersectionHarvest show real `plain` vs `mechanic-buckets` sibling-response flips across 4/5 existing family transform modes, opposite net directions; static object counts and simple portal-use timing do not mediate them. [`report`](../reports/2026-09-11-ws1-stage3-isolated-technique-resolve-001.md) Stage 4 (must-cross-order / portal-must-cross basin mediation) closed negative for both — non-separating for `R02094`, structurally inapplicable for `R02687` (zero must-cross cells). [`result`](../reports/2026-09-12-ws1-stage4-solution-space-mediation-result-001.md) Stage 5 (bounded operational first divergence, no new search-code instrumentation needed — existing `mechanic-bucket-culled`/`post-mechanic-bucket-selection` telemetry already carries both configs' implied selection) found both pairs diverge at the same depth, immediately after a mechanic-progress event creates a minority retention bucket. [`result`](../reports/2026-09-12-ws1-stage5-first-divergence-result-001.md) n=2 does not license a selector. Reopen only for a new pair/premise, not a third variant of this question.
