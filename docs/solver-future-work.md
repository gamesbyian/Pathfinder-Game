...
Do not duplicate active/concluded lanes here. Current dispositions: A bounded-positive/representation-stopped; D1 positive/economics-observation-stopped; F3 qualified-positive/discordance-stopped; C/E and DEAD-core size-1 negative; Class-3 dose evidence-gap; G1 candidate-completion-limited. Multi-pick found two referee-verified LIVE siblings, so the old no-LIVE D/E block is obsolete; real production-search frontier construction now exists. G1's remaining prerequisite is narrower: convert frozen real-search partials into a fair complete relaxed candidate without answer leakage.
...
| Region/AND-OR decomposition engine | Retained A/D3/F3 reconciliation first nominates a bounded dynamic interface signature; then Lane A demonstrates that contract preserves relevant crossing/history/length/intersection/mechanic/topology behavior on a frozen legal-prefix population |
...
| Full complete-path LNS | Reuse frozen production-search frontier states, convert them into fair complete relaxed candidates without answer leakage, and show those candidates are close/local-repairable often enough to justify implementation |
...
| Question | Reopen condition |
|---|---|
| Automatic selector/profile calibration | A new legal current-input relation predicts differentiated response among genuinely different actions under supported evidence. Remaining length is a live one-population nomination only; require one prespecified independent shared-budget transfer before treating it as a selector gate |
