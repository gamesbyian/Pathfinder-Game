...
Do not serially hand-tune hard levels. Select the specimen only after the class-5 freshness audit; `R03229` is the current provisional candidate, not yet frozen. Reuse the existing full-detail known-solution-prefix collector, the specimen-agnostic `build-class5-microscope-cases.mjs` postprocessor, and the existing explicit-prefix CP-SAT seam. [`audit`](../reports/2026-09-12-class5-microscope-specimen-and-freshness-audit-001.md)
...
| **Minimal technique-niche counterexamples** | Use reducer + exact/reference validation when a robust niche needs causal isolation. The microscope is the preferred one-specimen form for extinction-boundary future-feasibility questions. |
