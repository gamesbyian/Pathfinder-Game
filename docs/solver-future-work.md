...
**New, unlanded, better-scoped lever found instead: `REPAIR_LATE_PROBE_NODE_BUDGET` (`stage-budget.ts`, currently 2,000,000) may simply be under-sized**, rather than the population being mis-gated. Since `STRATEGY_REPAIR_LATE_PROBE` already reaches every `needsRepairFallback`-excluded level unconditionally at the dead-last tier, raising its flat node cap costs nothing on any level that solves earlier (main loop still exits at first success) — architecturally lower-risk than gate-widening, which changes budget-sharing for every newly-gated level including ones that already solve fast. Tested via `SolveOpts.repairLateProbeNodeBudgetOverride` (no constant edit needed) at 5,000,000 (2.5x the shipped cap) against the same 13 confirmed `hi:medium-high-catchall` gaps: **1/13 newly solved** (`R02271`), the other 12 still node-budget-exhausted at 66-249s each — a small but real, directionally-consistent signal with the shipped tier's own recorded 8.3% (26/314) hit rate at the current 2M cap. Not enough evidence from one hand-picked 13-level sample to promote a corpus-wide constant change (this tier is reached by most of the corpus, not just this one archetype, so the cost side of the tradeoff is corpus-wide too). Concrete next step: a population-scale or large-random-sample A/B comparing the shipped 2M cap against a candidate raise (e.g. 5-10M), measuring both new solves and the added wall-clock tax on levels that stay unsolved either way. `solver-archetype-sample-ab.yml` is a poor fit (the affected population isn't archetype-scoped — it's most of the corpus outside the must-cross-heavy/very-high-reqint carve-out); `solver-stress-refresh.yml` (full population) or a new plain-uniform-random-sample dispatch is the right instrument. Not started.
