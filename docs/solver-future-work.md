...
The family-comparison rung is now executed for the current class-5 cohort and closed negative as a naive/undecoupled probe: whole-ladder rescue rates are confound-dominated (same general-difficulty confound the mechanic-composition transfer pilot diagnosed for portal relocation, now population-scale-confirmed for `swap`/`cs` edits generally). [`report`](../reports/2026-09-12-class5-family-reference-comparison-001.md) A future probe needs that pilot's own decoupled-control design.
...
When broad frontier work has established a recurring failure phenotype but cheap generic treatments no longer provide a new premise, a **single representative hard level may be used as a microscope**. The purpose is not to optimize that level; it is to expose one future-relevant distinction deeply enough to generate a generic mechanism.

Freeze the specimen and extinction boundary before treatment search. Compare one known-live state/prefix against a tiny set of states that displace/outlive it, use WS5 exact/reference adjudication to establish which candidates genuinely retain valid futures, and classify the failure (ranking, retention, state representation, pruning, reachability/operator, or commitment/restart) before implementing anything. Hint identity and level ID stay offline labels only. A specimen solve is discovery evidence, never promotion evidence; any mechanism must immediately face a small independent phenotype-matched check.

The first active microscope is `R03351`, whose ordinary beam keeps accepted-witness support to depth 47 while both tested bucket-retention variants fail to rescue it. [`preflight`](../reports/2026-09-12-class5-single-level-microscope-r03351-preflight-001.md) This method may be reused on another specimen only after the current one produces either a concrete premise or an explicit non-diagnostic closeout; do not serially hand-tune hard levels.

## Structural response extension

**CLOSED.** The full evidence ladder (temporal persistence -> difficulty-stratified relative advantage -> existing-family controlled flips -> source-controlled solution-space mediator -> bounded operational first divergence) ran to completion for both frozen pairs (`R02687`/objectiveFirst, `R02094`/intersectionHarvest); current detail lives in `solver-optimization-workstreams.md`'s WS1 section. [`stage 3`](../reports/2026-09-11-ws1-stage3-isolated-technique-resolve-001.md), [`stage 4`](../reports/2026-09-12-ws1-stage4-solution-space-mediation-result-001.md), [`stage 5`](../reports/2026-09-12-ws1-stage5-first-divergence-result-001.md). n=2 licenses no selector. Reopen only for a new pair/premise, not a third variant of this question.
...
| **Minimal technique-niche counterexamples** | Use reducer + exact/reference validation when a temporally/difficulty-robust niche needs causal isolation. Exact paths adjudicate known-live loss, not broad solver acquisition. The microscope method is the preferred one-specimen form when the question is an extinction-boundary future-feasibility distinction rather than a reduced structural niche. |
...
| **Queryable analytical layer** | Reopen only if recurring joins still require bespoke scripts after existing helpers are extended. Capability memory is deliberately a narrow rebuildable view, not a warehouse. |
| **Class-1 compact beam-menu exposure** | Naive append/insert under strict total work is closed: 17/25 class-1 resolves fall in the two high-intersection policy rules where extra beam exposure dilutes protected suffix winners. Reopen only with a non-strict/additive-tier envelope, or after proving reserve headroom in the 8/25 outside those rules. [`reconciliation`](../reports/2026-09-11-class1-compact-beam-menu-budget-dilution-reconciliation-001.md) |

The census cross-evidence plan is a standing evidence map, not an active sequence. Source-stratified hint/path mining and basin analysis already promoted to WS1 belong in the live workstream authority.
