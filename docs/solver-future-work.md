...
| Hidden semantic forcedness | **Contingent measurement only.** After PR #1952 lands/reconciles, sample production multi-successor beam states and exact-label every surviving child with the existing explicit-prefix reference workflow. Advance only if exactly-one-LIVE states occupy a material resolved-state/work share; otherwise close this forced-future form. [preflight](solver-semantic-forcedness-preflight.md) |
