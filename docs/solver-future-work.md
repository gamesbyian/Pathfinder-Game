...
| **Repair restart allocation in the near-miss band** | The exact production proposal `STRATEGY_REPAIR_LATE_PROBE_RESTART_SPLIT` remains closed: its required W=150M pre-wiring pilot tied restart and continuation 9/36 on the harder bestBadness 7–9 population. Preserve, however, the independently replicated W=64M result on the fully spent bestBadness<=6 near-miss stratum: restart solved 8/43 vs continuation 3/43, with five restart-only gains and zero losses. Reopen only if current repair-tail work exposes a fresh legal population with the same near-miss texture, or if a changed production allocation contract makes that band-specific signal directly actionable. Do **not** rerun the closed 150M two-way split unchanged, and do not infer that restart is globally better than continuation. |
