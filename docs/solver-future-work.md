...
|---:|---|---|
| 1 | **Open-path topology / path-history representation** | **PREMISE EARNED; MICROSCOPE NEXT.** Controlled same-board/same-endpoint forks produced 4 exact LIVE/DEAD-discordant pairs across 2 independent human parents with matched mechanic-progress controls and one-turn puncture-side separation. Next compare both (a) a compact reusable descriptor and (b) richer generic per-instance topology procedures such as winding/phase state, separator-side commitment, region accessibility, topology-aware canonicalization, or exact impossibility checks. Compactness or semantic recurrence is required only for route (a), not by level-blindness itself. No production routing of raw phase. [`result`](../reports/2026-09-16-class5-controlled-topology-fork-pilot-result-001.md) [`reconsideration`](../reports/2026-09-16-level-blindness-audit-topology-reconsideration-001.md) |
...
| 3 | **Minimal DEAD-core / minimum-relaxation diagnosis** | If topology/H1 expose DEAD contrasts without a compact relation, relax tiny semantic commitment sets. Score separately: (a) recurring cores suitable for reusable generic abstractions and (b) per-instance core derivation useful enough for solve-local conflict learning even when individual cores differ across parents |
