...
### repair-fallback gate widening (portal-heavy / high-intersection-burden sub-rules) — partially resolved via REPAIR_LATE_PROBE_NODE_BUDGET 2026-08-22
...
The broader gate-widening idea above (widening `needsRepairFallback` itself to cover `portal-heavy` and more of `high-intersection-burden`) remains open and is a different mechanism from this cap raise — the late-probe tier's higher cap gives every `repairConfigs`-empty level a bigger *unconditional, dead-last* shot, but a level whose real winning repair search needs meaningfully more than 5,000,000 nodes still won't be reached by it; the gate-widening question (giving such levels repair as a *protected, budgeted* tier rather than a capped afterthought) is unresolved and still blocked on the "no clean feature" finding above.
