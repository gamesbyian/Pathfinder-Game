...
- **H2 marginal information velocity / structural stasis:** **CLOSED NEGATIVE** on two tested signal forms (within-run stasis fraction, distinct-badness count): both show near-zero correlation with a 10x work escalation's marginal badness gain, on dev and independent confirmation halves alike, despite real marginal value existing (78.6% of levels gained). Does not close other structural-response dimensions from the source hypothesis's candidate list. [`result`](../reports/2026-09-17-h2-structural-stasis-observer-result-001.md)
- **Behavioral-state quotient:** unchanged. The behavioral-state quotient / residual-language probe needs a tiny exact-probe first, advancing only if future behavior admits a materially coarser compact equivalence.
