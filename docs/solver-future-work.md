...
| **Reason-producing dead-state reuse / local nogoods** | Recovered from the older backlog after no later closeout/owner was found. Reopen only if exact labels or traces show materially repeated exploration of the same small, sound dead cause inside one solve. First measure recurrence and wasted work; then shadow the smallest solve-local reason key that includes all future-relevant state. No CDCL/LCG, non-chronological backtracking, or general learned-clause machinery unless this bounded form wins. [`excavation`](../reports/2026-09-12-solver-future-work-backlog-excavation-001.md) |
