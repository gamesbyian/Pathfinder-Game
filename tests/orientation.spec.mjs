...
async function readOrientationRoundTrip(page) {
    return page.evaluate(() => {
        ...
        const transformed = window.APP.LevelUtils.transformPoint(
            base.x, base.y, eng.variant, level.grid.w, level.grid.h,
        );
        const pointer = {
...
test.describe('Runtime orientation characterization', () => {
    test('perspective control cycles all eight transforms with render/input agreement and reset preservation', async ({ page }) => {
        ...
        const start = (await readOrientationRoundTrip(page)).variant;
        for (let step = 0; step < 8; step += 1) {
            const observed = await readOrientationRoundTrip(page);
            expect(observed.variant).toBe((start + step) % 8);
            expect(observed.mapped).toEqual(observed.base);
            expect(observed.swapped).toBe([1, 3, 6, 7].includes(observed.variant));
            await page.locator('#whoaBtn').click();
        }

        expect((await readOrientationRoundTrip(page)).variant).toBe(start);

        await page.locator('#whoaBtn').click();
        const beforeReset = (start + 1) % 8;
        expect((await readOrientationRoundTrip(page)).variant).toBe(beforeReset);

        await page.locator('#resetBtn').click();
        await expect.poll(async () => (await readOrientationRoundTrip(page)).variant).toBe(beforeReset);
