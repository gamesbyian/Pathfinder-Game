...
| Naming cleanup / rename migration | First run `npm run naming:status`. Then read [`docs/naming-and-vocabulary.md`](docs/naming-and-vocabulary.md), [`docs/naming-cleanup-plan.md`](docs/naming-cleanup-plan.md), the active phase/batch authority under [`docs/naming-cleanup-phase-records/`](docs/naming-cleanup-phase-records/phase-08.md), [`docs/naming-cleanup-process-hardening.md`](docs/naming-cleanup-process-hardening.md), and [`docs/change-recipes.md`](docs/change-recipes.md). Use immutable ledger IDs in records/PRs. The technical Phase-8 gate is ready, but implementation is serialized: claim only the next batch, create its checked-in execution record, and never stack the following batch before merge. |
