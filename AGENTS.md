...
| Existing solver research question / acquisition | `research:dossier -- --question-id=<id>`, then `research:acquisition-preflight` if needed; stable joins over lexical inference; no auto-generation |
...
4. Use cheap discovery first: question intake for new questions; `tooling-census --compact`; `research-status-index --compact`; `research:dossier` for known IDs; `research-asset-query.mjs` for resources.
