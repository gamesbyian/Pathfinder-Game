...
| New/unregistered solver research idea or question | [`docs/research-question-intake.md`](docs/research-question-intake.md); expand/contextualize first, use `research-status-index --compact --query=<term>` for candidate discovery, then switch to a dossier only when a stable question ID emerges |
| Existing solver research question / acquisition | `research:dossier -- --question-id=<id>`; prefer stable question/premise/MO joins and authored multi-asset recipes over lexical inference; then `research:acquisition-preflight` if needed; no authority/auto-generation |
| Research-system question / audit idea | [`docs/research-question-intake.md`](docs/research-question-intake.md); route by discoverability/authority/observability/retention/join/lifecycle/decision-quality/operational-cost/epistemic-coverage rather than forcing it into the solver-science question registry |
...
4. Use cheap discovery before broad context: [`research-question-intake.md`](docs/research-question-intake.md) for a new question, `tooling-census --compact`, `research-status-index --compact`, `research:dossier` for a known question, and `research-asset-query.mjs` for resources.
