...
| Naming / future rename migration | The repository-wide naming cleanup is complete. Use [`docs/naming-and-vocabulary.md`](docs/naming-and-vocabulary.md) for canonical terms and [`docs/change-recipes.md`](docs/change-recipes.md) for any new cross-boundary rename. `npm run naming:status` reports the completed program/history state. [`docs/naming-cleanup-plan.md`](docs/naming-cleanup-plan.md), the ledger, and Phase-8/15 records are completed implementation evidence; do not reopen them as a new phase sequence or invent Phase 16. |
...
| Solver optimization/research | [`docs/solver-optimization-workstreams.md`](docs/solver-optimization-workstreams.md), then [`docs/solver-research-post-naming-resumption.md`](docs/solver-research-post-naming-resumption.md) for the post-cleanup evidence bridge, then [`docs/solver-research-operating-model.md`](docs/solver-research-operating-model.md), [`docs/solver-research-data-assets.md`](docs/solver-research-data-assets.md), and the relevant specialist doc |
| Existing solver research data / cross-evidence | [`docs/solver-research-data-assets.md`](docs/solver-research-data-assets.md) for evidence families, join keys, inter-relevance, and leakage/freshness boundaries; then [`docs/tooling-catalog.md`](docs/tooling-catalog.md) for commands and `node scripts/research-status-index.mjs --compact --query=<term>` for prior work |
...
| Prior experiment | Query `node scripts/research-status-index.mjs --compact --query=<term>` first; stage/routing-regime/attempt-config/composite-action queries are bilingual through owning normalizers. Then use [`reports/README.md`](reports/README.md) and the current topic doc. |
...
4. **Check existing machinery and evidence before adding more.** For a named concept, start with `node scripts/tooling-census.mjs --compact --query=<concept>`; use [`docs/tooling-catalog.md`](docs/tooling-catalog.md) when choosing among broader tool families. For solver research, also consult [`docs/solver-research-data-assets.md`](docs/solver-research-data-assets.md) before new data generation or broad compute so existing hints/provenance, census, profiles, variants, logs, manifests, traces, exact labels, or other evidence can falsify/stratify the premise first. Open the full `package.json` or scan script/workflow directories only when those compact front doors are insufficient. [`scripts/README.md`](scripts/README.md) and [`.github/workflows/README.md`](.github/workflows/README.md) are the local/remote maps. Follow canonical naming vocabulary in [`docs/naming-and-vocabulary.md`](docs/naming-and-vocabulary.md) plus conventions in [`docs/README.md`](docs/README.md), [`reports/README.md`](reports/README.md), and [`logs/README.md`](logs/README.md).
5. **Audit propagation across boundaries.** Solver results/stages, mechanics, hints/provenance, app state, generated schemas, scripts, and workflows commonly have multiple consumers; use [`docs/change-recipes.md`](docs/change-recipes.md). The completed Phase-8+ naming program is evidence for the stronger impact-map, compatibility-owner, consumer-inward, and merge-barrier practices in [`docs/naming-cleanup-process-hardening.md`](docs/naming-cleanup-process-hardening.md); apply those practices to future risky renames without reopening the completed cleanup phases.
...
- **Inventory before generating.** Before a new broad run or dataset, use [`docs/solver-research-data-assets.md`](docs/solver-research-data-assets.md) to check which existing evidence can falsify, stratify, contextualize, or independently challenge the premise and which joins are scientifically valid. This is not a mandate to mine every available axis; materially searched joins still contribute selection pressure.
...
- **Respect the variant-family dataset boundary.** The ~2.5 GB dataset lives on `claude/variant-levels-solver-insights-tpk4qg`; use current `main` code with the dataset mounted separately. Existing variants are evidence, not a command to generate more. `family:index` reconciles mixed historical/current aggregate snapshots per logical variant and normalized evidence payload, preserves differing observations, and exposes `diagnostics.familyAttemptAggregates`; resolve reported conflicts/ambiguity before decision-bearing family analysis rather than implementing a scratch filename-precedence rule. See [`docs/variant-level-research.md`](docs/variant-level-research.md).
