...
| Solver research data / cross-evidence | `node scripts/research-asset-query.mjs --query=<term>` for one asset family; `npm run research:relations -- --list` for cross-authority joins/reconciliation; open [`docs/solver-research-data-assets.md`](docs/solver-research-data-assets.md) only for topology/boundary guidance |
