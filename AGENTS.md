...
| Naming cleanup / rename migration | [`docs/naming-and-vocabulary.md`](docs/naming-and-vocabulary.md), [`docs/naming-cleanup-plan.md`](docs/naming-cleanup-plan.md), [`docs/naming-cleanup-history-and-lessons.md`](docs/naming-cleanup-history-and-lessons.md), [`docs/naming-cleanup-process-hardening.md`](docs/naming-cleanup-process-hardening.md), [`docs/naming-cleanup-ledger.json`](docs/naming-cleanup-ledger.json), then [`docs/change-recipes.md`](docs/change-recipes.md) and the owning topic doc. The technical Phase-8 gate is ready, but implementation is serialized: claim only the next batch, create its checked-in execution record, and never stack the following batch before merge. |
...
| Completed GHA solver/research run | Run `npm run gha:result -- --run=<run-id>` (or `--workflow=<name>`). Only consult [the retrieval fallback](.github/workflows/README.md#agent-result-retrieval) or enumerate jobs/shards if the standard result is missing/incomplete. |
...
5. **Audit propagation across boundaries.** Solver results/stages, mechanics, hints/provenance, app state, generated schemas, scripts, and workflows commonly have multiple consumers; use [`docs/change-recipes.md`](docs/change-recipes.md). For Phase-8+ naming work, establish the one active batch/branch first, create the checked-in execution record, complete the contract-migration matrix and before/after parity where applicable, close from consumers inward, and clear the duplicate/no-op pre-merge barrier required by [`docs/naming-cleanup-process-hardening.md`](docs/naming-cleanup-process-hardening.md).
