...
- **Respect the variant-family dataset boundary.** The ~2.5 GB dataset lives on `claude/variant-levels-solver-insights-tpk4qg`; use current `main` code with the dataset mounted separately. Existing variants are evidence, not a command to generate more. `family:index` reconciles mixed historical/current aggregate snapshots per logical variant and exposes `diagnostics.familyAttemptAggregates`; inspect those diagnostics before decision-bearing family analysis rather than implementing a scratch filename-precedence rule. See [`docs/variant-level-research.md`](docs/variant-level-research.md).
