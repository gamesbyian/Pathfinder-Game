...
| Completed GHA solver/research run | Run `npm run gha:result -- --run=<run-id>` (or `--workflow=<name>`); then read [`.github/workflows/README.md#agent-result-retrieval`](.github/workflows/README.md#agent-result-retrieval) before enumerating jobs/shards |
