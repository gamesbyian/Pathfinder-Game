...
- On disk, all 3 corpora use the same per-level artifact format (`{schemaVersion: 3, hints: Hint[]}`), read/written exclusively through `scripts/level-data-io.mjs` (`readLevelHints`/`writeLevelsWithHints`/`parseHintFileContents`/`stringifyHints`) — never hand-edit or re-serialize these files directly: published → `data/hints/<NNN>.json`; stress-corpus-1 → `data/stress/hints/<NNN>.json`; stress-corpus-2 → `data/stress/hints-random/<NNN>.json` (a sibling directory, not `hints/`, since corpus 1 and 2 share `data/stress/` as a parent but number levels 1..N independently — see `hintsDirFor` in `level-data-io.mjs`, which derives the directory name from the levels-json basename).
