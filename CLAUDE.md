...
- On disk, all 3 corpora use the same per-level artifact format (`{schemaVersion: 3, hints: Hint[]}`), read/written exclusively through `scripts/level-data-io.mjs` (`readLevelHints`/`writeLevelsWithHints`/`parseHintFileContents`/`stringifyHints`) — never hand-edit or re-serialize these files directly: published → `data/hints/<NNNNN>.json`; stress-corpus-1 → `data/stress/hints/<id>.json`; stress-corpus-2 → `data/stress/hints-random/<id>.json` (a sibling directory, not `hints/`, since corpus 1 and 2 share `data/stress/` as a parent but number levels independently — see `hintsDirFor` in `level-data-io.mjs`, which derives the directory name from the levels-json basename). The join key is `hintKeyForLevel()`'s job: a level's own persistent `id` string when it has one (both stress corpora — `S00028.json`, `R00028.json`, used verbatim, not stripped to a bare number, since Corpus 1 mixes S-/R-prefixed ids whose numeric suffixes can collide), else its 1-based array position (published, which has no id field yet — see `docs/level-id-unification-plan.md`). **History**: this was position-only for all 3 corpora until 2026-07-12, when it turned out the stress corpora's `id` field was never actually used for hint storage — meaning reordering either stress corpus's array would have silently misattributed every hint file, despite an `id` field existing specifically to prevent exactly that. Migrated via `scripts/migrate-hint-files-to-id-keys.mjs` (one-time; verified lossless by summing hint counts per-file before/after the rename, not just trusting the file count). The one-time bare-array/`hintMetadata`-wrapper → `schemaVersion: 2` backfill (`scripts/migrate-hint-schema-v2.mjs`, already run) is now redundant with `parseHintFileContents`'s on-read auto-upgrade of any legacy shape (including the later v2 → v3 nested-provenance step) — kept only as a historical record, not something to re-run.
