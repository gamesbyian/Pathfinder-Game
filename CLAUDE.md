...
- ~8,300 hint paths total feed the in-game hint system; stored inline in `data/levels.json`, they are the bulk of its size.
- Player-submitted levels are imported from Firestore via `npm run levels:import-published` (dedupes by structural fingerprint, regenerates `level-heatmaps.json`).
