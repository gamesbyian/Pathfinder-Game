...
None currently checked in. The `confirm-residual-001-archetype-audit-one-shot.yml` workflow that pulled `solver-residual-confirmation.yml` run artifacts cross-run (via `download-artifact`'s `run-id` input) to run `scripts/stress/confirm-residual-001-archetype-audit.mjs` against them was deleted after use, per this section's own retention rule, once its answer (a main-loop late-reserve scheduling gap — see `docs/solver-opt-in-experiment-ledger.md`'s `STRATEGY_MUSTCROSS_FLIPPER_WIDE_BEAM_EXPOSURE` row) was recorded. The plain Node script itself (not a workflow, so not subject to this section) is retained under `scripts/stress/` since re-running it locally against a small repro is far cheaper than a fresh GHA dispatch, and the archetype-eligibility-vs-scheduling gap it found is likely to recur for other candidates in a trailing-reserve window; re-add a thin one-shot workflow wrapper like this one if a future candidate needs the same cross-run artifact pull again.