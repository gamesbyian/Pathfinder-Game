...
`repair-restart-continuation-pilot-one-shot.yml` shards `scripts/stress/restart-continuation-population-pilot.mjs`'s existing research harness by `--offset`/`--limit` ranges (fixed census order) so a large-`W` population pilot — individual levels observed to take anywhere from ~2 minutes to nearly an hour — runs in wall time bounded by the slowest shard rather than the slowest full population. Currently active for the pre-wiring pilot reports/2026-08-27-repair-restart-continuation-production-candidate-design.md's "Required pre-wiring pilot" calls for; delete per this section's own rule once that pilot's result is recorded. The underlying script (not a workflow, so not subject to this rule) is retained regardless, since the same sharded-dispatch need is likely to recur for any future W-scale check on this research line.