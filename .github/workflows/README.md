...
These are not solver-batch entrypoints, but remain listed here for workflow discoverability checks: `ci.yml`, `daily-file-size-ratchet.yml`, `solver-diagnostics.yml`, `deploy-pages.yml`, `deploy-firestore-rules.yml`, `main-push-validation.yml`, `premise-map-hardening.yml`, and `solver-evidence-integrity-guard.yml`. `premise-map-hardening.yml` validates the frozen solver premise-map inputs, relation layers, discovery-lineage coverage, and hardening overlay before those materials can be mined or changed. `daily-file-size-ratchet.yml` performs the repository-wide size/debt scan once per day (and on manual dispatch); ordinary PR CI checks only files changed by that PR. Completed migration/campaign browser gates belong in Git history and dated evidence, not in the maintained workflow surface.
