...
`hint-provenance-hostile-audit.yml` is a manual hostile forensic audit. Durable physical-reader, persistence, and semantic-consumer invariants are owned by ordinary validation; the standalone workflow remains for intentional whole-system re-audits.
nsuses maintained producer/consumer surfaces against the current physical schema, enforces central workflow persistence ownership, and runs semantic consumer floors so a green migration transaction cannot mask a stale downstream reader.
...
- Direct/isolated tools such as `method-probe` and technique census retain actual valid paths with `isolatedTechnique=true`, preserving the distinction from competitively-budgeted production-ladder capability.
...
- `static-portfolio-confirmation.yml` — fixed-work static-portfolio A/B: runs a named ordered technique-key "arm" against a committed population (`data/stress/<cohort>-population.json`) under one shared, per-technique-capped work budget (`technique-census-cell.mjs`'s `cell.perTechniqueWorkCap`), and reports coverage/work against a named control arm (`data/stress/<cohort>-arms.json`). Artifact-only, never commits to main. See [`solver-optimization-workstreams.md`](../../docs/solver-optimization-workstreams.md) Workstream 2 gate-sequence (C) rung 2 and [`2026-09-02-static-portfolio-construction-pilot.md`](../../reports/2026-09-02-static-portfolio-construction-pilot.md) for the evidence/design this workflow executes. Reuses `technique-census.mjs`'s shard execution as-is; the plan builder (`build-static-portfolio-plan.mjs`) and combiner (`combine-static-portfolio-shards.mjs`) are purpose-built, not shared with `technique-census.yml`'s own T1-T4-coupled plan/combine stages.
...
- `ws1-remaining-length-intrasolve-stage-a-one-shot.yml` — one-shot, frozen 20-level WS1 remaining-length intra-solve bridge experiment. It preserves the preregistered control/treatment candidate-ordering comparison, publishes the standard solver-result front door, and remains artifact-only; it is not a recurring general-purpose sweep.
...
These are not solver-batch entrypoints, but remain listed here for workflow discoverability checks: `ci.yml`, `ci-scoped-dry-run.yml`, `ci-node-concurrency-benchmark.yml`, `ci-deep-concurrency-benchmark.yml`, `ci-historical-value-audit.yml`, `ci-semantic-fault-injection-audit.yml`, `daily-file-size-ratchet.yml`, `solver-diagnostics.yml`, `deploy-pages.yml`, `deploy-firestore-rules.yml`, `main-push-validation.yml`, `premise-map-hardening.yml`, and `solver-evidence-integrity-guard.yml`. `main-push-validation.yml` also computes a dependency-free, non-gating impact shadow over the full push event range (`before` → `sha`) while retaining its existing broad `ci:fast` + build + solver-canary safety net. PR and main-push shadow jobs persist the machine-readable decision as a short-lived `ci-impact-shadow` artifact, including event/SHA context, selected surfaces/groups/capabilities, and lane packing. `ci-scoped-dry-run.yml` is manual-only activation rehearsal: given explicit base/head refs it runs the impact router, selected fast lane, conditionally selected deep lane, and stable `ci-success` contract without changing ordinary PR gating. `ci-node-concurrency-benchmark.yml` is manual-only measurement infrastructure: it executes the identical `test:node` population sequentially at requested `PATHFINDER_PARALLEL_JOBS` values on one runner so process-fan-out changes can be measured before changing the default. `ci-deep-concurrency-benchmark.yml` is evidence-only measurement infrastructure for the ≤35-second CI program: it runs the unchanged covered Vitest population, heavyweight solver proofs, and Firestore boundary concurrently on one ordinary runner, records child and combined wall time, and never weakens semantic pass/fail criteria. `ci-typescript-incremental-benchmark.yml` is manual-only measurement infrastructure for TypeScript build-info reuse: it measures cold/warm/restored-cache checks and fault-injects production/test-only type errors before any Actions cache is adopted. `ci-historical-value-audit.yml` is manual-only audit infrastructure: it reconstructs mechanical check lineage from git history, exhausts retained PR/main-push CI metadata through the GitHub API, records retrieval gaps explicitly, and uploads a compact normalized corpus for failure adjudication and cadence simulation; it does not change ordinary validation. `ci-testability-topology-audit.yml` is manual-only testability/cost topology infrastructure: it inventories permanent validation contracts for invocation/process/fixture coupling and can join that structure to a prior historical CI audit artifact; it does not change ordinary validation. `ci-semantic-fault-injection-audit.yml` is PR/manual audit infrastructure that injects representative game, persistence, solver, and router-authority semantic defects in isolated worktrees and requires both correct impact routing and an existing detector catch; it is evidence-only and never suppresses ordinary validation. `persist-targeted-failure-response.yml` is a reusable helper called only by the targeted sweep's opt-in compact-evidence persistence job; checkout intentionally precedes artifact download so worktree cleaning cannot erase staged evidence. `premise-map-hardening.yml` validates the frozen solver premise-map inputs, relation layers, discovery-lineage coverage, and hardening overlay before those materials can be mined or changed. `daily-file-size-ratchet.yml` performs the repository-wide size/debt scan once per day (and on manual dispatch); ordinary PR CI checks only files changed by that PR. `hint-consolidation-closeout.yml` is a manual forensic/reproducibility audit for the completed Hint consolidation plan. Durable merge-safety invariants live in ordinary validation; dispatch this workflow only when intentionally re-auditing closeout evidence.
nt-evidence-**`, hint-evidence reports, and the harvester/closeout workflows themselves): it re-runs the post-enrichment determinism/reconstructability/referee oracles and dry-runs the schema-v4 migration and level sparse-serialization benchmark to prove they still hold after a relevant change; it does not change ordinary validation for unrelated PRs. Completed migration/campaign browser gates belong in Git history and dated evidence, not in the maintained workflow surface.
...
Delete a bespoke one-shot workflow once its execution surface is no longer needed, but treat workflow retirement and evidence reconstructability as separate decisions. Before deletion, apply the ordinary investigation closeout rule: if artifact-bound or branch-bound primary rows are materially required to support or later audit a durable decision, capability/future-work premise, reusable exact/reference resource, or row-sensitive mechanism claim, preserve the smallest existing-compatible evidence bundle or explicitly record what will expire and which later re-analysis will no longer be possible. Merely recording the aggregate answer is sufficient when row-level reconstruction is not material; this rule does not require archiving every one-shot artifact.
...
`solver-elite-prefix-dfs-retry-validate.yml` was likewise a fixed-sample one-off. Its two recorded retry budgets produced zero recoveries and the owning report closes further investment in that retry form absent a materially different underlying operator, so the workflow was deleted after the evidence was retained in `reports/2026-08-07-repair-elite-prefix-dfs.md`. The local validation script remains classified as cold research for reproducibility.

`solver-repair-fallback-reserve-sample-ab.yml` was purpose-built for the now-closed `STRATEGY_REPAIR_FALLBACK_NODE_RESERVE` experiment. The opt-in ledger records that more fallback participation produced no solves and that more of the same repair work is not the missing capability, so the workflow was deleted rather than retained as a permanent dispatch surface. Its selector remains cold research for reproducibility; future targeted A/Bs should use the generic routing-regime/targeted-sweep infrastructure when their evidence semantics fit.