...
**Status: RETIRED 2026-07-17.** Replaced by
[`solver-stress-refresh.yml`](solver-stress-refresh.yml) (see
[`README-solver-stress-refresh.md`](README-solver-stress-refresh.md)) — a single matrix-based
workflow with no persistent branches, no checkpoint/resume, and a corpus-1 job folded in. The 20
`solver-corpus2-batch-*.yml` files and `scripts/generate-corpus2-batch-workflows.mjs` have been
deleted. The 20 `stress-corpus2-batch-NN` branches could not be deleted with this session's GitHub
credentials (same limitation noted in the "After you're done" section below) — they've been
force-reset to `main`'s tip instead, so they're inert orphaned refs, not live state. This file is
kept only as a historical record of that design and the two real stale-code incidents it caused
(see "Track record" below) — read `README-solver-stress-refresh.md` for how refreshes work now.

**Status (superseded): one-off, rare/on-demand infrastructure.** Generated by
