# Portfolio solve-only sweep

Generated: 2026-07-25T11:36:44.766Z
Commit: 6bbd91d8404b008d84a21e21223ffedcf5f66461
Corpus: data/stress/stress-levels-random.json
Scheduler mode: legacy
Budget: 200000ms
Node budget: 1200000000
Repair budget fraction override: (default, 6x)
Workers: 4
Resume: yes (0 level(s) loaded from logs/solver-highbudget-sweep/corpus2-batch-224.json.checkpoint.jsonl)
Feature filter: (none)
Priority: (none)
Attempt cache: (none)
Levels run: 10

- Solved (any phase): 2
- Solved before fallback (portfolio-tier find): 0
- Solved by fallback/legacy path only: 2
- Unsolved: 8
- Hints saved: yes (2 level(s), 2 hint file(s) changed)

## Pass distribution

- Pass 1: 0
- Pass 2: 0
- Pass 3: 0
- Conditional: 0
- Fallback (portfolio mode's embedded legacy-equivalent phase): 0
- Legacy (plain legacy-mode solve): 2
- Unsolved: 8

## Portfolio-tier finds (solvedBeforeFallback)

- None
