...
function makeCandidateEvents(paths, { generator, levelNumber, provenance = {}, diagnostics = {} }) {
...
async function processLevel(levelNumber, raw, opts) {
    ...
    return {
        ...
        rejected,
        runs,
        acceptedPaths: opts.writeLevels ? (opts.auditMode ? [] : accepted.map(a => a.path)) : maybePaths(opts.auditMode ? [] : accepted.map(a => a.path), opts),
...
for (const levelNumber of levelNumbers) {
    ...
    if (opts.writeLevels && !opts.auditMode && result.acceptedCount > 0) {
...
if (opts.writeLevels && !opts.auditMode && totalAccepted > 0) writeResult = writeLevelsWithHints(levelsPath, rawLevels);
await atomicWriteJson(opts.output, {
    ...
    writes: {
        requested: opts.writeLevels,
        skippedForAudit: opts.writeLevels && opts.auditMode,
        levelsPath: relativePath(levelsPath),
        changedFiles: writeResult?.levelsChanged ? [relativePath(levelsPath), ...changedHintFiles] : changedHintFiles,
        result: writeResult,
        postWriteReminders: opts.writeLevels && !opts.auditMode && totalAccepted > 0
...
if (opts.writeLevels && !opts.auditMode) {
...
if (opts.auditMode && opts.writeLevels) console.log('Audit mode enabled; skipped level writes despite --write-levels.');
