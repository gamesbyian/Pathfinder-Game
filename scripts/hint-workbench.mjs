...
await atomicWriteJson(opts.output, {
    ...
    writes: {
        ...
        postWriteReminders: (opts.writeLevels || Boolean(opts.writePatch)) && !opts.auditMode && totalAccepted > 0
            ? ['npm run levels:generate-heatmaps', 'npm run check:level-data-validity', 'npm run test:hint-path-validation']
            : [],
    },
    preset: {
...
if (opts.writePatch && !opts.auditMode) {
    ...
    if (totalAccepted > 0) console.log('Review patch, then run post-apply checks: npm run levels:generate-heatmaps && npm run check:level-data-validity && npm run test:hint-path-validation');
} else if (opts.writeLevels && !opts.auditMode) {
    console.log(`Updated -> ${opts.levelsJsonPath}`);
    if (totalAccepted > 0) console.log('Post-write checks: npm run levels:generate-heatmaps && npm run check:level-data-validity && npm run test:hint-path-validation');
}
