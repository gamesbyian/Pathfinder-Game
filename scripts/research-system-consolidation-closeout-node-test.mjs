...
assert.equal(frozenRetrospective.explorationTriggers.some(row =>
  row.kind === 'ontology-or-representation-block' || row.kind === 'shared-negative-assumption'), false,
  'the frozen 2026-09-19 closeout artifact itself must remain free of these triggers; new data added ' +
  'after that date can add refs that fall inside the frozen date window without retroactively changing ' +
  'what was true at closeout time, so this must read the frozen artifact, not a live recomputation');
