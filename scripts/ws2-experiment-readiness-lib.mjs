...
function wallMs(row) {
  const value = row.totalMs ?? row.elapsedMs;
  return Number.isFinite(value) && value >= 0 && row.deadlineTruncated !== true ? Number(value) : null;
}
...
export function materializeClass2Cohort(rows, provenance) {
  for (const field of ['seed', 'selectionVersion', 'corpusHash', 'baseSha', 'controlConfig', 'treatmentConfig', 'controlConfigHash', 'treatmentConfigHash', 'envelope', 'stageOrder', 'schedulerMode', 'workerConfig', 'workerCount']) {
    if (provenance[field] == null) throw new Error(`missing freeze provenance: ${field}`);
  }
  if (typeof provenance.schedulerMode !== 'string' || !provenance.schedulerMode.trim()) throw new Error('schedulerMode must be a non-empty string');
  if (typeof provenance.workerConfig !== 'object' || Array.isArray(provenance.workerConfig) || Object.keys(provenance.workerConfig).length === 0) throw new Error('workerConfig must be a non-empty object');
  if (!Number.isInteger(provenance.workerCount) || provenance.workerCount < 1) throw new Error('workerCount must be a positive integer');
  ...
  const freezeIdentityHash = stableHash({ entries, provenance });
  return { schemaVersion: 1, kind: 'ws2-class2-frozen-cohort', ...provenance, entries, populationHash, freezeIdentityHash, counts: { gain: selectedGain.length, collateral: selectedCollateral.length, total: entries.length } };
}
...
export function analyzeClass2Economics({ manifest, controlRows, treatmentRows }) {
  ...
  const downstreamDisplacement = collateralRows.map(({ entry, c, t }) => {
    const stageId = winningStage(c);
    const controlWork = stage(c, stageId)?.actualWork;
    const treatmentWork = stage(t, stageId)?.actualWork;
    const usable = Number.isFinite(controlWork) && Number.isFinite(treatmentWork);
    return { id: entry.id, stageId, controlWork: usable ? Number(controlWork) : null,
      treatmentWork: usable ? Number(treatmentWork) : null,
      workDelta: usable ? Number(treatmentWork) - Number(controlWork) : null,
      reductionFraction: usable && Number(controlWork) > 0 ? (Number(controlWork) - Number(treatmentWork)) / Number(controlWork) : null };
  });
  const downstreamWorkUnusable = downstreamDisplacement.filter(row => row.controlWork === null || row.treatmentWork === null).map(row => row.id);
  const downstreamStarved = downstreamDisplacement.filter(row => {
    const treatmentRow = treatment.get(row.id);
    return !validSolve(treatmentRow) && row.controlWork > 0 && row.treatmentWork === 0;
  }).map(row => row.id);
  const downstreamPartiallyReduced = downstreamDisplacement.filter(row => row.controlWork > 0 && row.treatmentWork > 0 && row.treatmentWork < row.controlWork).map(row => row.id);
  const usableDownstream = downstreamDisplacement.filter(row => row.controlWork !== null && row.treatmentWork !== null);
  const downstreamControlWork = usableDownstream.reduce((sum, row) => sum + row.controlWork, 0);
  const downstreamTreatmentWork = usableDownstream.reduce((sum, row) => sum + row.treatmentWork, 0);
  ...
  const wallPairs = pairs.map(({ entry, c, t }) => ({ id: entry.id, controlMs: wallMs(c), treatmentMs: wallMs(t),
    measurementMismatch: c.wallMeasurementIdentity != null && t.wallMeasurementIdentity != null && c.wallMeasurementIdentity !== t.wallMeasurementIdentity }));
  const unusableWallMeasurements = wallPairs.filter(row => row.controlMs === null || row.treatmentMs === null).map(row => row.id);
  const asymmetricWallMeasurements = wallPairs.filter(row => (row.controlMs === null) !== (row.treatmentMs === null) || row.measurementMismatch).map(row => row.id);
  const usableWallPairs = wallPairs.filter(row => row.controlMs !== null && row.treatmentMs !== null && !row.measurementMismatch);
  const controlWallMs = usableWallPairs.reduce((sum, row) => sum + row.controlMs, 0);
  const treatmentWallMs = usableWallPairs.reduce((sum, row) => sum + row.treatmentMs, 0);
  const wallDeltaMs = treatmentWallMs - controlWallMs;
  ...
  if (unusableWallMeasurements.length) reasons.push('unusable paired wall measurements');
  if (asymmetricWallMeasurements.length) reasons.push('asymmetric wall measurements');
  if (downstreamWorkUnusable.length) reasons.push('unusable downstream work measurements');
  ...
  return { decision, reasons, gains: gains.map(x => x.entry.id), losses: losses.map(x => x.entry.id), targetStageGains: targetStageGains.map(x => x.entry.id), gainParticipation, collateralReach,
    downstreamStarved, downstreamPartiallyReduced, downstreamWorkUnusable, downstreamDisplacement,
    downstreamWorkSummary: { rows: usableDownstream.length, controlWork: downstreamControlWork, treatmentWork: downstreamTreatmentWork,
      workDelta: downstreamTreatmentWork - downstreamControlWork,
      reductionFraction: downstreamControlWork > 0 ? (downstreamControlWork - downstreamTreatmentWork) / downstreamControlWork : null },
    asymmetricCensoring, targetWorkDelta, wholeWorkDelta,
    wallEconomics: { primaryMeasure: 'workSpent', usablePairs: usableWallPairs.length, unusableWallMeasurements, asymmetricWallMeasurements,
      controlWallMs, treatmentWallMs, wallDeltaMs, wallDeltaFraction: controlWallMs > 0 ? wallDeltaMs / controlWallMs : null,
      paired: wallPairs.map(row => ({ ...row, deltaMs: row.controlMs !== null && row.treatmentMs !== null ? row.treatmentMs - row.controlMs : null })) } };
}
