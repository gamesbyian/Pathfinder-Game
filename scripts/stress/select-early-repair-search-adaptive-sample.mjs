 ...
 * AND carries at least one must-turn cell (the only thing that makes a biased repair-probe tier
 * exist at all). A uniform random Corpus-2 sample wastes most of its compute re-confirming
 * zero-effect on levels that are structurally guaranteed byte-identical either way -- this script
 * instead draws a deterministic seeded subsample from the ELIGIBLE population specifically, plus a
 * small control sample from the ineligible remainder (to empirically catch anything the code trace
 * missed, not just to trust the trace).
 *
...
console.log(`Eligible population (repair-gated AND has mustTurn): ${eligible.length} / ${levels.length}`);
