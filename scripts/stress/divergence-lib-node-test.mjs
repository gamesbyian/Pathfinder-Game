...
const easy=tracePathRanks({api,level,prep,path:[0,1,2],scoringProfile:{}});
const hard=tracePathRanks({api,level,prep,path:[0,2],scoringProfile:{}});
...
const ab=scoreFlagAblation({trace:cfg=>tracePathRanks({api,level,prep,path:[0,1,2],scoringProfile:{},configOverride:cfg}),scoreFlags:['SCORE_X'],normalizeConfig:x=>x});
assert.equal(ab.flags[0].delta,-2);assert.equal(compareAblations(ab,{...ab,flags:[{flag:'SCORE_X',delta:0}]})[0].differential,2);
const invalid=tracePathRanks({api,level,prep,path:[0,9],scoringProfile:{}});assert.match(invalid.error,/not a candidate/);assert.equal(comparePathTraces(invalid,hard).valid,false);
