...
/** Historical existential capability candidate, excluding research-only search modalities. */
export function isDeterministicProductionContextEvidence(entry) {
    if (classifyProvenanceClass(entry, { standard: 'strict' }) !== 'cold-capability') return false;
    const facets = provenanceFacets(entry);
    return !facets.has('randomized') && !facets.has('complete-enumeration');
}
...
export function classifyEvidenceApplicability(entry, purpose, {
    ...
    if (entry?.search?.randomSeed !== null && entry?.search?.randomSeed !== undefined) {
        return { applicability: 'inadmissible', reason: 'randomized-research-context' };
    }
    if (entry?.search?.termination === 'exhaustive') {
        return { applicability: 'inadmissible', reason: 'enumeration-context' };
    }
    if (!isDeterministicProductionContextEvidence(entry)) {
