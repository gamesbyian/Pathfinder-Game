 ...
 * fixed set of elites, using the same cpsat-reference-probe.py reference model and native-prefix replay/referee
...
function referenceProbe(elite, depth) {
    ...
    const row = { depth, prefix, referenceLabel: classified.label, referenceReason: classified.reason, cpSatStatus: classified.status ?? null };
...
for (const eliteId of eliteIds) {
    ...
    const lo = elite.commonPrefixSteps - 1; // known-feasible depth, no reference call needed
    ...
    let probe = referenceProbe(elite, high);
    trace.push(probe);
    console.error(`  depth=${high} (full): ${probe.referenceLabel} (${probe.referenceReason})`);
    if (probe.referenceLabel === 'live') {
    ...
    if (probe.referenceLabel !== 'dead') {
        results[eliteId] = { elite, low, high, trace, note: `abstain at full length (${probe.referenceReason}); cannot bisect` };
        continue;
    }
    // Bisect.
    let abstained = false;
    while (high - low > 1) {
        ...
        const r = referenceProbe(elite, mid);
        trace.push(r);
        console.error(`  depth=${mid}: ${r.referenceLabel} (${r.referenceReason})`);
