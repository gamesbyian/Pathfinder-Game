#!/usr/bin/env python3
"""Solution-DENSITY probe. Counts solutions of the CORE RELAXATION (no must-cross / turn landmarks
/ portals / flipping filters), so the count is an UPPER BOUND on the level's true solution count.
That direction is what makes it useful: if even the relaxed count is small on a level our solver
cannot solve, the level is provably a rare-solution instance and no amount of heuristic tuning was
ever going to find it. Reports EXACT when enumeration finished, CAPPED when it hit the cap, and
TIMEOUT otherwise -- a timeout means "at least this many", not "this many"."""
...
class Counter(cp_model.CpSolverSolutionCallback):
    ...
    def on_solution_callback(self):
        self.n += 1
        if self.n >= CAP: self.StopSearch()
solver = cp_model.CpSolver()
solver.parameters.max_time_in_seconds = time_limit
solver.parameters.enumerate_all_solutions = True
solver.parameters.num_search_workers = 1   # enumeration requires a single worker
cb = Counter()
t0 = time.time()
...
exhausted = (name == 'OPTIMAL' and cb.n < CAP)   # search finished: the count is EXACT
print(f'{level_id}\treqLen={L}\treqInt={req_int}\tcells={len(grid)}\t'
      f'solutions={cb.n}{"" if exhausted else "+"}\t{"EXACT" if exhausted else ("CAPPED" if cb.n>=CAP else "TIMEOUT")}\t{el:.1f}s')
