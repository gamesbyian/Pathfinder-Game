 ...
 * heavy routing regime AND mustPass >= POLICY.OBJECTIVE_HEAVY_MUSTPASS AND flippers >=
 * POLICY.FLIPPER_HEAVY), the routing gate STRATEGY_MUSTCROSS_FLIPPER_WIDE_BEAM_EXPOSURE extends.
 *
 * Structural-feature-only: computes normalizeRawLevel()+classifyRoutingRegime() on each row and counts
...
// select-routing-regime-sample.mjs's own header comment), so ROOT is one `..` from the running file's
...
const { normalizeRawLevel, classifyRoutingRegime } = SOLVER_TESTING_API;
...
levels.forEach((raw, i) => {
    ...
    routingRegimeCounts.set(routingRegime, (routingRegimeCounts.get(routingRegime) ?? 0) + 1);
...
console.log(`Routing-regime distribution across ${levels.length} levels: ${[...routingRegimeCounts.entries()].map(([a, c]) => `${a}=${c}`).join(', ')}`);
console.log(`must-cross-heavy: ${routingRegimeCounts.get('must-cross-heavy') ?? 0} / ${levels.length}`);
...
if (outFile) {
    ...
    writeFileSync(outPath, rows.map(r => `${r.id}\t${r.routingRegime}\tmustPass=${r.mustPass}\tflippers=${r.flippers}\teligible=${r.eligible}`).join('\n') + '\n');
    console.log(`Wrote ${rows.length} rows (id/routing-regime/features, for review only) to ${outFile}`);
}
