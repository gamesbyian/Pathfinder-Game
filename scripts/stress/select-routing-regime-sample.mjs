 ...
 * routing regime matches one of the rules the change touches (attempts.ts routes purely on
...
const seedStr = argMap.get('--seed') || process.env.GITHUB_SHA || 'routing-regime-sample';
...
levels.forEach((raw, i) => {
    ...
    const entry = { id: raw.id, pos: i + 1, routingRegime };
    (routingRegimes.has(routingRegime) ? eligible : ineligible).push(entry);
});
...
console.log(`Routing-regime distribution across ${levels.length} levels: ${[...routingRegimeCounts.entries()].map(([regime, count]) => `${regime}=${count}`).join(', ')}`);
console.log(`Eligible population (routing regime in {${[...routingRegimes].join(', ')}}): ${eligible.length} / ${levels.length}`);
...
writeFileSync(idsPath, combined.map(e => `${e.id}\t${e.routingRegime}`).join('\n') + '\n');
console.log(`Wrote ${combined.length} position tokens to ${outFile}`);
console.log(`Wrote ${combined.length} ids+routing-regimes (for review only) to ${idsPath}`);
