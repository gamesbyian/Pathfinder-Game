#!/usr/bin/env node
/** Execute exact prefix-feasibility cases through the existing cpsat-reference-probe.py model.
 * This is an execution seam, not a new reference model: it supplies explicit --prefix cases, preserves
...
const outFile = args.get('--out') ?? 'reports/stress/cpsat-explicit-prefix-reference.json';
...
const document = {
    ...
    rows,
    caution: 'CP-SAT labels are reference-model evidence, not native-solver solves. Illegal prefixes, timeouts, unsupported mechanics, model errors, and referee-rejected SAT witnesses remain abstentions.',
};
