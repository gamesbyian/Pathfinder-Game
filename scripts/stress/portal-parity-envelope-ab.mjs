...
for (const entry of levels) {
    ...
    const onResult = await Solver.solve(level, { ablation: { PRUNE_PORTAL_PARITY_ENVELOPE: true }, nodeBudget: NODE_BUDGET, timeBudgetMs: WALL_MS, disableExtraBudgetPasses: true });
    const offResult = await Solver.solve(level, { ablation: { PRUNE_PORTAL_PARITY_ENVELOPE: false }, nodeBudget: NODE_BUDGET, timeBudgetMs: WALL_MS, disableExtraBudgetPasses: true });
