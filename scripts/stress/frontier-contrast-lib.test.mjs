...
describe('frontier contrast helpers', () => {
    it('reports numeric effects in frontier-minus-control direction', () => {
        const control = [{ x: 1 }, { x: 3 }];
        const frontier = [{ x: 5 }, { x: 7 }];
        const effect = numericEffect(control, frontier, item => item.x, 'x');
    ...
    it('reports binary rate differences and continuity-corrected odds ratios', () => {
        const control = [{ yes: true }, { yes: false }];
        const frontier = [{ yes: true }, { yes: true }];
        const effect = binaryContrast(control, frontier, item => item.yes, 'yes');
    ...
    it('builds the full class-control contrast with static, routing, telemetry, and category outputs', () => {
        const control = [
