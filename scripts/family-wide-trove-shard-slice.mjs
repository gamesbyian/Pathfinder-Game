#!/usr/bin/env node
/**
 * Slices data/families/wide-trove-manifest.json into N shards by index modulo, for the
 * family-wide-trove.yml workflow. Prints the shard's full manifest entries as a JSON array
 * (id, corpus, corpusPath, modes, group) -- unlike family-census-shard-ids.mjs's bare id list,
 * the shard script here needs per-level mode/group assignment, not just an id.
 *
 * Usage: node scripts/family-wide-trove-shard-slice.mjs --manifest=<file> --shard=<1-based> --shards=<n>
 */
