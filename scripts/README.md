...
- research composition: `research:dossier -- --question-id=<id>` (stable question/premise/MO refs and authored multi-asset joins when available); cross-family and reverse traversal: `research:query -- --entity=<type:id> [--direction=in|out|both] [--depth=N]`; lower-level `research:relations -- --list [--discover]`; lineage: `research:record-consumption`; integrity: `research:integration-audit`;
