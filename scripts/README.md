...
- completed GHA solver/research runs: `npm run gha:fetch-result -- --run=<run-id>`; use `--workflow=<name>` when the run id is unknown, `--json` for the manifest, or `--out=<dir>` to retain the standard result. Do not enumerate shards for a normal successful run.
