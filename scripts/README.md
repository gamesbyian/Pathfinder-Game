...
- research composition: `research:dossier -- --question-id=<id>` (stable question/premise/MO refs and authored multi-asset joins when available); cross-family/reverse traversal and semantic views: `research:query -- --entity=<type:id> [--direction=in|out|both] [--depth=N]`, `--view=answerability`, `--view=impact --entity=<type:id>`, `--view=coverage`, `--view=system-findings`, plus `--snapshot` / `--compare-snapshot=<file>`; queryability regression surface: `research:queryability-audit`; lower-level `research:relations -- --list [--discover]`; lineage: `research:record-consumption`; integrity: `research:integration-audit`;
