...
const provenance = { seed: 'seed-1', selectionVersion: 'v1', corpusHash: 'corpus', baseSha: 'abc', controlConfig: { flag: false }, treatmentConfig: { flag: true }, controlConfigHash: 'control', treatmentConfigHash: 'treatment', envelope: { nodes: 50e6, work: 67e6 }, stageOrder: ['late-repair-search', CLASS2_TARGET_STAGE] };
...
const arm = (treatment, gain = true) => manifest.entries.map((entry, i) => ({ id: entry.id, configHash: treatment ? 'treatment' : 'control', ok: treatment && gain && i === 0, refereeValid: treatment && gain && i === 0, winningStage: treatment && gain && i === 0 ? CLASS2_TARGET_STAGE : null, workSpent: treatment ? 110 : 100, stageLifecycle: treatment ? lifecycle(CLASS2_TARGET_STAGE) : {} }));
const control2 = arm(false), treatment2 = arm(true);
assert.equal(analyzeClass2Economics({ manifest, controlRows: control2, treatmentRows: treatment2 }).decision, 'advance');
