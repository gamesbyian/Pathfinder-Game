...
for (const required of ['git', 'rev-parse', 'HEAD', 'inferredPairedArms', 'immutable 40-character commit SHAs']) {
  if (!contractWriter.includes(required)) failures.push(`experiment contract writer lacks immutable execution identity guard: ${required}`);
}
