...
import {
  ...
  validateWs2FailureResponseAnalysisEnvelope,
...
const contractPath = analysis?.analysisContract?.path;
if (typeof contractPath !== 'string' || !contractPath || !existsSync(contractPath)) {
  throw new Error('analysis references a missing frozen analysis contract');
}
...
validateWs2FailureResponseAnalysisEnvelope(analysis, {
  contractIdentity: ws2FailureResponseAnalysisContractIdentity(contract),
});
