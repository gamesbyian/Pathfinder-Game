...
const rows = scriptFiles.map(file => {
    ...
    const orphanCandidate = kind === 'entrypoint' && !surfaced && codeRefs.length === 0;
    return {
        file,
        kind,
...
const result = { schemaVersion: 1, generatedAt: new Date().toISOString(), summary, rows: selected };
...
console.log(`Tooling census: ${summary.trackedScripts} tracked script files; ${summary.entrypoints} executable-looking entrypoints; ${summary.surfacedEntrypoints} surfaced; ${summary.orphanCandidates} orphan candidates.`);
console.log('');
...
for (const row of selected) {
    ...
    console.log(`${signals} ${row.kind.padEnd(10)} ${age.padStart(5)}  ${row.file}${marker}`);
}
