...
export const FEATURES = {
    ...
    STRATEGY_REPAIR_PROBE_ADAPTIVE_BIASED_BUDGET: 'production default-OFF; PILOT, not yet promoted (2026-08-12): scales the repair probe\'s biased-tier node budget down when the ordinary tier\'s own live bestBadness evidence (current-invocation only, no exact-level history) shows no sign repair is close, freeing shared mainLoopEarlyNodeBudget headroom for the early main-loop configs the probe otherwise runs ahead of unconditionally. A single-signal, single-recipient instance of online failure-conditioned allocation (docs/future-work.md item #4, docs/solver-interoperability-and-cooperation-plan.md §17) -- motivated by tracing a since-refined starvation hypothesis to its real mechanism (the probe and early main-loop configs share one unprotected pool; the late-reserve/admissible-order reserves are NOT actually reachable by the probe, contrary to the hypothesis that first suggested this). Calibrated from an n=12 local sample only (n=1 for the "needs full budget" case) -- needs its own dedicated population A/B before promotion. See reports/2026-08-12-repair-probe-early-main-loop-starvation.md and REPAIR_PROBE_ADAPTIVE_BIASED_BADNESS_GATE\'s own comment in orchestration.ts.',
...
export const OPT_IN_FEATURES = new Set([
    ...
    'STRATEGY_REPAIR_TURN_BIAS',
    'STRATEGY_REPAIR_PROBE_ADAPTIVE_BIASED_BUDGET',
]);
