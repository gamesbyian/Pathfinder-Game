...
export const FEATURES = {
    ...
    STRATEGY_ADMISSIBLE_ORDER_PROFILE_NODE_RESERVE: 'production default-OFF; NEW, unvalidated (landed 2026-08-13): withholds a slice of the admissible-order tier\'s own node reserve (ADMISSIBLE_ORDER_PROFILE_NODE_RESERVE_FRACTION, orchestration.ts) from the tier\'s dominant \'default\' profile (ADMISSIBLE_ORDER_PROFILES, attempts.ts), which runs first and can otherwise consume the tier\'s entire pool before \'none\'/\'mustCrossFirst\'/\'intersectionHarvest\'/\'nearClosureRescue\' ever get a node -- a documented real regression, not a hypothesis: reports/2026-07-30-admissible-order-node-reserve.md found R03148 solved by \'none\' at 1.97M nodes with the tier reserve OFF, but \'default\' ate the whole 20M slice and \'none\' never ran when it was ON. ASYMMETRIC RISK unlike this session\'s two prior reserves: \'default\' is the tier\'s dominant contributor (103 of 115 validated solves; 21 of 22 measured gains in that report), so a careless reserve here can trade a large proven win for a small speculative one -- that report explicitly declined to build this fix for exactly that reason. Needs its own dedicated A/B, specifically checking \'default\'-winning levels are unaffected, before promotion.',
