...
export const FEATURES = {
    ...
    STRATEGY_DEDUP_NEAR_TIE_RETRY: 'production default-OFF; NEW/UNVALIDATED (2026-08-15): last-resort whole-ladder rerun with STRATEGY_DEDUP_NEAR_TIE_RETENTION disabled, tried only after the main loop AND repair fallback have already failed on every gate -- same shape as STRATEGY_ATTRACTION_DIVERSITY, toggling a different flag. Exists to recover the 34 Corpus-2 losses STRATEGY_DEDUP_NEAR_TIE_RETENTION\'s own full-corpus A/B found (see that flag\'s own comment) without giving back its 27 gains: those gains solve via the main ladder with retention ON and never reach this tier, while the losses solve cheaply (median 6.5M nodes) without retention and should be recoverable here. Not yet validated at population scale -- carries its own node reserve (DEDUP_NEAR_TIE_RETRY_NODE_RESERVE_FRACTION, orchestration.ts) sized from the loss population\'s own cost distribution (p90 8.2M of 50M), since a late tier without one is reliably starved (see ADMISSIBLE_ORDER_NODE_RESERVE_FRACTION\'s own history of this exact mistake).',
