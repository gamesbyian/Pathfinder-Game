...
export const FEATURES = {
    ...
    STRATEGY_ADMISSIBLE_ORDER_NON_DEFAULT_RETRY: 'production default-OFF; NEW/UNVALIDATED (2026-08-15, built the same day as STRATEGY_DEDUP_NEAR_TIE_RETRY and directly modeled on it): applies that tier\'s validated "run dead last, additive-only budget" pattern to ADMISSIBLE_ORDER_PROFILE_NODE_RESERVE_FRACTION\'s own known double-edged shape -- that reserve recovers R03148 (\'none\' solves it at 1.97M nodes reserve-off, never runs reserve-on since \'default\' eats the whole pool) but turns R02644 from SOLVED to unsolved at the same fraction, because \'default\' there genuinely needed the room the reserve shrinks. Instead of shrinking \'default\'\'s ceiling in the tier\'s own unreserved pass (so R02644-shaped levels are fully unaffected), this tier reruns ONLY the non-\'default\' profiles afterward, dead last (after even STRATEGY_DEDUP_NEAR_TIE_RETRY), with a fresh additive node ceiling (ADMISSIBLE_ORDER_NON_DEFAULT_RETRY_NODE_RESERVE_FRACTION, orchestration.ts) and a fresh additive prep._workCap override -- necessary even though this tier calls runAttempt directly rather than through the shared-pool runInterleavedAttempts/runGateSerialAttempts machinery dedup-retry\'s own work-starvation bug came from, since prep._workCap is a single mutable field neither function resets fresh for a runAttempt-direct caller this late in the ladder. Not yet locally or population validated -- needs the same rigor STRATEGY_DEDUP_NEAR_TIE_RETRY went through (spot-check R03148 recovery + R02644 non-regression, then a population-scale GHA A/B) before any promotion decision. See reports/2026-08-15-connectivity-axis-exhausted-regression.md and reports/2026-07-30-admissible-order-node-reserve.md.',
