...
export const FEATURES = {
    ...
    STRATEGY_MC_NEIGHBOR_BUDGET_RETRY: 'production default-OFF (NEW, 2026-08-19): applies the validated "run dead last, additive-only budget" pattern (STRATEGY_DEDUP_NEAR_TIE_RETRY / STRATEGY_ADMISSIBLE_ORDER_NON_DEFAULT_RETRY / STRATEGY_CONNECTIVITY_AXIS_EXHAUSTED_RETRY) to a FOURTH known double-edged mechanism: PRUNE_MC_NEIGHBOR_BUDGET. That prune was promoted default-ON on a strong population result (611/1700 OFF -> 665/1700 ON, 59 gained / 5 lost), and its five losses were individually diagnosed in reports/2026-08-12-neighbor-budget-five-loss-diagnosis.md: the exact deterministic beam attempt that wins under OFF is still tried under ON, runs to a similar node count, and fails -- a bounded-width diverse-beam effect, not budget exhaustion. Three of those five (R00635, R02823, R02867) have since been recovered by unrelated solver work; R02119 and R02422 remain unsolved at the 2026-08-16 capability baseline (run 31918095910, 819/1700) and BOTH recover at HEAD when the prune is disabled -- referee-valid, via exactly the winning configs that diagnosis named (beam:mustCrossFirst@beam2000 and beam:intersectionHarvest@beam5000(diverse)). Reruns the same mainConfigs ladder as its three sibling tiers with PRUNE_MC_NEIGHBOR_BUDGET disabled via a Proxy override, a fresh additive node ceiling stacked on the preceding tier\'s own ceiling (MC_NEIGHBOR_BUDGET_RETRY_NODE_RESERVE_FRACTION, orchestration.ts), and a fresh additive work allocation. Unlike its three siblings it also carries a SOUNDNESS-BASED eligibility gate: prune-gauntlet.ts only ever reaches this prune when state.mustCrossMask !== 0, so on a level with prep.initialMustCrossMask === 0 the rerun would be bit-identical to the ladder that already ran and is skipped outright -- free on 44% of the unsolved Corpus-2 population (389 of 881), which is the point: the three shipped tiers pay their cost unconditionally and the last of them cost +28.2% corpus-2 nodes for +10 solves. Levels the prune HELPS are structurally protected the same way R02644/R03248 were for the two prior tiers: they already solve via the normal flag-on ladder, so this tier\'s own !result.solution guard skips them entirely. NOT yet population-validated -- opt-in until a full-corpus A/B measures gained/lost IDs and cost on both corpora.',
...
export const OPT_IN_FEATURES = new Set([
    ...
    'STRATEGY_MC_NEIGHBOR_BUDGET_RETRY',
