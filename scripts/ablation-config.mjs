...
export const FEATURES = {
    ...
    STRATEGY_REPAIR_FALLBACK_NODE_RESERVE: 'production default-OFF; NEW, unvalidated (landed 2026-08-13): withholds a slice of earlyTierNodeBudget (REPAIR_FALLBACK_NODE_RESERVE_FRACTION, orchestration.ts) from the probe and the WHOLE main loop, protecting the repair fallback loop (and, as a side effect, whatever the attraction-diversity pass finds still available) from the main loop\'s own consumption -- same mechanism as ADMISSIBLE_ORDER_NODE_RESERVE_FRACTION, one tier boundary further down the ladder. Confirmed on an n=8 local repair-gated Corpus-2 sample (15,000,000-node budget, workers=1): 5/6 unsolved levels gave the repair fallback loop ZERO attempts and all 6 gave the attraction-diversity pass ZERO attempts, while the admissible-order tier (which already has its own reserve) got its own slice on every one of the same 6 levels -- same clean before/after control. 0.15 is a starting point borrowed from MAIN_LOOP_LATE_RESERVE_FRACTION\'s own validated magnitude, not derived from any A/B on this specific mechanism. Needs its own dedicated A/B before promotion -- do not promote on the n=8 spot-check evidence alone.',
