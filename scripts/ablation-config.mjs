...
export const FEATURES = {
    ...
    STRATEGY_REPAIR_LATE_PROBE: 'PROMOTED to production default-ON (2026-08-20/21). Priority 7 (docs/solver-optimization-current-queue.md): needsRepairFallback structurally excludes 94 of 158 currently-unsolved Corpus-2 levels where repair wins in the census (18 of those in <=1,000,000 nodes), and widening that gate directly was rejected -- a matched-comparison feature analysis found no single/pair feature cleanly separates the winning population from the much larger ineligible population that never wins. This tier sidesteps the "which levels" question by trying a single plain repair attempt on every level repairConfigs left empty, but ONLY dead last, after the entire rest of the ladder (including STRATEGY_MC_NEIGHBOR_BUDGET_RETRY, the current true end) has already failed -- unlike the existing early repair PROBE (runRepairProbe), which already documented the cost of running unconditionally before the main loop ("burns its FULL node budget as pure dead search every single solve" on a level that never succeeds, confirmed on R02401, ~10.7s of unconditional overhead per solve). Dead-last placement means a level that solves via any earlier technique never reaches this tier at all, so it costs nothing there regardless of budget size; the cost lands only on levels already reporting unsolved. Sized as a flat REPAIR_LATE_PROBE_NODE_BUDGET (2,000,000 nodes, orchestration.ts) -- not a fraction of nodeBudget/timeBudgetMs like the five whole-ladder-rerun tiers above it, since this tier is meant to be cheap and tightly bounded, not thorough (mirroring REPAIR_PROBE_ORDINARY_NODE_BUDGET\'s own flat-constant shape). Validated end-to-end against the real production ladder (not the isolated census attempt): all 94 gate-excluded levels tested with the flag on, 20 net new recoveries (21.3%), all referee-valid, zero marginal cost on repair-eligible controls, solver:bench --check byte-identical (160/160). Population-confirmed via GHA run 32418694112 (level-blind capability refresh, deterministic=true): Corpus-2 +20 gained / 0 lost relative to the shared baseline (ids R00347/R00373/R00477/R01799/R02097/R02159/R02186/R02259/R02348/R02495/R02573/R02760/R02816/R02884/R03020/R03079/R03119/R03122/R03245/R03252), matching the local result exactly. That GHA dispatch predates the same-day prep._workCap staleness fix and the ablated-repairConfigs eligibility guard (orchestration.ts); both are no-ops for this measurement\'s own config (24h non-binding deadline, 50,000,000 node cap, no STRATEGY_REPAIR_FALLBACK ablation), so neither fix could have changed this result. See docs/solver-opt-in-experiment-ledger.md for the full disposition record.',
