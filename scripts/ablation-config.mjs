...
export const FEATURES = {
    ...
    STRATEGY_ATTRACTION_DIVERSITY_NODE_RESERVE: 'production default-OFF; NEW, unvalidated (landed 2026-08-13, same day as and directly motivated by STRATEGY_REPAIR_FALLBACK_NODE_RESERVE\'s close-out): withholds a slice of the main loop\'s late-suffix reserve specifically FROM the repair fallback loop (which the sibling flag\'s close-out proved always burns its full ceiling), protecting the attraction-diversity pass\'s own room (ATTRACTION_DIVERSITY_NODE_RESERVE_FRACTION, orchestration.ts). Unlike the repair fallback loop, the diversity pass is a full deterministic DFS/beam ladder rerun (not a plateau-prone randomized restart search) built for a different, already-documented failure mode (self-defeating attraction-scoring commitments) -- a priori more promising than the receptor just closed, but unvalidated. Needs its own dedicated A/B before promotion.',
