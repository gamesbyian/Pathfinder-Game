...
export const FEATURES = {
    ...
    STRATEGY_REPAIR_ELITE_PREFIX_DFS_RETRY: 'production default-OFF; NEW/UNVALIDATED (2026-08-16, built the same day as the three promoted retry tiers above and directly modeled on them, but applied to a DIFFERENT known double-edged mechanism): applies the "run dead last, additive-only budget" pattern to STRATEGY_REPAIR_ELITE_PREFIX_DFS itself. That mechanism (reports/2026-08-07-repair-elite-prefix-dfs.md) is sound and mechanistically real (confirmed badness-improvement feedback loop) but net-negative in its own 20-level A/B (4/20 vs 5/20 with it off) due to ONE confirmed cause: R02239 solves via ordinary repair at 14,194,203 nodes with the mechanism off, but exhausts the SAME repair call\'s own 15,000,000-node budget without solving when it\'s on -- the identical shared-node-budget displacement shape the three tiers above were each built to fix, which the original report\'s own "what would need to change" section never considered fixing this way. Reruns repairConfigs via the same per-config/per-gate manual loop shape as the ordinary repair fallback loop, with prep._cfg Proxy-overridden to force STRATEGY_REPAIR_ELITE_PREFIX_DFS: true (the OPPOSITE polarity from the three tiers above, which each disable a flag), a fresh additive node ceiling stacked on STRATEGY_CONNECTIVITY_AXIS_EXHAUSTED_RETRY\'s own ceiling (REPAIR_ELITE_PREFIX_DFS_RETRY_NODE_RESERVE_FRACTION, orchestration.ts), and a fresh additive prep._workCap override. R02239 is structurally protected the same way R02644/R03248 were for the other two tiers: the ordinary repair fallback loop above already ran with the flag off and its own untouched budget, so a level that solves there is unaffected regardless of what this tier does afterward. Not yet locally or population validated -- needs the same rigor the three tiers above went through (spot-check R02239 recovery + no regressions on the rest of the original 20-level sample, then a population-scale GHA A/B) before any promotion decision. See reports/2026-08-07-repair-elite-prefix-dfs.md and reports/2026-08-15-connectivity-axis-exhausted-regression.md.',
