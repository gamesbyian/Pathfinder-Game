#!/usr/bin/env node
/**
 * Stress-corpus benchmark using solveLevelRaced (worker-thread attempt racing) instead of the
 ...
 * before the next level starts) — this is orthogonal to, and does not combine with,
 * stress:benchmark's own --parallel flag (which parallelizes ACROSS levels instead of within
 * one level's attempt ladder).
 *
...
import { solveLevelRaced } from './race.mjs';
...
for (const entry of levels) {
    ...
    let result;
    try {
        result = await solveLevelRaced(raw, { timeBudgetMs: BUDGET_MS, poolSize: POOL_SIZE });
    } catch (err) {
