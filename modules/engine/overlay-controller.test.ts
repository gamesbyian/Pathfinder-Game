...
const createHarness = () => {
  ...
  state.engineState.isDirty = false;
  const ui = {
...
test('setOverlayState updates overlay, invalidates render, and applies UI effects', () => {
  ...
  assertEqual(state.engineState.overlayState, HINT_ANIMATING, 'overlay state should update');
  assertEqual(state.engineState.isDirty, true, 'overlay change should mark dirty');
...
test('leaving hint animation resets animation alpha and pin UI', () => {
  const { calls, state, controller } = createHarness();
  state.engineState.overlayState = HINT_ANIMATING;
  state.engineState.hinter.alpha = 1;
  state.engineState.hinter.persistedPath = [1, 2, 3] as any;
  controller.setOverlayState(OVERLAY_NONE);
  assertEqual(state.engineState.hinter.alpha, 0, 'leaving hint animation should reset alpha');
...
test('startHintAnimation starts only when hint paths exist', () => {
  ...
  assertEqual(state.engineState.overlayState, OVERLAY_NONE, 'empty hint list should not start animation');
  state.engineState.hinter.pathList = [[1, 2], [3, 4]] as any;
  state.engineState.hinter.currentPathIdx = 1;
  controller.startHintAnimation();
  assertEqual(state.engineState.overlayState, HINT_ANIMATING, 'hint paths should start animation');
  assertEqual(state.engineState.hinter.alpha, 1, 'start should reset alpha to visible');
  assertEqual(state.engineState.hinter.index, 0, 'start should reset animation index');
...
test('pin and clear persisted hint route through state and UI effects', () => {
  const { calls, state, controller } = createHarness();
  state.engineState.overlayState = HINT_ANIMATING;
  state.engineState.hinter.pathList = [[7, 8]] as any;
  controller.pinCurrentHint();
  assertArrayEqual(state.engineState.hinter.persistedPath, [7, 8], 'pin should persist current hint path');
  assertEqual(state.engineState.overlayState, OVERLAY_NONE, 'pin should close hint overlay');
  ...
  assertArrayEqual(state.engineState.hinter.persistedPath, [], 'clear should remove persisted hint path');
  assertEqual(state.engineState.hinter.persistedHintIdx, -1, 'clear should reset persisted hint index');
...
test('stopHintAnimation preserves source while clearing transient paths', () => {
  ...
  state.engineState.overlayState = HINT_ANIMATING;
  state.engineState.hinter.pathList = [[1, 2]] as any;
  state.engineState.hinter.source = 'solver';
  state.engineState.hinter.alpha = 1;
  controller.stopHintAnimation();
  assertArrayEqual(state.engineState.hinter.pathList, [], 'stop should clear transient paths');
  assertEqual(state.engineState.hinter.source, 'solver', 'stop should preserve hint source');
  assertEqual(state.engineState.hinter.alpha, 0, 'stop should reset animation alpha');
  assertEqual(state.engineState.overlayState, OVERLAY_NONE, 'stop should close overlay');
});
