...
test('complete mode enumerates every solution and reports exhausted', () => {
    const { level, prep } = tinyLevel();
    const sols: number[][] = [];
    const res = enumerateFromGate(level, prep, PACK(0, 0), { onSolution: p => sols.push(p), rng: null, nodeBudget: Infinity });
    ...
    const again: number[][] = [];
    enumerateFromGate(level, prep, PACK(0, 0), { onSolution: p => again.push(p), rng: null });
    assert.deepEqual(new Set(again.map(p => p.join(','))), new Set(sols.map(p => p.join(','))));
});

test('a node budget stops early and reports NOT exhausted', () => {
    const { level, prep } = tinyLevel();
    const res = enumerateFromGate(level, prep, PACK(0, 0), { onSolution: () => {}, rng: null, nodeBudget: 3 });
    assert.equal(res.exhausted, false, 'cut short by the node budget');
});

test('shouldStop halts enumeration and reports NOT exhausted', () => {
    const { level, prep } = tinyLevel();
    let seen = 0;
    const res = enumerateFromGate(level, prep, PACK(0, 0), {
...
test('random order finds the same solution SET as deterministic order', () => {
    const { level, prep } = tinyLevel();
    const detSet = new Set<string>();
    enumerateFromGate(level, prep, PACK(0, 0), { onSolution: p => detSet.add(p.join(',')), rng: null });
    let a = 123456;
    const rng = () => { a = (a * 1103515245 + 12345) & 0x7fffffff; return a / 0x7fffffff; };
    const randSet = new Set<string>();
    const res = enumerateFromGate(level, prep, PACK(0, 0), { onSolution: p => randSet.add(p.join(',')), rng, nodeBudget: Infinity });
    assert.equal(res.exhausted, true);
    assert.deepEqual(randSet, detSet, 'randomization changes order, not the complete result');
});

test('anchoredFromSeed completes a known prefix and stays consistent with it', () => {
    const { level, prep } = tinyLevel();
    // Pick a concrete solution as the seed: right, right, up, up.
    const seed = [PACK(0, 0), PACK(1, 0), PACK(2, 0), PACK(2, 1), PACK(2, 2)];
    const sols: number[][] = [];
    const res = anchoredFromSeed(level, prep, seed, 2, { onSolution: p => sols.push(p), rng: null });
    assert.equal(res.exhausted, true);
...
test('completeFromState works from an arbitrary positioned state', () => {
    const { level, prep } = tinyLevel();
    const state = createState(PACK(0, 0), level, prep);
    const sols: number[][] = [];
    const res = completeFromState(level, prep, state, { onSolution: p => sols.push(p), rng: null });
    assert.equal(res.exhausted, true);
