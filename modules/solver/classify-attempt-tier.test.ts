...
test('classifyAttemptTier reads stageId first when present — legacy booleans never override it', () => {
    ...
    const conflicting = { stageId: 'admissible-order' as const, repairLateProbe: true, repair: true, admissibleOrder: false };
    assert.equal(classifyAttemptTier(conflicting), 'admissible-order');
});
...
test('classifyAttemptTier maps every canonical stageId to its own label (or the two documented renames), never the legacy fallthrough default', () => {
    // main-loop and repair-probe-shrink-recovery are the two stages whose label differs from their
    // own stageId — see STAGE_ID_TO_TIER_LABEL's own comment. Every other stageId is its own label.
    const renamed: Partial<Record<string, string>> = { 'main-loop': 'main-ladder', 'repair-probe-shrink-recovery': 'repair-probe' };
    for (const stageId of SOLVER_STAGE_IDS) {
        if (stageId === 'prime' || stageId === 'portfolio-pass' || stageId === 'portfolio-fallback') continue;
