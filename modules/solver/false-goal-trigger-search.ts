...
type OnTriggerableCellFn = ((key: number) => void) | null;
/** A false-goal-trigger-search DFS frame. */
...
async function dfsEnumerateTriggerableFalseGoalCells(
    startKey: number, level: NormalizedLevel, prep: PrepLevel,
    budgetMs: number, startTime: number, triggerableCells: Set<number>, yieldFn: YieldFn, onTriggerableCell: OnTriggerableCellFn,
): Promise<boolean> {
    const recordTriggerableCell = (key: number) => {
        if (triggerableCells.has(key)) return;
        triggerableCells.add(key);
        if (onTriggerableCell) onTriggerableCell(key);
    };
    ...
    const stack: FalseGoalTriggerFrame[] = [{ key: startKey, children: getNeighbors(startKey, state, level, prep), childIdx: 0, undoChain: [] }];
...
export interface FalseGoalTriggerSearchProgress { gatesProcessed: number; gatesCompleted: number; totalGates: number; triggerableCells: number; }

export type FalseGoalTriggerSearchOptions = {
    timeLimitMs?: number;
    yieldFn?: (() => Promise<void>);
    onProgress?: (p: FalseGoalTriggerSearchProgress) => void | Promise<void>;
    ...
    onTriggerableCell?: (key: number) => void;
};
...
export function isParityCompatibleEndpoint(level: NormalizedLevel, key: number): boolean {
...
export type FalseGoalTriggerability = 'triggerable' | 'untriggerable' | 'unknown';

// Classifies each of the level's placed false goals against a false-goal-trigger-search result.
//  - 'triggerable':   the cell is a valid triggerable false-goal cell (a path can end there).
//  - 'untriggerable': provably can never be triggered — either the search fully
...
export function classifyFalseGoalTriggerability(
    ...
    const out = new Map<number, FalseGoalTriggerability>();
    const searchComplete = result.status === 'complete' && result.gatesCompleted >= result.totalGates;
    for (const fg of level.falseGoalKeys) {
        if (result.triggerableCells.has(fg)) { out.set(fg, 'triggerable'); continue; }
        if (!isParityCompatibleEndpoint(level, fg)) { out.set(fg, 'untriggerable'); continue; }
        out.set(fg, searchComplete ? 'untriggerable' : 'unknown');
    }
    return out;
}

// Finds all valid false-goal-trigger positions across all gates.
//
...
export async function findTriggerableFalseGoalCells(
    level: NormalizedLevel,
    opts: FalseGoalTriggerSearchOptions = {},
): Promise<FalseGoalTriggerSearchResult> {
    const startTime = Date.now();
    const budgetMs = opts.timeLimitMs ?? 30000;
    const yieldFn = opts.yieldFn ?? null;
    const onProgress = opts.onProgress ?? null;
    const onTriggerableCell = opts.onTriggerableCell ?? null;
    const prep = prepLevel(level, { allowFalseGoalNeighbors: true });
    const triggerableCells = new Set<number>();
    const gates = level.gateKeys;
    const totalGates = gates.length;
    let gatesProcessed = 0;
    let gatesCompleted = 0;

    const finalize = (status: FalseGoalTriggerSearchResult['status']): FalseGoalTriggerSearchResult => ({
        status,
        triggerableCells,
        gatesProcessed,
        gatesCompleted,
    ...
    for (let gi = 0; gi < totalGates; gi++) {
        ...
        try {
            completed = await dfsEnumerateTriggerableFalseGoalCells(gates[gi], level, prep, gateBudget, now, triggerableCells, yieldFn, onTriggerableCell);
        } catch (err) {
        ...
        if (onProgress) {
            try { await onProgress({ gatesProcessed, gatesCompleted, totalGates, triggerableCells: triggerableCells.size }); }
            catch (err) {
                if ((err as { message?: string })?.message === 'Solver:cancelled') return finalize('aborted');
            }
        }
