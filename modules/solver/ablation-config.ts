...
export const FEATURES: Record<string, string> = {
    ...
    STRATEGY_REPAIR_FALLBACK_GATE_WIDEN: 'Production default-OFF; NEW unvalidated experiment (2026-08-23): widens attempts.ts\'s needsRepairFallback gate to unconditionally cover isHighInt(f) (dropping the VERY_HIGH_REQINT floor) and portal-heavy, matching the 55-row genuine-gap population docs/solver-optimization-current-queue.md Priority 7 identified but left unlanded pending a population-scale wall-time check (this gate also drives the unconditional early repair PROBE, not just the additive late fallback config — see needsRepairFallback\'s own comment). Do not promote without that A/B evidence.',
