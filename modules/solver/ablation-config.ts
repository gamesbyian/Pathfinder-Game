...
export const FEATURES: Record<string, string> = {
    ...
    SCORE_PORTAL_PARITY_GUIDANCE: 'Guidance toward a mismatched-parity portal when requiredLength parity requires one',
    SCORE_GOAL_ATTRACTION_GUIDANCE_DISTANCE: 'Production default-OFF; NEW unvalidated experiment (2026-08-23): makes SCORE_GOAL_ATTRACTION read prep.guidanceGoalDistArr (pre-6f00baf routing: geese/gates/false-goals treated as ordinary passable cells) instead of the corrected prep.goalDistArr, for move-ordering guidance only — pruning/admissible bounds are unaffected. See distance.ts\'s DistMapOpts.legacyGuidanceRouting and docs/solver-optimization-workstreams.md\'s "Distance-guidance/pruning split" entry. Do not promote without matched-work evidence that it recovers solves without new losses.',

    // ── Pruning rules (dfsFromGate + beamSearchFromGate) ─────────────────────
    PRUNE_MC_CEILING:           'Intersection ceiling: ints + pending-MC-crossings > requiredIntersections',
    ...
    STRATEGY_PORTAL_COARSE_STATE_MERGE_DEAD_LAST_RETRY: 'Production default-ON (promoted 2026-09-16): after every promoted retry fails on a portal level, rerun the clean main ladder with portal coarse-state merge enabled only inside a fresh additive retry scope.',
    STRATEGY_PORTAL_COARSE_STATE_MERGE_DEAD_LAST_RETRY_TREATMENT: 'Production default-ON (promoted 2026-09-16) arm selector. Read only by the enabled dead-last retry shell: false is the funded control retry, true enables portal coarse-state merge inside that retry. It never globally enables the merge.',
    ...
    STRATEGY_REPAIR_SHRINK_RECOVERY: 'Production default-OFF; retained opt-in: rerun an early-repair-search config at its full budget after adaptive biased-budget shrink withheld nodes. Current disposition: docs/solver-opt-in-experiment-ledger.md.',
    ...
    STRATEGY_PARITY_GATE_FILTER:'Pre-filter infeasible gates by exact-length parity (portal-free and same-parity-portal-only levels when prep is available)',
    STRATEGY_REPAIR_FALLBACK:   'Iterated-local-search repair fallback attempts (extra budget, after the main loop)',
    STRATEGY_GOAL_ATTRACTION_DISABLED_RETRY: 'Last-resort attempt with one attraction/position-scoring term disabled, after the main loop and repair fallback have both failed',
    ...
    STRATEGY_EARLY_REPAIR_SEARCH:      'Early small-budget repair probe before the main DFS/beam loop',
    STRATEGY_EARLY_REPAIR_SEARCH_MULTI_SEED: 'Retry the ordinary-tier repair probe across a few extra gate-derived PRNG seeds before falling through to the main loop',
    STRATEGY_EARLY_REPAIR_SEARCH_ADAPTIVE_BIASED_BUDGET: "Production default-ON: scale biased early-repair-search node budgets from the ordinary early-repair-search attempt's live bestBadness, bounded by the configured gate and minimum scale. Shrink recovery is separately gated.",
    ...
    STRATEGY_MAIN_SEARCH_LATE_RESERVE: 'Production default-ON: reserve a fixed main-search node slice for a late config suffix without reordering attempts (MAIN_SEARCH_LATE_RESERVE_FRACTION in orchestration.ts).',
    ...
    STRATEGY_GOAL_ATTRACTION_DISABLED_RETRY_NODE_RESERVE: 'Production default-ON: reserve part of the main-search late slice for goal-attraction-disabled-retry stage. Promoted 2026-09-10 as a validated PAIR with STRATEGY_GOAL_ATTRACTION_DISABLED_RETRY_FRESH_WORK_POOL (see that flag\'s own comment) — this flag\'s own node-dimension-alone evidence was previously closed as negligible, but every A/B in this line (development, confirmation-001, confirmation-002) ran it paired with the fresh work pool, never alone; the pair is what the evidence actually supports. Current disposition: docs/solver-opt-in-experiment-ledger.md.',
    STRATEGY_GOAL_ATTRACTION_DISABLED_RETRY_FRESH_WORK_POOL: 'Production default-ON: gives the goal-attraction-disabled-retry pass a fresh work pool (sized off diversityBudgetFraction of the solve\'s own workBudget) instead of sharing the outer, already-depleting pool with every earlier tier. Motivated by reports/2026-09-02-goal-attraction-disabled-retry-work-pool-starvation.md, which found the shared pool independently starves 64% of otherwise-eligible attempts even with the sibling node reserve on. Promoted 2026-09-10 after confirmation-002 (reach-conditioned, control-side-starvation-selected 150-level population, disjoint from every prior population this line touched): control (node reserve alone) 14/150 vs. treatment (node reserve + fresh work pool) 17/150 — +3/-0, control\'s solved set a strict subset of treatment\'s. All three gains (R01124, R02020, R02060) directly attributable to a winning goal-attraction-disabled-retry attempt (stage-level solve, not incidental). Tier reach rose 104/150 -> 140/150 between arms with real, substantial per-attempt work (not a non-participation artifact); zero errors, zero deadline truncation, zero node/work-budget asymmetry between arms. See reports/2026-09-05-goal-attraction-disabled-retry-fresh-work-pool-confirmation-002-preflight.md and docs/solver-opt-in-experiment-ledger.md.',
    ...
    STRATEGY_ADMISSIBLE_ORDER_NON_DEFAULT_RETRY_WORK_CAP_ENFORCEMENT: 'Production default-OFF, research-only: makes admissibleOrderSearch\'s hot loop additionally consult prep._workCap (mirroring admissibleOrderSearchLDS\'s existing check), but ONLY for the admissible-order-non-default-retry tier\'s own dispatch (attempt-dispatch.ts scopes this to attemptConfig.scoringProfileId !== \'default\', i.e. exactly admissibleOrderNonDefaultConfigs — the tier orchestration.ts:2479 already wraps in withWorkCapScope). The plain \'default\'-profile admissible-order tier and admissibleOrderSearchLDS are completely unaffected regardless of this flag. Exists solely to make the admissible-order-non-default-retry-budget-fraction 1.0->0.18 candidate testable at all: reports/2026-09-10-admissible-order-non-default-retry-matched-work-methodology-001.md found the fraction has NO code path capable of binding without this. Current disposition: docs/solver-opt-in-experiment-ledger.md.',
    ...
    STRATEGY_NEAR_HAMILTONIAN_INTERSECTION_HARVEST_MECHANIC_BUCKET_EXPOSURE: 'Production default-ON, promoted 2026-09-26: adds beam:intersectionHarvest@beam5000 with mechanicBucketRetention to attempts.ts\'s near-Hamiltonian intersection-heavy routing rule, reserve-preserving placement (before the protected trailing-5 window, not appended after it). Motivated by CID-0027 (data/stress/capability-invention-demand.json): R00118 (requiredIntersections=6, non-portal, near-Hamiltonian path coverage) is solved by exactly this action in EW1 isolation, but production only offered the plain-retention sibling at the same score/width via this rule. Pilot+confirmation (52/196 solved-branch, 0 regressions) plus a full 104/104 residual-unsolved sweep found 7 referee-valid solves (R00118 plus 6 new: R00118, R01416, R02530, R02615, R02944, R03209, R03348), 0 regressions across every tested row (156/196, 79.6% branch coverage). See reports/2026-09-25-capability-invention-demand-ew1-routing-exposure-test-design-001.md and reports/2026-09-26-capability-invention-demand-ew1-residual-unsolved-upside-round-design-001.md.',
    STRATEGY_VERY_HIGH_INT_WIDTH2000_HARVEST_KNOT_MUSTCROSS_EXPOSURE: 'Production default-ON, promoted 2026-09-26: adds beam:harvestThenFinish/knotBuilder/mustCrossFirst@beam2000 to the very-high-requiredIntersections non-portal routing rule (attempts.ts), reserve-preserving placement (before the protected trailing pair, not appended after it). Motivated by CID-0028 (data/stress/capability-invention-demand.json): R02696 (requiredIntersections=12, non-portal, singleton-T1-supported) is solved by all three of these score functions in EW1 isolation, but production offered none of them at any width for this feature profile. Pilot+confirmation (52/335 solved-branch, 0 regressions) plus a full 96/96 residual-unsolved sweep found 3 referee-valid solves (R02696 plus 2 new: R02229, R02539), 0 regressions across every tested row (148/335, 44.2% branch coverage). See reports/2026-09-25-capability-invention-demand-ew1-routing-exposure-test-design-001.md and reports/2026-09-26-capability-invention-demand-ew1-residual-unsolved-upside-round-design-001.md.',
...
export const OPT_IN_FEATURES = new Set([
    ...
    'STRATEGY_ADMISSIBLE_ORDER_NON_DEFAULT_RETRY_WORK_CAP_ENFORCEMENT',
...
export const LEGACY_FEATURE_ALIASES: Readonly<Record<string, string>> = Object.freeze({
    ...
    STRATEGY_GOAL_ATTRACTION_LEGACY_DISTANCE_RETRY: 'STRATEGY_GOAL_ATTRACTION_GUIDANCE_DISTANCE_RETRY',
    // Phase-6 derived-vocabulary rename (naming-cleanup-ledger.json): these ablation flags derive
    // from the same stage-id renames as their sibling constants (early-repair-search, main-search,
    // goal-attraction-disabled-retry) but were left unrenamed pending a canonical name decision.
