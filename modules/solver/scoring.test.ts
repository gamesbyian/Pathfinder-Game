...
test('ordering research observer compares profiles and ordering biases without changing survivor order', () => {
  ...
  const off = [...original];
  scoreAndSort(off, pos, state, level, prep, SCORING_PROFILES.default, null);
  ...
  const on = [...original];
  scoreAndSort(on, pos, state, level, prep, SCORING_PROFILES.default, null);
  assert.deepEqual(on, off, 'observer cannot mutate active ranking');
  assert.equal(records.length, 1);
  assert.deepEqual(records[0].candidates, original);
  assert.deepEqual(records[0].rankings.map(row => row.policyId), ['base', 'orderingBias', 'must-cross']);
...
test('ordering divergence contribution sum reproduces the ordinary score-margin change', () => {
  ...
  scoreAndSort([PACK(0, 2), PACK(2, 2)], pos, state, level, prep, SCORING_PROFILES.default);
...
test('scoreMove rewards moving toward an unsatisfied must-turn cell, and stops once it is satisfied', () => {
  ...
  const scoreUnsatisfied = scoreMove(K(2, 1), K(1, 1), state, level, prep, SCORING_PROFILES.default, 3, null);
  ...
  // gain=1; SCORING_PROFILES.default doesn't set mustTurnUrgencyWeight, so wmt defaults to 1) —
  ...
  const scoreSatisfied = scoreMove(K(2, 1), K(1, 1), satisfiedState, level, prep, SCORING_PROFILES.default, 3, null);
  ...
  const scoreRepairUnsatisfied = scoreMove(K(2, 1), K(1, 1), state, level, prep, SCORING_PROFILES.repair, 3, null);
  const scoreRepairSatisfied = scoreMove(K(2, 1), K(1, 1), satisfiedState, level, prep, SCORING_PROFILES.repair, 3, null);
...
test('scoreMove must-turn exit guidance is gated by its own SCORE_MUST_TURN_EXIT_GUIDANCE flag, independent of SCORE_MUST_TURN_URGENCY', () => {
  ...
  const allScoreFlagsOn = {
    ...
    SCORE_ORDERING_BIAS_BONUS: true, SCORE_SURROUND_URGENCY: true, SCORE_ADJ_TURN_URGENCY: true,
    SCORE_MUST_TURN_URGENCY: true, SCORE_MUST_TURN_EXIT_GUIDANCE: true, SCORE_PORTAL_PARITY_GUIDANCE: true,
  };

  const scoreWithGuidance = scoreMove(turningTarget, K(3, 2), state, level, prep, SCORING_PROFILES.default, 5, null);
  prep._cfg = { ...allScoreFlagsOn, SCORE_MUST_TURN_EXIT_GUIDANCE: false };
  const scoreWithoutGuidance = scoreMove(turningTarget, K(3, 2), state, level, prep, SCORING_PROFILES.default, 5, null);
  prep._cfg = null;
...
test('scoreAndSort orders neighbors by extracted score function', () => {
  ...
  scoreAndSort(neighbors, pos, state, level, prep, SCORING_PROFILES.default, null);
...
test('scoreMove returns an identical score with and without a precomputed curCtx (must-pass urgency)', () => {
  ...
  const withCtx = scoreMove(target, pos, state, level, prep, SCORING_PROFILES.default, 5, null, ctx);
...
test('scoreAndSort still orders neighbors correctly using its internally-built curCtx', () => {
  ...
  scoreAndSort(neighbors, pos, state, level, prep, SCORING_PROFILES.objectiveFirst, null);
...
test('scoreMove must-cross urgency: curCtx keeps the same axis for every sibling; the no-curCtx fallback can flip per candidate (the original bug, preserved only for callers that opt out)', () => {
  ...
  const allOtherScoreFlagsOn = {
    ...
    SCORE_ANTI_DITHER: true, SCORE_REVISIT_PENALTY: true, SCORE_ORDERING_BIAS_BONUS: true,
  ...
  for (const [target, label] of [[exitH, 'H-exit'], [exitV, 'V-exit']] as const) {
    const full = scoreMove(target, mcKey, entryState, level, prep, SCORING_PROFILES.default, 6, null, ctx);
    prep._cfg = { ...allOtherScoreFlagsOn, SCORE_MUST_CROSS_URGENCY: false };
    const withoutTerm = scoreMove(target, mcKey, entryState, level, prep, SCORING_PROFILES.default, 6, null, ctx);
    prep._cfg = null;
  ...
  const fullNoCtx = scoreMove(PACK(4, 2), mcKey, postApplyHState, level, prep, SCORING_PROFILES.default, 6, null);
  prep._cfg = { ...allOtherScoreFlagsOn, SCORE_MUST_CROSS_URGENCY: false };
  const withoutTermNoCtx = scoreMove(PACK(4, 2), mcKey, postApplyHState, level, prep, SCORING_PROFILES.default, 6, null);
  prep._cfg = null;
...
test('buildCurUrgencyContext(includeMcAxisFix=false) still populates mpCur but nulls out must-cross fields', () => {
  ...
  const withScopedCtx = scoreMove(PACK(3, 2), mcKey, state, level, prep, SCORING_PROFILES.default, 6, null, ctx);
  const withNoCtx = scoreMove(PACK(3, 2), mcKey, state, level, prep, SCORING_PROFILES.default, 6, null);
...
test('pooled urgency buffers fully overwrite large, zero, small, shifted, then original calls', () => {
  ...
  const snapshot = (keys: number[], pos = PACK(0, 2)) => {
    ...
    const pooled = buildCurUrgencyContext(pos, state, level, prep, true, SCORING_PROFILES.default);
    const fresh = __buildFreshCurUrgencyContextForTests(pos, state, level, prep, true, SCORING_PROFILES.default);
    const target = PACK(1, 2);
    assert.equal(
      scoreMove(target, pos, state, level, prep, SCORING_PROFILES.default, 3, null, pooled),
      scoreMove(target, pos, state, level, prep, SCORING_PROFILES.default, 3, null, fresh),
