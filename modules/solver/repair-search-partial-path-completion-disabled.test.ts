...
deepTest('searchCompletionFromPartialPath disabled cannot rescue R02560 within the shared regression node budget', async () => {
    ...
    assert.equal(
        path,
        null,
        'expected the shared node budget to remain insufficient when only searchCompletionFromPartialPath is disabled',
    );
}, 20000);
