// ============================================================
// theme-engine.ts  –  color utilities + algorithmic derivation
// ============================================================

interface Hsl { h: number; s: number; l: number; }
/** Raw theme seed colors (all optional hex strings). */
type Seeds = Record<string, string | undefined>;
...
export function luminance(hex: string): number {
    const h = hex.replace('#', '').slice(0, 6);
    const toLin = (v: number) => { const n = v / 255; return n <= 0.03928 ? n / 12.92 : Math.pow((n + 0.055) / 1.055, 2.4); };
...
export function readableOn(bg: string, light = '#ffffff', dark = '#0f172a'): string {
...
export function isSeedTheme(theme: any): boolean {
...
export function deriveTokens(seeds: Seeds): any {
    ...
    return {
        ...
        btns: {
            ...
            reject: secondary,
            editClear: secondary,
            editTrapSpots: lighten(secondary, 0.07),
            editNew: hintColor,
        },
        ...
        colors: {
            ...
            falseGoalBlastRing: secondary,
            falseGoalBlastRays: '#f97316',
