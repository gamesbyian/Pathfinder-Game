    ...
    const deps = {
        ...
        engine: {
            navigation: { remapNavKeys: (mapKey: (key: number) => number) => {
                state.ENGINE.nav.path = state.ENGINE.nav.path.map(mapKey);
            } },
...
test('editor rotation and reflection preserve path, portal, axis, and directional-landmark semantics', () => {
    ...
    const originalPath = state.ENGINE.nav.path.slice();
    ...
    assert.deepEqual(state.ENGINE.nav.path, originalPath.map(rotateKey), 'navigation path follows the level rotation');
    ...
    assert.equal(state.ENGINE.editor.pendingPortal, rotateKey(PACK(1, 1)), 'pending portal endpoint follows the rotation');
    assert.equal([...level.mustPassTurnDirs.values()][0], 'cw', 'rotation preserves chirality');

    const beforeMirrorPath = state.ENGINE.nav.path.slice();
    ...
    assert.deepEqual(state.ENGINE.nav.path, beforeMirrorPath.map(mirrorKey), 'navigation path follows the reflection');
    assert.equal(state.ENGINE.editor.pendingPortal, mirrorKey(rotateKey(PACK(1, 1))), 'pending portal endpoint follows the reflection');
    ...
    assert.equal(state.ENGINE.editor.isModified, true);
    assert.equal(state.ENGINE.editor.triggerableFalseGoalCells.size, 0);
